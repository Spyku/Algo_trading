...
def build_hourly_features(df_hourly, horizon=PREDICTION_HORIZON, verbose=True, keep_label_nan_tail=False):
    ...
    # ===== Added 2026-06-08 (mirror of faye; feattest DV picked 5/5) =====
    # vol-of-vol + KAMA(10) adaptive trend. Price-derived, causal (rolling backward / recursive forward),
    # 0h lag (point-in-time hourly), NO leak, NO daily/24h lag, NO candle issue.
...
def build_all_features(df_hourly, asset_name='BTC', horizon=PREDICTION_HORIZON, verbose=True, keep_label_nan_tail=False):
    ...
    if funding_series is not None:
        ...
        df['deriv_funding_chg3d'] = df['deriv_funding_rate'].diff(72) * 100  # NEW 2026-06-08 (mirror of faye) — 3-day funding momentum, 0h lag
        ...
        try:
            ...
            if 'perp_close' in deriv_df.columns:
                ...
                df['deriv_basis_zscore72'] = (df['deriv_basis'] - df['deriv_basis'].rolling(72, min_periods=24).mean()) / \
                                             (df['deriv_basis'].rolling(72, min_periods=24).std() + 1e-10)  # NEW 2026-06-08 (mirror of faye) — faster basis regime, 0h lag
