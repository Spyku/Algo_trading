...
CLI Usage:
  python crypto_trading_system_ein.py P BTC 6h
  python crypto_trading_system_ein.py DV BTC 8h
  python crypto_trading_system_ein.py H BTC 5,6,7,8h
  python crypto_trading_system_ein.py D BTC,ETH 6,7h

Outputs:
  models/crypto_ein_best_models.csv       (top 6 candidates from Mode D)
  models/crypto_ein_production.csv         (best live performer from Mode V)
  config/regime_config_ein.json                  (horizon + min_confidence per asset)
...
============================================================
  ...
  2. COMPLETE HORIZON SWEEP — Run BTC 7h and 8h:
       python crypto_trading_system_ein.py DV BTC 7h
       python crypto_trading_system_ein.py DV BTC 8h
...
ASSETS = {
    ...
    'ADA':   {'source': 'binance', 'ticker': 'ADA/USDT',  'file': 'data/ada_15m_data.csv',  'start': '2018-04-01T00:00:00Z'},
    'AVAX':  {'source': 'binance', 'ticker': 'AVAX/USDT', 'file': 'data/avax_15m_data.csv', 'start': '2021-09-01T00:00:00Z'},
    'DOT':   {'source': 'binance', 'ticker': 'DOT/USDT',  'file': 'data/dot_15m_data.csv',  'start': '2020-08-01T00:00:00Z'},
}

CANDLE_MINUTES = 15               # Ein = 15-minute candles
...
REPLAY_HOURS = 800          # ~200h at 15min candles
REPLAY_HOURS_S = 1600       # Mode S — longer window
DIAG_STEP = 144             # walk-forward step (= 36h at 15min)
DIAG_WINDOWS = [200, 400, 600, 800]  # candle windows (50h to 200h at 15min)
...
MODE_G_REPLAY_HOURS = 1344      # 2 full weeks at 15min candles (336*4)
...
def _compute_pysr_features(df, all_cols, asset_name, horizon, verbose=True):
    ...
    pysr_path = os.path.join(models_dir, f'pysr_{asset_name}_{horizon}c_15m.json')
...
def _check_pysr_leakage(features, asset, horizon):
    ...
    models_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models')
    pysr_path = os.path.join(models_dir, f'pysr_{asset}_{horizon}c_15m.json')
...
GRID_WINDOWS = [200, 400, 600, 800, 1000, 1200]  # candle windows (50h-300h at 15min)
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False):
    ...
    print("\n" + "=" * 70)
    print(f"  EIN V1.0 MODE D: EXHAUSTIVE GRID -- {horizon}h HORIZON")
...
def run_mode_v(assets_list, horizons=None):
    ...
    if production_models:
        ...
        tcfg_path = f'{CONFIG_DIR}/regime_config_ein.json'
        try:
...
def run_mode_s(assets_list, horizons, args=None):
    """
    Ein Mode S: Given a regime detector and bull/bear horizon pair (from regime_config_ein.json
    ...
    Flow:
        1. Read regime_config_ein.json for detector + horizon pair
        ...
        6. Write optimized config to regime_config_ein.json
    """
...
Examples:
  python crypto_trading_system_ein.py HRS BTC 5,6,7,8h          # full Ed pipeline
