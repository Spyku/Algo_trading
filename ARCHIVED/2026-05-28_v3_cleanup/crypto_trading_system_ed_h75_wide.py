"""
Ed — H75_WIDE fork (2026-05-26) — extended grid with high-window basin coverage
============================================================
Built from production engine (H75 K=5 multi-seed + strict (combo,w) dedup + 75
refine trials). Sole purpose: test whether adding GRID_WINDOWS [250, 300] and
proportionally bumping the V2/V3 top-N funnels lets refine reach LIVE's
w=281/w=293 basin from a single HRST, rather than depending on data-state luck.

Changes vs production crypto_trading_system_ed.py:
  - GRID_WINDOWS:        [72,100,150] -> [72,100,150,250,300]   (3 -> 5 windows)
  - DOOHAN_SAVE_TOP_N:   10 -> 17                                (V2 funnel; 1 per (combo,w) cluster + 2 buffer)
  - REFINE_TOP_N:        3  -> 5                                 (V3 inputs proportional to cluster growth)
  - REFINE_WINDOW_RANGE: 20 -> 25                                (bridges 72-150 into continuous coverage; 175-225 gap remains by design)

Holdout 0-trade filter kept ON (per user 2026-05-26: "if there is zero trades
setup should be ignored"). w=250/300 candidates that fire 0 trades across all
3 holdout folds will still be filtered; this is intentional, not a bug. Test
question becomes "given the filter, do high-window grid points survive?".

Output isolation: write paths read from H75_WIDE_MODELS_DIR / H75_WIDE_CONFIG_DIR
env vars (defaults: 'models' / 'config' — set them to keep this fork's noprod
writes off the production tree and off any G_narrow_d-tagged tree).

Run safely with --no-persist (writes to *_noprod.csv / *_noprod.json variants
inside the redirected dirs). Production engine source / live CSV / live regime
...
# ── H75_WIDE output isolation (added 2026-05-26) ────────────────────────────
# H75_WIDE fork uses its own env var prefix (H75_WIDE_*) to avoid colliding
# with the production engine's H_STRICT_MODELS_DIR / H_STRICT_CONFIG_DIR.
# Both env vars fall back to production engine's vars, then to 'models'/'config'.
# Set H75_WIDE_MODELS_DIR + H75_WIDE_CONFIG_DIR to keep H75_WIDE writes off the
# production tree AND off any concurrent G_narrow_d-tagged tree on the same
# Drive-synced engine folder.
H75_MODELS_DIR = os.environ.get(
    'H75_WIDE_MODELS_DIR',
    os.environ.get('H_STRICT_MODELS_DIR', 'models'),
)
H75_CONFIG_DIR = os.environ.get(
    'H75_WIDE_CONFIG_DIR',
    os.environ.get('H_STRICT_CONFIG_DIR', 'config'),
)
if H75_MODELS_DIR != 'models' or H75_CONFIG_DIR != 'config':
    print(f"[H75_WIDE_ISO] output dirs redirected: "
          f"models={H75_MODELS_DIR} config={H75_CONFIG_DIR}")

# Engine's to_csv / Optuna-checkpoint calls assume parent dirs exist.
# When env-var redirected, create them up-front so the first grid CSV write
# doesn't crash with OSError. Same fix G_narrow_d_parallel landed for its
# isolated dirs. Idempotent — safe to re-run.
for _d in (H75_MODELS_DIR, H75_CONFIG_DIR, f'{H75_MODELS_DIR}/.resume_hourly'):
...
GRID_WINDOWS = [72, 100, 150, 250, 300]  # H75_WIDE: extended 2026-05-26 from [72,100,150] to test whether high-window basin (LIVE 5h=281, 8h=293) surfaces in a single HRST.
...
REFINE_TOP_N = 5                   # H75_WIDE: bumped from 3 (proportional to cluster growth 9 -> 15 from GRID_WINDOWS extension)
...
REFINE_WINDOW_RANGE = 25           # H75_WIDE: bumped from 20 to bridge GRID_WINDOWS [72,100,150,250,300] into continuous coverage except 175-225 gap.
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False, replay_hours=None):
    ...
    for asset_name in assets_list:
        ...
        grid_csv_path = os.path.join(H75_MODELS_DIR, f'crypto_ed_grid_{asset_name}_{horizon}h{_tag}.csv')
        ...
        DOOHAN_SAVE_TOP_N = 17  # H75_WIDE: bumped from 10 (which was for 9 clusters + 1 buffer). With GRID_WINDOWS extension producing 15 (combo,w) clusters, 17 gives 15 reps + 2 buffer slots (matches current proportional eval-coverage 10/108 -> 17/180).
        candidates_to_save = []
        seen_regions = set()
