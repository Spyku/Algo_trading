...
# C05 fork (2026-05-09): default = 'cvar' so grid CSVs go to
# crypto_ed_grid_<asset>_<h>h_cvar.csv (untagged path is untouched, used
# by live engine + production smoke tests). Override with --grid-tag NAME.
GRID_TAG_SUFFIX = 'cvar'
...
# Optuna scoring metric — set via --metric flag.
# C05 (CVaR_5%) fork (2026-05-09): default flipped to 'cvar' so this engine fork
# uses CVaR-aware config selection throughout Mode D + Mode V + Mode S/T.
# 'cvar' = cum_return - CVAR_LAMBDA × |CVaR_5%|, where CVaR_5% = mean of worst 5%
# per-trade returns (negative number). λ=1.0 default (smoke-test value).
# Backtest evidence: avg +22pp Mode T return delta vs APF on clean ETH 5,6,7,8h
# grids with pysr included (M-32 fix). 6h+7h clean wins, 7h shows +52pp return
# AND -6pp DD improvement.
OPTUNA_METRIC = 'cvar'
VALID_METRICS = {'apf', 'rawpf', 'calmar', 'return', 'rpf_sqrt', 'cvar'}
CVAR_LAMBDA = 1.0   # weight on |cvar_5pct| penalty. Sweep {0.5, 1.0, 2.0} if tuning.
CVAR_ALPHA = 0.05   # 5% tail (worst 5% of trades averaged for CVaR)
...
# C05 fork: write to _cvar paths so live trader never sees these files.
# Live trader reads crypto_ed_production.csv + regime_config_ed.json (untouched).
PRODUCTION_CSV = 'models/crypto_ed_production_cvar.csv'
REGIME_CONFIG_PATH = 'config/regime_config_ed_cvar.json'
...
def _compute_optuna_score(result):
    ...
    Result tuple: (combo, window, acc, total, cum_return, win_rate,
                   ...
                   cvar_5pct)  ← C05 fork: index 13 added"""
    cum_return = result[4]
    trades = result[6]
    max_dd = result[9]  # already in %
    adjusted_pf = result[10]
    raw_pf = result[11]
    # C05 fork: cvar_5pct at index 13 (negative %, mean of worst 5% trades).
    # Tuple may be 13-long (legacy callers / first-sim) or 14-long (with cvar).
    cvar_5pct = result[13] if len(result) > 13 else 0.0
    ...
    elif OPTUNA_METRIC == 'cvar':
        # C05: penalized return: cum_return − λ × |cvar_5pct|.
        # cum_return + cvar_5pct both in %, direct subtraction with lambda weight.
        return cum_return - CVAR_LAMBDA * abs(cvar_5pct)
    else:
        return adjusted_pf
...
def _eval_one_config(features_np, labels_np, closes_np, combo, window, n, step, model_factories, pred_horizon=4, gamma=1.0):
    ...
    # C05 fork: compute CVaR_5% (mean of worst 5% per-trade returns, in %).
    # negative number; |cvar_5pct| is the tail-loss magnitude.
    if len(trade_returns) >= 4:
        import numpy as _np
        sorted_r = sorted(trade_returns)
        k = max(1, int(len(sorted_r) * CVAR_ALPHA))
        cvar_5pct = float(_np.mean(sorted_r[:k])) * 100  # to %
    else:
        cvar_5pct = 0.0  # too few trades to estimate tail; treat as no penalty

    # Return tuple: index 10 = adjusted_pf, 11 = raw_pf, 12 = bh_pf, 13 = cvar_5pct (NEW)
    return ('+'.join(combo), window, accuracy, total, cum_return, win_rate,
            trades, avg_gain, avg_loss, max_dd * 100, adjusted_pf, raw_pf, bh_pf, cvar_5pct)
...
def run_diagnostic_for_asset(asset_name, df_features, feature_cols, gamma=1.0, resume=False, horizon=None):
    ...
    for result in all_results:
        if result is None:
            continue
        combo_name, window, acc, n_total, cum_ret, win_rate, trades, avg_gain, avg_loss, max_dd, adjusted_pf, raw_pf, bh_pf, *_cvar_extra = result
    ...
    if sorted_results:
        ...
        with open(csv_path, 'w', newline='') as f:
            ...
            for r in sorted_for_csv:
                combo_name, window, acc, n_total, cum_ret, win_rate, trades, avg_gain, avg_loss, max_dd, adjusted_pf, raw_pf, bh_pf, *_cvar_extra = r
...
def _deku_eval_with_pruning(features_np, labels_np, closes_np, combo, window, n,
    ...
    total_gain = 0.0
    total_loss = 0.0
    trade_returns = []  # C05 fork: per-trade return list for CVaR_5% computation
    ...
    for i in range(min_start, n, step):
        ...
        if ensemble_pred == 1 and not in_position:
            ...
            portfolio *= (1 + trade_return)
            trades += 1
            trade_returns.append(trade_return)  # C05 fork
            if trade_return > 0:
                wins += 1
                total_gain += trade_return
            else:
    ...
    if len(trade_returns) >= 4:
        ...
        sorted_r = sorted(trade_returns)
        k = max(1, int(len(sorted_r) * CVAR_ALPHA))
        cvar_5pct = float(_np.mean(sorted_r[:k])) * 100
    else:
        cvar_5pct = 0.0

    return ('+'.join(combo), window, accuracy, total, cum_return, win_rate,
            trades, total_gain, total_loss, max_dd * 100, adjusted_pf, raw_pf, bh_pf, cvar_5pct)
