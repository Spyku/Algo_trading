...
GRID_TAG_SUFFIX = 'cpcv'  # CPCV fork: write to crypto_ed_grid_<asset>_<h>h_cpcv.csv by default
...
PRODUCTION_CSV = 'models/crypto_ed_production_cpcv.csv'
REGIME_CONFIG_PATH = 'config/regime_config_ed_cpcv.json'

# CPCV fork constants (research-only — production untouched)
CPCV_N_GROUPS = 6           # 6 contiguous groups -> C(6,2)=15 train/test paths
CPCV_K_TEST = 2             # k groups held out as test set per path
CPCV_PURGE_CANDLES = 8      # purge rows at train/test boundary (>= max horizon)
CPCV_PBO_MAX = 0.50         # reject candidates where >50% of paths lose money
CPCV_TOP_N_REVALIDATE = 6   # how many top-grid candidates to re-validate via CPCV
CPCV_MIN_PATH_TRADES = 4    # require at least N trades per path to count its return
...
# ============================================================
# CPCV (Combinatorial Purged Cross-Validation) — research-only
# Reference: López de Prado, AFML Ch 12 ("Backtesting through Cross-Validation")
# Reference: Bailey & López de Prado 2014, "The Probability of Backtest Overfitting"
#
# Why fork: the production engine uses single walk-forward + 3-fold rolling
# holdout to rank Mode D candidates. CPCV adds a 15-path validation pass on
# the top-6 candidates to (a) re-rank by median fold return, (b) compute PBO
# (probability of backtest overfitting) per candidate.
#
# DESIGN OPTION A — adaptive gamma per fold (vs Deku V1.4 which forced
# gamma=1.0 and "failed"). Each fold's walk-forward backtest uses the
# candidate's NATURAL gamma; recency bias is preserved within each fold's own
# rolling training window. The "global gamma weights subset per fold" idea
# was the user's framing, but the engine's online-retrain architecture
# accomplishes the same effect by keeping the candidate's gamma intact
# rather than disabling decay.
# ============================================================


def _build_cpcv_paths(n_total, n_groups=CPCV_N_GROUPS, k_test=CPCV_K_TEST,
                       purge=CPCV_PURGE_CANDLES):
    """Split n_total contiguous rows into n_groups, generate C(n_groups, k_test)
    train/test partitions with purge rows removed at boundaries.

    Returns: list of (train_indices, test_indices) tuples — np.arrays.
    """
    ...
    for g in range(n_groups):
        s = g * group_size
        e = (g + 1) * group_size if g < n_groups - 1 else n_total
        boundaries.append((s, e))

    paths = []
    for test_combo in _combinations(range(n_groups), k_test):
        test_set = set()
        for g in test_combo:
            s, e = boundaries[g]
        ...
        train_idx = np.array(sorted(i for i in range(n_total)
...
def _compute_pbo(path_returns):
    """Probability of Backtest Overfitting = fraction of paths with return <= 0.
    Lower is better. PBO > 50% => more paths lose than gain (overfit risk)."""
    if not path_returns:
        return 1.0
...
def _cpcv_evaluate_one_candidate(c_combo_str, c_window, c_gamma, c_n_feat,
                                    features_np_all, labels_np_all, closes_np_all,
                                    ranked_features, model_factories, horizon,
                                    cpcv_step=12, verbose=False):
    """Run CPCV evaluation for a single candidate config.

    Returns dict: {median_ret, pbo, n_positive, n_paths, path_rets, path_trades}.
    """
    n_total = len(features_np_all)
    sel_idx = _feature_floor_indices(ranked_features, c_n_feat)
    feat_subset = features_np_all[:, sel_idx]
    c_combo = c_combo_str.split('+')

    paths = _build_cpcv_paths(n_total)
    path_rets = []
    path_trades = []

    for pi, (train_idx, test_idx) in enumerate(paths):
        if len(test_idx) < 100:
            path_rets.append(0.0)
            path_trades.append(0)
            continue
        # Walk-forward backtest on test partition (V1.4 pattern)
        # Adaptive gamma: candidate's natural c_gamma is passed through to
        # _deku_eval_with_pruning, which applies decay weights within each
        # rolling training window. This preserves recency bias INSIDE the
        # fold while still validating across multiple disjoint test slices.
        p_feat = feat_subset[test_idx]
        p_labels = labels_np_all[test_idx]
        p_closes = closes_np_all[test_idx]
        n_p = len(test_idx)
        ...
        try:
            ...
            if p_result and p_result[6] >= CPCV_MIN_PATH_TRADES:
                path_rets.append(float(p_result[4]))   # cum_return %
                path_trades.append(int(p_result[6]))    # n_trades
            else:
                path_rets.append(0.0)
                path_trades.append(0)
        except Exception as e:
            if verbose:
                print(f"    [cpcv] path {pi+1}/{len(paths)} FAILED: {type(e).__name__}: {e}")
            path_rets.append(0.0)
            path_trades.append(0)

    if not path_rets:
        return {'median_ret': 0.0, 'pbo': 1.0, 'n_positive': 0,
                'n_paths': 0, 'path_rets': [], 'path_trades': []}
    ...
    return {
        ...
        'n_paths': len(path_rets),
        'path_rets': path_rets,
        'path_trades': path_trades,
    }


def _cpcv_revalidate_top_candidates(candidates_to_save, features_np_all,
                                       labels_np_all, closes_np_all,
                                       ranked_features, model_factories,
                                       horizon, asset_name):
    """Re-validate the Mode D top candidates via CPCV (15 paths each).
    Re-rank by median CPCV return, attach CPCV stats to each candidate.

    Mutates candidates_to_save in place by adding (cpcv_dict,) to each tuple.
    Returns the re-ordered list.
    """
    print(f"\n  {'='*70}")
    print(f"  CPCV RE-VALIDATION: {asset_name} {horizon}h "
          f"(top {len(candidates_to_save)} candidates × {CPCV_N_GROUPS}-group/{CPCV_K_TEST}-test "
          f"= 15 paths each)")
    print(f"  PBO threshold: <= {CPCV_PBO_MAX*100:.0f}% | Re-rank by median path return")
    print(f"  {'='*70}")

    augmented = []
    for ci, ho_entry in enumerate(candidates_to_save):
        trial = ho_entry[0]
        ...
        t0 = time.time()
        cpcv = _cpcv_evaluate_one_candidate(
            c_combo, c_window, c_gamma, c_n_feat,
            features_np_all, labels_np_all, closes_np_all,
            ranked_features, model_factories, horizon
        )
        dur_s = time.time() - t0
        print(f"  [{ci+1}/{len(candidates_to_save)}] {c_combo:<12s} w={c_window:3d}h "
              f"g={c_gamma:.4f} f={c_n_feat:2d}  "
              f"CPCV med_ret={cpcv['median_ret']:+6.2f}%  "
              f"PBO={cpcv['pbo']*100:3.0f}%  "
              f"+{cpcv['n_positive']}/{cpcv['n_paths']}  "
              f"({dur_s:.1f}s)")

        # Append cpcv dict as the LAST element in the tuple
        augmented.append(ho_entry + (cpcv,))

    # Re-sort by CPCV median return (descending). Penalize PBO>max by zeroing.
    def _cpcv_rank(entry):
        cpcv = entry[-1]
        if cpcv['pbo'] > CPCV_PBO_MAX:
            return -999.0
        return cpcv['median_ret']

    augmented.sort(key=_cpcv_rank, reverse=True)

    # Print final ordering
    print(f"\n  {'CPCV-RANK':<12s} {'med_ret':>8s} {'PBO':>5s} {'+/N':>6s}  combo                    w    g       f")
    print(f"  {'-'*100}")
    for i, entry in enumerate(augmented, 1):
        trial = entry[0]
        cpcv = entry[-1]
        flag = " <-- BEST" if i == 1 else (" REJECTED (PBO>50%)" if cpcv['pbo'] > CPCV_PBO_MAX else "")
        print(f"  #{i:<11d} {cpcv['median_ret']:+7.2f}% {cpcv['pbo']*100:4.0f}% "
              f"{cpcv['n_positive']:3d}/{cpcv['n_paths']:<2d}  "
              f"{trial.params['combo']:<22s} {trial.params['window']:3d}h "
              f"{trial.params['gamma']:.4f} {trial.params['n_features']:2d}{flag}")
...
def run_mode_p(assets_list, horizons):
    ...
    try:
        from pysr_discover_features import save_results
    except ImportError:
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False, replay_hours=None):
    ...
    for asset_name in assets_list:
        ...
        # ═══════════════════════════════════════════════════════════════
        # CPCV FORK INSERT: re-validate top candidates via 15-path CPCV
        # before writing to grid CSV. Re-ranks by median CPCV return.
        # Adds (cpcv_dict,) as the LAST element of each candidate tuple.
        # ═══════════════════════════════════════════════════════════════
        candidates_to_save = _cpcv_revalidate_top_candidates(
            candidates_to_save, features_np_all, labels_np_all, closes_np_all,
            ranked_features, model_factories, horizon, asset_name
        )

        print(f"\n  {'='*70}")
        print(f"  TOP {len(candidates_to_save)} CANDIDATES (CPCV-re-ranked): {asset_name} {horizon}h")
        print(f"  {'='*70}")

        for rank_i, ho_entry in enumerate(candidates_to_save, 1):
            # CPCV fork: cpcv dict is the LAST element of the tuple
            trial, ho_score, ho_ret, ho_acc, ho_trades, ho_raw_pf, ho_fold_scores, ho_fold_rets, cpcv = ho_entry
            c_combo = trial.params['combo']
            c_window = trial.params['window']
            c_gamma = trial.params['gamma']
            c_n_feat = trial.params['n_features']
            ...
            cpcv_tag = f"cpcv_med={cpcv['median_ret']:+.1f}% pbo={cpcv['pbo']*100:.0f}%"
            print(f"  #{rank_i}: {c_combo}  w={c_window}h  g={c_gamma:.4f}  f={c_n_feat}  "
                  f"ho_apf={ho_score:.3f}  ho_ret={ho_ret:+.1f}%  [{fold_str}]  "
                  f"{cpcv_tag}  {c_sampler}  {gen_icon}{marker}")
            ...
            best_config = {
                ...
                'rank': rank_i,
                'sampler': c_sampler,
                # CPCV fork: persist CPCV stats to grid CSV
                'cpcv_median_ret': round(cpcv['median_ret'], 2),
                'cpcv_pbo': round(cpcv['pbo'], 3),
                'cpcv_n_positive': cpcv['n_positive'],
...
def _build_regime_indicators_and_detectors(asset):
    ...
    # named detector found in regime_config_ed.json (back-compat for legacy
