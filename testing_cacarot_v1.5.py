"""
TESTING: Cacarot V1.5 — Expanded Feature Set
============================================================
Tests 32 previously-excluded features with temporal decay.
Hypothesis: decay makes short-term features useful again.

Compares FEATURE_SET_C (all 49 base features) vs production (18 in Set A).
Results saved to models/testing_cacarot_v1.5_results.csv

Usage:
  python testing_cacarot_v1.5.py D BTC 4,8h
  python testing_cacarot_v1.5.py D ETH 4,8h

Based on V1.5 Cacarot TEST production.
"""
...
# V1.5 TEST: All 49 base technical features — previously excluded ones may
# become useful with temporal decay weighting recent data more heavily
FEATURE_SET_C = [
    # Logreturns (all 12)
    ...
    'price_jerk_1h',
    # V1.5 TEST: 15 rejected V5.6 features
    'parkinson_vol_14h', 'rogers_satchell_vol_14h',
    'hurst_168h', 'mfi_14h',
    'realized_skew_48h', 'realized_skew_168h',
    'realized_kurt_48h', 'realized_kurt_168h',
    'amihud_14h', 'obv_roc_20h',
    'close_position', 'body_ratio', 'close_pos_12h', 'body_ratio_12h',
    'cci_20h',
]

ACTIVE_FEATURE_SET = 'C'
...
def _get_active_features():
    if ACTIVE_FEATURE_SET == 'C':
        return list(FEATURE_SET_C)
...
def _get_set_label():
    if ACTIVE_FEATURE_SET == 'C':
        return f"Set C (ALL 49 base technical, {len(FEATURE_SET_C)} features)"
    if ACTIVE_FEATURE_SET == 'B':
...
def build_hourly_features(df_hourly, horizon=PREDICTION_HORIZON, verbose=True):
    ...
    # ---- V1.5 TEST: REJECTED V5.6 FEATURES (re-test with decay) ----
    # Parkinson (1980): high-low range volatility, 5.2x more efficient
    park_single = np.log(df['high'] / df['low'])**2 / (4 * np.log(2))
    df['parkinson_vol_14h'] = park_single.rolling(14).mean().apply(np.sqrt)

    # Rogers-Satchell (1991): works with non-zero drift (trending markets)
    ...
    # Hurst exponent (regime detection) — rolling R/S over 168h
    ...
    for w in [48, 168]:
        ...
        df[f'realized_kurt_{w}h'] = standardized.pow(4).rolling(w).mean() - 3
    ...
    feature_cols = [
        ...
        'price_accel_1h', 'price_accel_4h', 'price_accel_12h', 'price_accel_24h',
        'price_jerk_1h',
        # V1.5 TEST: 15 rejected V5.6 features (re-test with decay)
        'parkinson_vol_14h', 'rogers_satchell_vol_14h',
        'hurst_168h', 'mfi_14h',
        'realized_skew_48h', 'realized_skew_168h',
        'realized_kurt_48h', 'realized_kurt_168h',
...
DATA_DIR = 'data'
CHARTS_DIR = 'charts/v1.5_test'
MODELS_DIR = 'models'
CONFIG_DIR = 'config'
# Test harnesses set this env var to redirect model output away from production CSV
MODELS_CSV_OVERRIDE = f'{MODELS_DIR}/testing_cacarot_v1.5_results.csv'
...
def run_mode_d(assets_list, horizon=PREDICTION_HORIZON, permtest=False, resume=False):
    ...
    print("\n" + "=" * 60)
    print(f"  MODE D: FULL PIPELINE -- {horizon}h HORIZON (V1.5 Cacarot TEST)")
...
def main():
    ...
    # Training period removed — decay weighting handles recency (V1.5 Cacarot TEST)
    # ================================================================
    ...
    if cli_args and cli_args[0].upper() in ('B', 'D', 'DF', 'E', 'F', '5', '6', '7'):
        ...
        print("=" * 60)
        print("  CRYPTO HOURLY ML TRADING SYSTEM -- V1.5 Cacarot TEST")
        ...
        if mode in ('D', 'E'):
            print("\n  [V1.5 Cacarot TEST] Uses ALL available data — decay weighting handles recency")
