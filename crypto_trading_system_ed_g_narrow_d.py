"""
Ed — G_narrow_d fork (2026-05-16) — narrow Mode D grid + force combo diversity + wide V3 refine
============================================================
Built from production engine + B (K=5 multi-seed). Changes vs prod:
  - GRID_FEATURES: [10,13,17,25] -> [10,15,20]      (wide spacing; let V3 fill gaps)
  - GRID_GAMMAS:   [0.999,0.997,0.995] -> [0.999,0.996]   (wide spacing)
  - N_FEATURES_RANGE upper -> 100                    (drop hard cap)
  - REFINE_TRIALS: 50 -> 75                          (more V3 trials per seed)
  - V1 top-6 selection: forces >=1 of each combo in (RF+LGBM, XGB+LGBM)
  - _diversity_key: includes combo in tuple
  - V3 refine search: ABSOLUTE ranges window[50,300] features[4,60] gamma[0.995,1.0]
  - K=5 multi-seed denoising inlined (same as B's reliability_multi_seed)

Hypothesis: narrower V1 with wide spacing + drop n_features cap in V3 fixes the
B 7h failure mode where 9 tied APF=14.42 XGB+LGBM 72h candidates monopolized
top 6 and RF+LGBM never reached the refine pool. Tested 7h-only first against
B's snapshot.

Snapshot reuse: set V2_DATA_SNAPSHOT env var to a snapshot directory. The fork
applies pd.read_csv redirect at import time (same mechanism as
...
Original prod docstring follows:
============================================================
...
Pipeline:
  Mode D: Narrow grid (2 combos × 3 windows × 3 features × 2 gammas = 36 evals)
    - Combos: RF+LGBM, XGB+LGBM
    - Windows: 72, 100, 150
    - Features: 10, 15, 20 (wide-spaced; V3 refine fills gaps)
    - Gammas: 0.999, 0.996
  Mode V: Backtest top 6 (force diversity) → 75-trial Optuna refine per top 3 → pick winner
...
# ============================================================
# G_NARROW_D: snapshot redirect block (inlined from _idea_patchers/v2_data_snapshot.py)
# When V2_DATA_SNAPSHOT env var is set, redirect pd.read_csv reads of data/*
# files into the snapshot directory. Lets G_narrow_d reuse B's frozen snapshot.
# ============================================================
_G_SNAPSHOT_DIR = os.environ.get('V2_DATA_SNAPSHOT')
if _G_SNAPSHOT_DIR:
    from pathlib import Path as _G_Path
    _g_snapshot = _G_Path(_G_SNAPSHOT_DIR).resolve()
    _g_data_dir = _g_snapshot.parent.resolve()
    _g_orig_read_csv = pd.read_csv

    def _g_redirected_read_csv(filepath_or_buffer, *args, **kwargs):
        try:
            if isinstance(filepath_or_buffer, (str, os.PathLike)):
                p = _G_Path(filepath_or_buffer).resolve()
                try:
                    rel = p.relative_to(_g_data_dir)
                except ValueError:
                    return _g_orig_read_csv(filepath_or_buffer, *args, **kwargs)
                rel_str = str(rel)
                # Recursion guard — any snapshot prefix
                if rel_str.startswith('_v2_snapshot_') or rel_str.startswith('_reliability_'):
                    return _g_orig_read_csv(filepath_or_buffer, *args, **kwargs)
                redirected = _g_snapshot / rel
                if redirected.exists():
                    return _g_orig_read_csv(str(redirected), *args, **kwargs)
        except Exception:
            pass
        return _g_orig_read_csv(filepath_or_buffer, *args, **kwargs)
...
# G_NARROW_D: drop hard cap. LGBM's internal reg already prunes; capping at 40/80
# is what created B's 7h tied-APF block (couldn't escape the low-feature trap).
N_FEATURES_RANGE = {
    HORIZON_SHORT: (4, 100),
    HORIZON_LONG:  (4, 100),
}
N_FEATURES_RANGE_DEFAULT = (4, 100)
...
GRID_GAMMAS = [0.999, 0.996]       # G_NARROW_D: wide spacing (gap 0.003) so V3 refine
                                   # interpolates new values (0.997, 0.998, etc.).

# Refine step: Optuna fine-tuning around top 3 live-validated configs
REFINE_TOP_N = 3                   # how many configs to refine from Mode V
REFINE_TRIALS = 75                 # G_NARROW_D: was 50; more trials with smaller V1.
REFINE_GAMMA_RANGE = 0.002         # unused in G_NARROW_D — V3 ranges are absolute (see refine fns)
REFINE_FEAT_RANGE = 5              # unused in G_NARROW_D
REFINE_WINDOW_RANGE = 20           # unused in G_NARROW_D
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False, replay_hours=None):
    ...
    for asset_name in assets_list:
        ...
        # G_NARROW_D: combo added to diversity tuple AND force >=1 of each combo.
        def _diversity_key(trial):
            c = trial.params['combo']
        ...
        seen_regions = set()
        seen_combos = set()
        required_combos = set(GRID_COMBOS)
        if holdout_results:
            # G_NARROW_D Pass A: force >=1 of each combo (take highest APF per combo)
            for ho_entry in holdout_results:
                if seen_combos == required_combos:
                    break
                trial = ho_entry[0]
                ...
                seen_regions.add(_diversity_key(trial))
                candidates_to_save.append(ho_entry)
            # Pass B: diversity dedup over remaining APF-sorted candidates
            for ho_entry in holdout_results:
                if len(candidates_to_save) >= DOOHAN_SAVE_TOP_N:
                    break
                if ho_entry in candidates_to_save:
                    continue
                trial = ho_entry[0]
                ...
                candidates_to_save.append(ho_entry)
            # Pass C: fill any remaining slots
            if len(candidates_to_save) < DOOHAN_SAVE_TOP_N:
                for ho_entry in holdout_results:
                    if len(candidates_to_save) >= DOOHAN_SAVE_TOP_N:
                        break
...
def _refine_top_configs(asset, horizon, top3_for_refine, df_raw, df_clean, all_cols, ranked_features):
    ...
    for cfg_idx, (lbl, r) in enumerate(top3_for_refine):
        ...
        # Search ranges
        # G_NARROW_D: absolute ranges (not seed-relative). V3 explores the full
        # space between V1 seeds, filling gaps the wide V1 spacing left open.
        gamma_lo = 0.995
        gamma_hi = 1.0
        feat_lo = 4
...
# ============================================================
# G_NARROW_D: K=5 multi-seed denoising (inlined from _idea_patchers/reliability_multi_seed.py)
...
# ============================================================
_G_K = int(os.environ.get('RELIABILITY_K', '5'))
_G_SEEDS = list(range(42, 42 + _G_K))
_G_ORIG_DEKU_EVAL = _deku_eval_with_pruning


def _g_factories_seeded(seed):
...
def _g_deku_eval_median_k(features_np, labels_np, closes_np, combo, window, n,
                          step, model_factories, gamma=1.0, trial=None,
                          horizon=None):
    ...
    results = []
    for seed in _G_SEEDS:
        factories = _g_factories_seeded(seed)
        r = _G_ORIG_DEKU_EVAL(
