"""
FUJIWARA_30 — 30-MINUTE-candle ML Trading System (research fork of FAYE, 2026-06-19)
====================================================================================
Self-contained 30-minute-candle fork of crypto_trading_system_faye.py. Built to
...
CHINESE WALL — NO INFECTION OF PRODUCTION. Research-only, physically walled:
  * Output -> models_fujiwara_30/ + config_fujiwara_30/ (never models/ or config/).
  * Price data -> data/<asset>_30m_data.csv (never *_hourly_data.csv).
...
HORIZONS ARE IN CANDLE (PERIOD) UNITS: 5,6,7,8 periods = 150, 180, 210, 240 minutes.
...
What was inlined and from where:
  ...
  P1  Identity rebrand                  (was Ed module renames)
       FAYE_MODELS_DIR / FAYE_CONFIG_DIR / crypto_fujiwara_30_production.csv /
       regime_config_fujiwara_30.json — isolated paths, no Ed file collisions.
  ...
  P5b Mode D 8-worker grid dispatcher   (was v3 inspect-frame state capture)
       ...
       seed-by-seed breakdown at models_fujiwara_30/mode_d_full_*.csv.
...
NEAR_LIVE_MODE defaults (always on, no env-var gating):
  ...
  - na_policy = 'mean_last_10'  (smoothed fill — was 'ffill' in Ed; near-equivalent in practice)
...
Pipeline (all modes):
  Mode D: Exhaustive grid (2-3 combos × 5 windows × 3 features × 2 gammas = 60 evals)
  Mode V: Backtest top 10 → Optuna refine top 3 → 3-fold holdout → winner
  Mode R: Regime backtest (bull/bear horizon pairs × detectors)
  Mode S: Shield tuning (often picks 'no shield' = OFF per the empirical
          memory `feedback_trading_analysis.md`)
...
Core contract:
  ...
  - Fee: BACKTEST_FEE_PER_LEG = 0.0005 (5 bps maker blend) for ALL backtest sims
...
CLI (order-independent, identical to v3):
  python crypto_trading_system_faye.py HRST "ETH," 5h,6h,7h,8h --replay 1440
  python crypto_trading_system_faye.py T ETH --replay 1440 --max-iter 4
  python crypto_trading_system_faye.py G ETH --replay 1440 --rank recent
  python crypto_trading_system_faye.py P BTC 6h
  python crypto_trading_system_faye.py --help

Env overrides:
  MODE_D_OUTER_WORKERS=8   (default 8; tune for hardware)
  FAYE_MODELS_DIR=...      (default 'models_fujiwara_30/' relative to script)
  FAYE_CONFIG_DIR=...      (default 'config_fujiwara_30/' relative to script)
...
Outputs (all in FAYE-isolated directories — does NOT touch Ed's production files):
  models_fujiwara_30/crypto_fujiwara_30_production.csv    (winner per (asset, horizon))
  models_fujiwara_30/eth_models_<H>h.pkl           (trained models, ed-pickle-compatible)
  models_fujiwara_30/mode_d_full_<asset>_<H>h_<ts>.csv (per-eval CSV w/ K=5 seed breakdown)
  config_fujiwara_30/regime_config_fujiwara_30.json       (per-asset: detector, bull/bear horizon+conf+shield)
...
Promotion to production (one-time, after FAYE validated; trader flat — Rule 19):
  ...
  Copy-Item "models_fujiwara_30\\crypto_fujiwara_30_production.csv" "models\\crypto_ed_production.csv" -Force
  Copy-Item "config_fujiwara_30\\regime_config_fujiwara_30.json"   "config\\regime_config_ed.json"     -Force
  Copy-Item "models_fujiwara_30\\eth_models_*.pkl"          "models\\" -Recurse -Force
  Copy-Item "models_fujiwara_30\\pysr_*.json"               "models\\" -Force   # lagged-discovered PySR
...
# creation + config_fujiwara_30 seeding + banners). All function/constant DEFINITIONS still
...
# would print [FAYE_*] banners during Mode V Step 2 refine
...
_FAYE_IS_MAIN_PROCESS = _mp_check_e.current_process().name == 'MainProcess'
...
from hardware_config import (
    ...
    PARALLEL_BACKTESTS, PARALLEL_LGBM_DEVICE, PYSR_PARALLEL_RUNS,
...
FAYE_MODELS_DIR = _resolve_dir('FUJI30_MODELS_DIR', 'models_fujiwara_30')
FAYE_CONFIG_DIR = _resolve_dir('FUJI30_CONFIG_DIR', 'config_fujiwara_30')
_FAYE_MODELS_RAW = os.environ.get('FUJI30_MODELS_DIR', 'models_fujiwara_30')
_FAYE_CONFIG_RAW = os.environ.get('FUJI30_CONFIG_DIR', 'config_fujiwara_30')
...
# run-as-main startup I/O below is skipped — no output-dir creation, no config_fujiwara_30
...
if not _FAYE_LIBRARY_MODE:
    ...
    # "Cannot save file into a non-existent directory: '...\\models_fujiwara_30'" because
    ...
    # FAYE 2026-05-30: seed FAYE_CONFIG_DIR/regime_config_fujiwara_30.json so Mode R/S
    ...
    _faye_seed_regime_cfg = os.path.join(FAYE_CONFIG_DIR, 'regime_config_fujiwara_30.json')
    _faye_noprod_regime_cfg = _faye_seed_regime_cfg.replace('.json', '_noprod.json')
    _faye_live_regime_cfg = os.path.join(FAYE_CONFIG_DIR, 'regime_config_fujiwara_30_template.json')  # WALL: never read production
...
RESUME_DIR = f'{FAYE_MODELS_DIR}/.resume_30m'
...
# ── Candle granularity (FUJIWARA_30 fork) ──────────────────────────────────────────
CANDLE_MINUTES = 30
CANDLE_TIMEFRAME = '30m'        # ccxt timeframe for download_binance
CANDLE_TAG = '30m'             # data-file tag
PERIODS_PER_HOUR = 60 // CANDLE_MINUTES   # 2
...
_ASSET_DEFS = [
    ('BTC',   'binance',  'BTC/USDT',  'btc_30m_data.csv',  '2024-01-01T00:00:00Z'),
    ('ETH',   'binance',  'ETH/USDT',  'eth_30m_data.csv',  '2024-01-01T00:00:00Z'),
    ('XRP',   'binance',  'XRP/USDT',  'xrp_30m_data.csv',  '2024-01-01T00:00:00Z'),
    ('SOL',   'binance',  'SOL/USDT',  'sol_30m_data.csv',  '2024-01-01T00:00:00Z'),
    ('LINK',  'binance',  'LINK/USDT', 'link_30m_data.csv', '2024-01-01T00:00:00Z'),
    ('BNB',   'binance',  'BNB/USDT',  'bnb_30m_data.csv',  '2024-01-01T00:00:00Z'),
...
# ============================================================
# Trimmed 5→2 on 2026-04-30 after cross-run analysis of 35 1440-window HRSTs:
# tsmom_672h and sma24>sma100 are the only detectors that consistently appear
# in TOP 15 (66% and 69% of runs respectively). The other 3 detectors are
# either single-run wonders (price>sma72, sma168>sma480) or weak (vol_calm,
# 37% presence and 4 wins by luck on biased 4mo runs). Restricting search to
# the 2 robust detectors reduces Mode S multiple-testing 3920→1568 combos
# (60% reduction) and gives much more reproducible HRST winners.
#
...
PRODUCTION_CSV = f'{FAYE_MODELS_DIR}/crypto_fujiwara_30_production.csv'
REGIME_CONFIG_PATH = f'{FAYE_CONFIG_DIR}/regime_config_fujiwara_30.json'
...
CHARTS_DIR = 'charts'
MODELS_DIR = FAYE_MODELS_DIR  # was 'models'; honors FAYE_MODELS_DIR env var
CONFIG_DIR = FAYE_CONFIG_DIR  # was 'config'; honors FAYE_CONFIG_DIR env var
...
def _compute_pysr_features(df, all_cols, asset_name, horizon, verbose=True):
    ...
    models_dir = FAYE_MODELS_DIR  # isolated: FAYE PySR lives in models_fujiwara_30/, not live models/
    ...
    for i, expr_info in enumerate(expressions):
        col_name = f'pysr_{i+1}_{CANDLE_MINUTES}'
...
def _pysr_run_worker(X, y, all_cols, feature_subset, seed,
    ...
    Closes OS-level fd 0 (stdin) BEFORE PySR/Julia loads. Julia opens
    stdin at the OS file-handle level, bypassing Python's sys.stdin —
    closing sys.stdin alone doesn't stop Julia's interactive prompt
    polling. fd 0 → /dev/null makes Julia's read() see EOF immediately.

    Returns (run_label, results_list_or_None, captured_stdout, error_str_or_None).
    """
    # OS-level stdin closure — must run before PySR import (which triggers
    # Julia init, which captures stdin at startup time).
    try:
    ...
    results = None
    try:
        import pysr_discover_features_fujiwara as _pysr_mod
        # progress=False + verbosity=0 disable PySR's interactive dashboard
        # AND its OS-level stdin poll (defense in depth alongside fd 0 close).
...
def _discover_features_parallel(asset, horizon, n_top=5, iterations=100,
                                populations=30, max_corr=0.7, n_runs=4,
                                load_data_fn=None, build_features_fn=None,
                                horizon_suffix='h', max_diag_hours=None):
    """Drop-in parallel replacement for pysr_discover_features.discover_features.
    Same signature, same outputs (results list + pysr_rows count). Differs
    only in that the N outer PySR runs (4-7 by default) execute in parallel
    across PYSR_PARALLEL_RUNS processes instead of sequentially.
    ...
    Falls back to sequential discover_features if PYSR_PARALLEL_RUNS <= 1
    or only 1 task is configured.
    """
    from concurrent.futures import ProcessPoolExecutor, as_completed
    import pysr_discover_features_fujiwara as pysr_mod
    ...
    # Build dynamic feature groups (mirrors pysr_discover_features.discover_features)
    ...
    if PYSR_PARALLEL_RUNS <= 1 or len(tasks) <= 1:
        print(f"  [pysr-parallel] only {len(tasks)} task(s) — falling back "
              f"to sequential pysr_mod.discover_features")
        return pysr_mod.discover_features(
...
def run_mode_p(assets_list, horizons):
    """Mode P: PySR symbolic regression feature discovery (parallel by default).

    Uses _discover_features_parallel which dispatches the 4-7 outer PySR runs
    across PYSR_PARALLEL_RUNS processes. Each PySR run stays deterministic
    (procs=0, parallelism='serial'). Speedup: ~ceil(n_runs / PYSR_PARALLEL_RUNS)
    compared to sequential — typically 2× on laptop (3 workers, 4 runs).
    """
    print("\n" + "=" * 60)
    print("  MODE P: PySR FEATURE DISCOVERY (PARALLEL)")
    print(f"  Workers: {PYSR_PARALLEL_RUNS} concurrent (per-machine config)")
    print("=" * 60)

    try:
        from pysr_discover_features_fujiwara import save_results
    except ImportError:
...
GRID_COMBOS = [
    ...
    # 'GB+LGBM' REMOVED 2026-06-09 pending validation (was added 2026-06-08). The real-engine
    # backtest (tools/bt_lgbm_tune_8h.py — ETH 8h, 2mo regime sim) had GB+LGBM LOSE to the live
    # RF+LGBM (+43.5% vs +51.2% @1440h), and the step=36 screen's LGBM-reg "win" reversed under
    # the live conf gate. Re-add only after it beats RF+LGBM in a proper step=1 / multi-window test.
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False, replay_hours=None):
    ...
    for asset_name in assets_list:
        ...
        # floor of 2 (lowered from 4 on 2026-05-02 — short --replay windows like
        # 504h gave n=302 → 4-trade floor killed all candidates for short horizons,
        # blocking Mode R with "Need at least 2 horizons with production models").
        # For n=1440: 4. For n=2880: 8 (unchanged from old behavior).
        # For n=4320: 12 (slightly stricter, appropriate for longer history).
        MIN_GRID_TRADES = max(2, n // 360)
        print(f"\n{'='*70}")
...
def run_mode_s_serial(assets_list, horizons, args=None):
    ...
    Flow:
        ...
        6. Write winning detector + horizons + confidences to regime_config_fujiwara_30.json
    """
    ...
    for asset in assets_list:
        ...
        n_det = len(ENABLED_DETECTORS)  # trimmed 5→2 on 2026-04-30 (see ENABLED_DETECTORS)
...
def run_mode_t_serial(assets_list, args=None):
    ...
    Saves winner to regime_config_fujiwara_30.json:
...
def _build_regime_indicators_and_detectors(asset):
    ...
    # named detector found in regime_config_fujiwara_30.json (back-compat for legacy
    # configs and quarterly rediscovery runs that override ENABLED_DETECTORS).
    detectors = {k: v for k, v in _all_detectors.items() if k in ENABLED_DETECTORS}
...
def _run_mode_r(assets, horizons, args):
    ...
    if len(signals_cache) == 0:
        print("\n  ERROR: No production models available for any horizon — cannot proceed")
        return
    if len(signals_cache) == 1:
        only_h = list(signals_cache.keys())[0]
        print(f"\n  WARNING: only 1 horizon ({only_h}h) has a production model. "
              f"Mode R will use {only_h}h for both bull AND bear (degenerate pair).")
        print(f"  Mode S joint sweep will be reduced to confidence search only.")
...
def _apply_mode_r_to_config(r_results):
    """Write Mode R's winning horizons to regime_config_fujiwara_30.json so Mode S picks them up."""
    if not r_results:
        return
    tcfg_path = REGIME_CONFIG_PATH
    try:
...
def _sweep_rally_cooldown(asset, signals, asset_cfg, replay_h, rank='recent',
    ...
    Writes winner to regime_config_fujiwara_30.json if write_config=True:
...
def main():
    ...
    # `--nopersist` → `--no-persist` (underscore-free alias)
    # Normalize sys.argv in-place BEFORE any flag parsing so all downstream
    # checks see the canonical form.
    _normalized = []
    for tok in sys.argv:
        if tok == '--nopersist':
            _normalized.append('--no-persist')
        elif tok.startswith('--') and tok[2:].isdigit():
            # --1440 → --replay 1440 (two tokens)
            _normalized.extend(['--replay', tok[2:]])
        else:
            _normalized.append(tok)
    ...
    # default = config_fujiwara_30/regime_config_fujiwara_30.json + FAYE_MODELS_DIR/crypto_fujiwara_30_production.csv)
    flag_chal_config = ''
    flag_chal_models = ''
    for i, a in enumerate(sys.argv[1:], 1):
    ...
    # Copies _noprod -> faye live (crypto_fujiwara_30_production.csv / regime_config_fujiwara_30.json)
    ...
    if '--promote' in sys.argv:
        print("\n  [FUJIWARA_30] --promote is DISABLED on this research fork (Chinese wall).")
        print("  This fork can NEVER write live production (models/ or config/).")
        print("  Results live in models_fujiwara_30/ + config_fujiwara_30/ only.")
        return
    ...
    for i, a in enumerate(sys.argv[1:], 1):
        if a == '--label-threshold' and i < len(sys.argv) - 1:
            try:
                ...
                PRODUCTION_CSV = os.path.join(FAYE_MODELS_DIR, f"crypto_faye_production_lt{pct:g}.csv")
                print()
                print("  " + "!" * 76)
...
Modes:
  P       PySR feature discovery (symbolic regression → models_fujiwara_30/pysr_*.json)
...
Options:
  ...
  --label-threshold X Research: retrain with label=1 iff future_return > X (e.g., 0.01 = 1%).
                      Default = 2×fee (0.0022). Output redirected to models/crypto_faye_production_lt<pct>.csv.
...
""")
    ...
    if run_all_metrics and mode == 'D':
        ...
        for metric in sorted(VALID_METRICS):
            ...
            tcfg_path = f'{CONFIG_DIR}/regime_config_fujiwara_30{metric_suffix}.json'
            csv_path = _get_models_csv_path()
            try:
                with open(tcfg_path) as f:
                    tcfg = json.load(f)
    ...
    # deployable winner in config_fujiwara_30/regime_config_fujiwara_30.json + models_fujiwara_30/.
    ...
    if '--no-persist' not in sys.argv and '--noprod' not in sys.argv and mode in _PROMOTE_MODES:
        try:
            ...
            _payload = {
                ...
                'assets': assets_list,
                'horizons': horizons,
                'config_src': os.path.join(FAYE_CONFIG_DIR, 'regime_config_fujiwara_30.json'),
                'models_csv_src': os.path.join(FAYE_MODELS_DIR, 'crypto_fujiwara_30_production.csv'),
...
def _backtest_one_config_worker(asset, horizon, label, cfg, replay_hours, lgbm_device):
    """Runs in a loky worker process. Each worker is fully independent:
      - Mutating ALL_MODELS in this process does NOT affect parent or
    ...
    for asset in assets_list:
        for horizon in horizons:
            ...
            if top3_for_refine:
                print(f"\n{'='*70}")
                print(f"  STEP 2: OPTUNA REFINE — top {len(top3_for_refine)} live performers")
                print(f"  {REFINE_TRIALS} trials per config | "
                      f"gamma ±{REFINE_GAMMA_RANGE} | "
                      f"features ±{REFINE_FEAT_RANGE} | "
                      f"window ±{REFINE_WINDOW_RANGE}h")
                print(f"{'='*70}")
...
def _finish_mode_v(all_results, production_models, assets_list, horizons, _replay):
    ...
    if production_models:
        ...
        # No standalone helper exists post-merge — inline the upsert.
        if True:
            ...
            try:
                ...
                _atomic_write_csv(df_prod, PRODUCTION_CSV, index=False)
                print(f"\n  Production CSV updated: {PRODUCTION_CSV}")
            except Exception as e:
...
# in parallel via loky, then monkey-patch generate_signals for the
# duration of the run_mode_s / run_mode_t call so the sequential
...
def _generate_signals_cached(prebuilt_cache):
    """Monkey-patch generate_signals to look up from prebuilt_cache
    on matching args, fall through to the real implementation on miss.
    Restores the original on exit.

    Uses globals() to mutate the module-level binding — bare assignment
    `generate_signals = patched` would be flagged by Python as a local
    variable for the whole function scope, causing UnboundLocalError on
    `real = generate_signals` above.
    """
    real = globals()['generate_signals']
    ...
    globals()['generate_signals'] = patched
    try:
        yield
    finally:
...
def run_mode_t(assets_list, args=None):
    ...
    # regime_config_fujiwara_30.json. Replicate that prediction here.
    try:
        with open(REGIME_CONFIG_PATH) as f:
...
# When None (default), the worker uses REFINE_TRIALS.
...
def _refine_one_config_worker(cfg_idx, top_entry_pickle, asset, horizon,
    ...
    n_trials_override: if not None, overrides REFINE_TRIALS for this
    ...
    #   models_fujiwara_30/refine_progress_cfg<idx>_pid<PID>.log
    ...
    try:
        ...
        if completed:
            ...
            print(f"    → Best: w={best.params['window']}h "
                  ...
                  f"f={best.params['n_features']} apf={best.value:.3f}")
            best_sel_idx = _feature_floor_indices(
...
def _refine_top_configs(asset, horizon, top3_for_refine, df_raw,
    ...
    if len(df_full_r) > MAX_DIAG_HOURS:
        df_full_r = df_full_r.tail(MAX_DIAG_HOURS).reset_index(drop=True)
    df_full_r, all_cols_r, dropped_r = _filter_sparse_features(
...
# models_fujiwara_30/). MACHINE-LOCAL: the lock lives in this machine's temp dir, NOT the
...
def run_mode_c(assets_list, args=None):
    ...
    inc_cfg, inc_csv = f'{FAYE_CONFIG_DIR}/regime_config_fujiwara_30.json', f'{FAYE_MODELS_DIR}/crypto_fujiwara_30_production.csv'  # WALL: fork incumbent, not production
    chal_cfg = getattr(args, 'chal_config', '') or f'{FAYE_CONFIG_DIR}/regime_config_fujiwara_30.json'
    chal_csv = getattr(args, 'chal_models', '') or f'{FAYE_MODELS_DIR}/crypto_fujiwara_30_production.csv'
    ...
    if '--refine-trials' in sys.argv:
        ...
        try:
            ...
            print(f"  --refine-trials override: {_REFINE_TRIALS_OVERRIDE} "
                  f"(default REFINE_TRIALS={REFINE_TRIALS})")
