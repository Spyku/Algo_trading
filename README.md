...
| Date | Milestone |
|---|---|
| **2026-06-25** | **Telegram `/setup` toggle now persists (was a no-op until /cfg_save).** The asset enabled/tp/maker/conf/max toggles edited only the in-memory `_setup_state` snapshot and saved to `regime_config_ed.json` *solely* on /cfg_save — so a toggle "didn't stick," and each handler sent a confirmation + a menu = 2 popups. Added `_setup_commit()` ([crypto_revolut_ed_v2.py:3917](crypto_revolut_ed_v2.py#L3917)) → saves to disk + syncs live `trading_cfg` after every mutation (6 sites); dropped the redundant confirmation so each action sends one menu. Smoke `tools/smoke_test_setup_toggle.py` (11/11). Loads on next trader restart. |
| **2026-06-29** | **ROOT CAUSE: backtest ≠ live = different TRAINING-WINDOW edge (not execution/data). 75% → 99% agreement.** Chased the 4h/4h backtest(+3.85%)-vs-live(−3.78%) gap to ground: ruled out execution (fills clean ~1.7pp), timing (60s lag sweep ~0pp), data revision (inference+training features **0.000000** diff), device/nondeterminism (engine refits live's frozen training matrix at **97.5%**). The actual cause: the backtest (`generate_signals`) and the live trader (`generate_live_signal`) are **two code paths that build different training windows** — backtest `[i-window, i-embargo]` (last row i-5) vs live's last `window` labelled rows ending at i-3. **Same data, ~1 different highest-gamma-weight row → 25% different signals.** Fix: additive **`FAYE_FAITHFUL_WINDOW`** flag ([faye.py:3106](crypto_trading_system_faye.py#L3106)) replicates live's edge → backtest↔live **75% → 99%** (default-off, backtest-only, does not touch live). Use it for any live-prediction backtest; the old head-to-head verdicts ran on the non-faithful path. **LEAK FIXED same day (F2):** live + shadow now drop the forming-bar-labelled row from `df_train` (the closed-bar stepback left the last training row's label peeking at the forming bar, 162/162 hours) → last training row = `i-horizon`, leakage-free; `FAYE_FAITHFUL_WINDOW` moved to edge-4 to match. NOT an embargo (the dropped row's label came from the incomplete forming bar). Takes effect on trader restart; shadow validates ≥99% post-restart. Re-run selection leakage-free: `python crypto_trading_system_faye.py HRST ETH 4,5,6,7,8h --replay 1440` (leakage-free DEFAULT now). |
| **2026-06-29** | **Incremental ETH derivatives download — cut the live order's blocking pre-compute (P1) from ~33s to ~4s.** Root cause of the 4h/4h backtest-vs-live gap: the order was placed ~36s after the bar close, ~33s of it spent **re-paginating the full 2022→now derivatives history every 2h** (perp klines = 40 pages) on the critical path — so in fast moves (06-26 crash) the price had already moved ~2.4% before the order existed. Fix: `download_derivatives_data` now fetches only from `(last CSV row − 3d)` and merges onto preserved history via `_merge_preserve_history` (new `_deriv_incremental_start_ms`, [download_macro_data.py](download_macro_data.py)). **Validated byte-identical to a full pull** (39,359 rows, 0 cells differ except the forming candle) at **4.3s vs 32s (7.4×)**. `full=True` keeps the legacy backfill; missing/empty CSV → safe full fallback. NOT execution slippage (maker fills are clean) and NOT a data bug — a slow download on the critical path. Takes effect on trader restart. |
| **2026-06-28** | **cross_asset.csv silent history-clobber found, restored, and guarded (+ new daily DATA-INTEGRITY check).** A concurrent-write race (the same one the atomic-write fix targets) caught `cross_asset.csv` mid-write during a FAYE HRST run on 09:24; `_yf_merge_with_existing` read it as empty and **silently returned the 9-row incremental slice over 1637 rows of history** ([download_macro_data.py:524](download_macro_data.py#L524)) — no alert. That NaN'd every `xa_*_corr*` feature on recent rows, which the **8h PySR formula divides by**, so `pysr_1`→NaN → the row-skip at [faye.py:3120](crypto_trading_system_faye.py#L3120) zeroed all 8h signals (surfaced while backtesting an 8h config). **Fixes:** (1) restored full history via `download_cross_asset(full=True)` → xa-corr NaN 91/200→**0/200**; (2) the empty-existing merge branch now **retries-to-recover then ALERTs critical** instead of silently writing the slice; (3) new **[4] DATA INTEGRITY** check in `tools/sanity_check.py` (`data_integrity_check`) floors row-count + date-span for 6 critical CSVs and **drives the daily verdict** — a collapse now FAILs the daily sanity + Telegrams (filter token added in `crypto_revolut_ed_v2.py`). Live ETH 4h was unaffected (doesn't use xa-corr). |
| **2026-06-28** | **Atomic-write race fix completed — the 7 merge/download writes the 06-24 fix missed.** The 2026-06-24 fix made only the **3 forward-only snapshot saves** atomic; the **7 merge/download `to_csv` writes** in `download_macro_data.py` stayed bare (`macro :656`, `fear_greed :716`, `cross_asset :816`, `onchain :975`, `derivatives :1290`, `hourly :1426`, `stablecoin :1481`), so those files kept hitting the 0-byte/partial-write race. It resurfaced as recurring **"No columns to parse from file"** (empty read) + **"time data does not match format"** (partial read) at hourly cycle boundaries (2026-06-28 02:01/07:01/10:01). Converted all 7 to `_atomic_to_csv` → **10/10 writes atomic**. The merge reads were already `try/except`-guarded (self-heal), so the residual gap was purely write-side. |
| **2026-06-24** | **Snapshot-write race fixed ("No columns to parse from file").** Three snapshot saves in `download_macro_data.py` (IV `:1521`, orderbook `:1587`, whale `:1660`) did a bare `to_csv` (truncate→write) + an **unguarded** `read_csv` of the existing file — so a concurrent reader / Drive-sync catching the file mid-write at 0 bytes raised `EmptyDataError`, breaking a live cycle (2026-06-24 18:01). Fix **A** (read-guard): `getsize>0` + `try/except` → an empty/corrupt file self-heals to a fresh write instead of crashing. Fix **B** (atomic write): new `_atomic_to_csv` (temp file + `os.replace`) so the target is never seen empty/half-written by any reader. Smoke test `tools/smoke_test_snapshot_atomic.py` (9/9). |
| **2026-06-23** | **Rally-cooldown no-winner now DISABLES the gate** (`crypto_trading_system_faye.py` `_sweep_rally_cooldown`, commit `30817e9`) instead of leaving the prior config — a stale `enabled:true` double could otherwise survive every run and ride a verbatim promote to live (the 2026-06-22 4h/4h double-gate incident). Writes explicit `enabled:false` single-form on no STRICT winner. Regression `tools/test_gate_disable_on_nowinner.py`. |
