"""
Crypto Hourly Trading System -- V5.2 (EXPERIMENTAL)
============================================================
V5.2 adds full 1-8h horizon support and Mode G: pair comparison.
Tests all C(8,2)=28 horizon pairs to find the best 2-horizon
strategy combination over the last week.

NOT for production. Run Mode G results, then promote winning pair
to production V5 via Mode D + F.

Modes:
  B. Quick run (saved models)    D. Full pipeline (1-8h horizons)
  E. Iterative refinement        F. Strategy comparison (400h)
  G. Horizon pair test (all 28 pairs x 2 strategies over 168h)
  5/6/7. Quick BTC/ETH/XRP

CLI Usage:
  python crypto_trading_system_v5.2.py D BTC 1,2,3,4,5,6,7,8h 2y  # all 8 horizons
  python crypto_trading_system_v5.2.py D BTC 4,8h 2y               # standard
  python crypto_trading_system_v5.2.py G BTC                        # pair test (168h)
  python crypto_trading_system_v5.2.py F BTC 4,8h                   # strategy comparison
...
AVAILABLE_HORIZONS = [1, 2, 3, 4, 5, 6, 7, 8]  # V5.2: all horizons
...
# ============================================================
# MODE G: HORIZON PAIR COMPARISON
# Tests all C(n,2) pairs of available horizons over last 168h.
# Shows which 2-horizon combination maximises net return.
# Read-only: does NOT write to trading_config.json.
# ============================================================
def run_horizon_pair_comparison(assets_list):
    ...
    for asset in assets_list:
        ...
        if len(available_horizons) < 2:
            ...
            print(f"  Run: python crypto_trading_system_v5.2.py D {asset} 1,2,3,4,5,6,7,8h 2y")
            continue
...
def main():
    ...
    if cli_args and cli_args[0].upper() in ('B', 'D', 'E', 'F', 'G', '5', '6', '7'):
        ...
        print("=" * 60)
        print("  CRYPTO HOURLY ML TRADING SYSTEM -- V5.2 (EXPERIMENTAL)")
        ...
        print("  G. HORIZON PAIR TEST (all 28 pairs x 2 strategies over 168h)")
        print("  ---")
        ...
        mode_labels = {'B': 'Quick Run',
                       ...
                       'F': 'Strategy Comparison', 'G': 'Horizon Pair Test'}
    ...
    if mode == 'B':
        ...
        # After Mode D with 2+ horizons: offer pair comparison
