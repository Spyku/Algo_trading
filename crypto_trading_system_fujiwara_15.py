"""
FUJIWARA_15 — 15-MINUTE-candle ML Trading System (research fork of FAYE, 2026-06-19)
====================================================================================
Self-contained 15-minute-candle fork of crypto_trading_system_faye.py. Built to
...
CHINESE WALL — NO INFECTION OF PRODUCTION. Research-only, physically walled:
  * Output -> models_fujiwara_15/ + config_fujiwara_15/ (never models/ or config/).
  * Price data -> data/<asset>_15m_data.csv (never *_hourly_data.csv).
...
HORIZONS ARE IN CANDLE (PERIOD) UNITS: 5,6,7,8 periods = 75, 90, 105, 120 minutes.
...
What was inlined and from where:
  ...
  P1  Identity rebrand                  (was Ed module renames)
       FAYE_MODELS_DIR / FAYE_CONFIG_DIR / crypto_fujiwara_15_production.csv /
       regime_config_fujiwara_15.json — isolated paths, no Ed file collisions.
  ...
  P5b Mode D 8-worker grid dispatcher   (was v3 inspect-frame state capture)
       ...
       seed-by-seed breakdown at models_fujiwara_15/mode_d_full_*.csv.
...
Env overrides:
  ...
  FAYE_MODELS_DIR=...      (default 'models_fujiwara_15/' relative to script)
  FAYE_CONFIG_DIR=...      (default 'config_fujiwara_15/' relative to script)
...
Outputs (all in FAYE-isolated directories — does NOT touch Ed's production files):
  models_fujiwara_15/crypto_fujiwara_15_production.csv    (winner per (asset, horizon))
  models_fujiwara_15/eth_models_<H>h.pkl           (trained models, ed-pickle-compatible)
  models_fujiwara_15/mode_d_full_<asset>_<H>h_<ts>.csv (per-eval CSV w/ K=5 seed breakdown)
  config_fujiwara_15/regime_config_fujiwara_15.json       (per-asset: detector, bull/bear horizon+conf+shield)
...
Promotion to production (one-time, after FAYE validated; trader flat — Rule 19):
  ...
  Copy-Item "models_fujiwara_15\\crypto_fujiwara_15_production.csv" "models\\crypto_ed_production.csv" -Force
  Copy-Item "config_fujiwara_15\\regime_config_fujiwara_15.json"   "config\\regime_config_ed.json"     -Force
  Copy-Item "models_fujiwara_15\\eth_models_*.pkl"          "models\\" -Recurse -Force
  Copy-Item "models_fujiwara_15\\pysr_*.json"               "models\\" -Force   # lagged-discovered PySR
...
# creation + config_fujiwara_15 seeding + banners). All function/constant DEFINITIONS still
...
FAYE_MODELS_DIR = _resolve_dir('FUJI15_MODELS_DIR', 'models_fujiwara_15')
FAYE_CONFIG_DIR = _resolve_dir('FUJI15_CONFIG_DIR', 'config_fujiwara_15')
_FAYE_MODELS_RAW = os.environ.get('FUJI15_MODELS_DIR', 'models_fujiwara_15')
_FAYE_CONFIG_RAW = os.environ.get('FUJI15_CONFIG_DIR', 'config_fujiwara_15')
...
# run-as-main startup I/O below is skipped — no output-dir creation, no config_fujiwara_15
...
if not _FAYE_LIBRARY_MODE:
    ...
    # "Cannot save file into a non-existent directory: '...\\models_fujiwara_15'" because
    ...
    # FAYE 2026-05-30: seed FAYE_CONFIG_DIR/regime_config_fujiwara_15.json so Mode R/S
    ...
    _faye_seed_regime_cfg = os.path.join(FAYE_CONFIG_DIR, 'regime_config_fujiwara_15.json')
    _faye_noprod_regime_cfg = _faye_seed_regime_cfg.replace('.json', '_noprod.json')
    _faye_live_regime_cfg = os.path.join(FAYE_CONFIG_DIR, 'regime_config_fujiwara_15_template.json')  # WALL: never read production
...
RESUME_DIR = f'{FAYE_MODELS_DIR}/.resume_15m'
...
# ── Candle granularity (FUJIWARA_15 fork) ──────────────────────────────────────────
CANDLE_MINUTES = 15
CANDLE_TIMEFRAME = '15m'        # ccxt timeframe for download_binance
CANDLE_TAG = '15m'             # data-file tag
PERIODS_PER_HOUR = 60 // CANDLE_MINUTES   # 4
...
_ASSET_DEFS = [
    ('BTC',   'binance',  'BTC/USDT',  'btc_15m_data.csv',  '2024-01-01T00:00:00Z'),
    ('ETH',   'binance',  'ETH/USDT',  'eth_15m_data.csv',  '2024-01-01T00:00:00Z'),
    ('XRP',   'binance',  'XRP/USDT',  'xrp_15m_data.csv',  '2024-01-01T00:00:00Z'),
    ('SOL',   'binance',  'SOL/USDT',  'sol_15m_data.csv',  '2024-01-01T00:00:00Z'),
    ('LINK',  'binance',  'LINK/USDT', 'link_15m_data.csv', '2024-01-01T00:00:00Z'),
    ('BNB',   'binance',  'BNB/USDT',  'bnb_15m_data.csv',  '2024-01-01T00:00:00Z'),
...
REGIME_CONFIG_PATH = f'{FAYE_CONFIG_DIR}/regime_config_fujiwara_15.json'
...
def _compute_pysr_features(df, all_cols, asset_name, horizon, verbose=True):
    ...
    models_dir = FAYE_MODELS_DIR  # isolated: FAYE PySR lives in models_fujiwara_15/, not live models/
...
def run_mode_s_serial(assets_list, horizons, args=None):
    ...
    Flow:
        ...
        6. Write winning detector + horizons + confidences to regime_config_fujiwara_15.json
    """
...
def run_mode_t_serial(assets_list, args=None):
    ...
    Saves winner to regime_config_fujiwara_15.json:
...
def _build_regime_indicators_and_detectors(asset):
    ...
    # named detector found in regime_config_fujiwara_15.json (back-compat for legacy
...
def _apply_mode_r_to_config(r_results):
    """Write Mode R's winning horizons to regime_config_fujiwara_15.json so Mode S picks them up."""
    if not r_results:
        return
    tcfg_path = REGIME_CONFIG_PATH
    try:
...
def _sweep_rally_cooldown(asset, signals, asset_cfg, replay_h, rank='recent',
    ...
    Writes winner to regime_config_fujiwara_15.json if write_config=True:
...
def main():
    ...
    # default = config_fujiwara_15/regime_config_fujiwara_15.json + FAYE_MODELS_DIR/crypto_fujiwara_15_production.csv)
    flag_chal_config = ''
    flag_chal_models = ''
    for i, a in enumerate(sys.argv[1:], 1):
    ...
    # Copies _noprod -> faye live (crypto_fujiwara_15_production.csv / regime_config_fujiwara_15.json)
    ...
    if '--promote' in sys.argv:
        print("\n  [FUJIWARA_15] --promote is DISABLED on this research fork (Chinese wall).")
        print("  This fork can NEVER write live production (models/ or config/).")
        print("  Results live in models_fujiwara_15/ + config_fujiwara_15/ only.")
        return
...
Modes:
  P       PySR feature discovery (symbolic regression → models_fujiwara_15/pysr_*.json)
...
""")
    ...
    if run_all_metrics and mode == 'D':
        ...
        for metric in sorted(VALID_METRICS):
            ...
            tcfg_path = f'{CONFIG_DIR}/regime_config_fujiwara_15{metric_suffix}.json'
            csv_path = _get_models_csv_path()
            try:
                with open(tcfg_path) as f:
                    tcfg = json.load(f)
    ...
    # deployable winner in config_fujiwara_15/regime_config_fujiwara_15.json + models_fujiwara_15/.
    ...
    if '--no-persist' not in sys.argv and '--noprod' not in sys.argv and mode in _PROMOTE_MODES:
        try:
            ...
            _payload = {
                ...
                'assets': assets_list,
                'horizons': horizons,
                'config_src': os.path.join(FAYE_CONFIG_DIR, 'regime_config_fujiwara_15.json'),
                'models_csv_src': os.path.join(FAYE_MODELS_DIR, 'crypto_fujiwara_15_production.csv'),
...
def run_mode_t(assets_list, args=None):
    ...
    # regime_config_fujiwara_15.json. Replicate that prediction here.
    try:
        with open(REGIME_CONFIG_PATH) as f:
...
def _refine_one_config_worker(cfg_idx, top_entry_pickle, asset, horizon,
    ...
    #   models_fujiwara_15/refine_progress_cfg<idx>_pid<PID>.log
...
# models_fujiwara_15/). MACHINE-LOCAL: the lock lives in this machine's temp dir, NOT the
...
def run_mode_c(assets_list, args=None):
    ...
    inc_cfg, inc_csv = f'{FAYE_CONFIG_DIR}/regime_config_fujiwara_15.json', f'{FAYE_MODELS_DIR}/crypto_fujiwara_15_production.csv'  # WALL: fork incumbent, not production
    chal_cfg = getattr(args, 'chal_config', '') or f'{FAYE_CONFIG_DIR}/regime_config_fujiwara_15.json'
    chal_csv = getattr(args, 'chal_models', '') or f'{FAYE_MODELS_DIR}/crypto_fujiwara_15_production.csv'
