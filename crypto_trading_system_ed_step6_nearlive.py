...
# Step 6 refactor: shared signal-generation core used by both live trader and
# the engine's backtest path. Single source of truth for train + standardize
# + fit + predict + aggregate. Defaults in 6a preserve current behavior.
# NEAR-LIVE TEST FORK: imports the _nearlive variant of the core which adds
# the `mean_last_10` NaN-fill policy. Production callers still use the
# regular `crypto_signal_core` — this fork is sandbox-only.
import crypto_signal_core_nearlive as _csc
...
def _deku_eval_with_pruning(features_np, labels_np, closes_np, combo, window, n,
                             ...
                             # === Step 6b NEW PARAMETERS (defaults preserve 6a regression) ===
                             embargo=None,         # None -> use `horizon` (current behavior)
                             na_policy='skip',     # 'skip' (current) | 'ffill' (live-equivalent)
                             signal_mode='binary', # 'binary' (current) | 'ternary' (live)
                             return_probas=False,  # False (current) | True (live)
                             eval_step=None):      # None -> use `step` (DIAG_STEP=36); 1 = every hour
    """Walk-forward evaluation with optional Optuna pruning.

    Step 6 refactor (2026-05-27):
      6a — inline train+standardize+predict+aggregate replaced by a call to
           crypto_signal_core.compute_signal_core(). With defaults, output is
           bit-identical to the pre-refactor engine (regression-safe).
      6b — new parameters (embargo, na_policy, signal_mode, return_probas,
           eval_step) let callers opt into live-trader semantics.
      6c — env var LIVE_EQUIVALENT_MODE=1 overrides all four to live conventions.
           Use this for diagnostic HRST that should predict live performance.

    Original docstring: Walk-forward evaluation with optional Optuna pruning.
    ...
    """
    # Phase 6b: resolve parameter defaults
    if embargo is None:
        embargo = horizon
    step_to_use = eval_step if eval_step is not None else step

    # Phase 6c: LIVE_EQUIVALENT_MODE env-var override
    if os.environ.get('LIVE_EQUIVALENT_MODE') == '1':
        embargo = 1
        na_policy = 'ffill'
        signal_mode = 'ternary'
        return_probas = True
    ...
    # NEAR-LIVE TEST FORK: NEAR_LIVE_MODE override (2026-05-27).
    # Matches live conventions EXCEPT keeps embargo at `horizon` for label-leak
    # safety (user's explicit policy choice). Uses the new `mean_last_10` NaN
    # policy that fills NaN with the mean of the 10 most-recent non-NaN values
    # preceding each cell (column-wise) — cleaner than ffill (which can carry
    # stale values forward) and more honest than zero-fill.
    if os.environ.get('NEAR_LIVE_MODE') == '1':
        # embargo intentionally NOT overridden — keeps `horizon` gap
        na_policy = 'mean_last_10'
        signal_mode = 'ternary'
        return_probas = True
        step_to_use = 1
    ...
    # Pre-compute the factories subset once (combo doesn't change across iterations)
    factories_subset = {name: model_factories[name] for name in combo
                        if name in model_factories}

    for i in range(min_start, n, step_to_use):
        train_start = max(0, i - window)
        train_end = max(train_start, i - embargo)  # Step 6b: was `i - horizon`
        X_train = features_np[train_start:train_end]
        y_train = labels_np[train_start:train_end]
        X_test = features_np[i:i+1]
        y_true = labels_np[i]
        ...
        # === Step 6a refactor: delegate to shared signal-generation core ===
        # Replaces ~30 lines of inline train+standardize+fit+predict+aggregate.
        # With defaults na_policy='skip', return_probas=False, binary_signal=True,
        # this is byte-identical to the original engine path.
        _core_result = _csc.compute_signal_core(
            X_train=X_train, y_train=y_train, X_test=X_test,
            model_factories=factories_subset, gamma=gamma,
            na_policy=na_policy, return_probas=return_probas,
            binary_signal=(signal_mode == 'binary'),
        )
        if _core_result is None or _core_result.get('signal') is None:
            # core returned skipped_reason in (na_skip, single_class, all_models_failed)
            step_idx += 1
            continue

        # Phase 6b: in ternary mode, HOLD means model is uncertain. Skip the
        # accuracy + portfolio update (maintain current position) but still
        # tick drawdown. In binary mode this branch never fires.
        if signal_mode == 'ternary' and _core_result['signal'] == 'HOLD':
            current_val = portfolio * (price / entry_price) if in_position else portfolio
            if current_val > peak:
                peak = current_val
            dd = (peak - current_val) / peak
        ...
        ensemble_pred = _core_result['ensemble_pred']
        if ensemble_pred == y_true:
            correct += 1
        total += 1

        # Portfolio logic (UNCHANGED from original)
        if ensemble_pred == 1 and not in_position:
            in_position = True
        ...
        # Max drawdown tracking (UNCHANGED)
        current_val = portfolio * (price / entry_price) if in_position else portfolio
        if current_val > peak:
            peak = current_val
        dd = (peak - current_val) / peak
        ...
        # Hyperband pruning: report intermediate APF after warmup (UNCHANGED)
...
def _h_deku_eval_median_k(features_np, labels_np, closes_np, combo, window, n,
                          step, model_factories, gamma=1.0, trial=None,
                          horizon=None,
                          # === Step 6b: pass-through to inner function ===
                          embargo=None,
                          na_policy='skip',
                          signal_mode='binary',
                          return_probas=False,
    ...
    Step 6b update: pass-through for embargo / na_policy / signal_mode /
    return_probas / eval_step so live-equivalent mode also propagates through
    the K=5 averaging layer.
    """
    if horizon is None:
        horizon = PREDICTION_HORIZON
    ...
    for seed in _H_SEEDS:
        ...
        r = _H_ORIG_DEKU_EVAL(
            ...
            embargo=embargo, na_policy=na_policy, signal_mode=signal_mode,
            return_probas=return_probas, eval_step=eval_step,
        )
        if r is not None:
            results.append(r)
