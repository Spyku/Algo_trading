...
**Production:**
- **Ed V2** — Regime-switching trading (`crypto_trading_system_faye.py` engine + `crypto_revolut_ed_v2.py` trader; ed.py retired 2026-07-01). Dynamic bull/bear horizon selection via external config (`config/regime_config_ed.json`). Maker-order pricing at `bid+0.01` with `post_only` for 0% fees. Mode R regime backtest. Currently ETH-only.
...
- **Volatility-scaled horizons (C01).** Originally promising on 2-month replay 2026-04-19: `vol_2band low→8h high→6h @90%` = +33.82% / 46 trades / 65% WR, +5.02pp over `tsmom_672h` baseline. **4-month confirmation FAILED to replicate.** Best 4mo variant `vol_2band low→8h high→6h @90%` = +30.98% vs current baseline `tsmom_672h bull=6h bear=8h @90% +38.49%` = **−7.51pp**. A different best variant on 4mo (`vol_median below→8h above→6h @90%` +41.48%) only beats baseline by +2.99pp — below the +5pp promotion threshold. Verdict: 2mo win was window-shopping, doesn't generalize. **Vol-scaled horizons family closed.** Additional caveat: the 2mo +5.02pp gain was measured against the now-defunct `tsmom_672h` baseline used pre-H75-promote; with live now on `sma24>sma100` (H75) the baseline has shifted further. SHELVED. **RECONFIRMED leakage-free 2026-07-02** (post-leak-fix audit): re-ran `tools/test_vol_scaled_horizon.py` (rewritten faye/leakage-free, re-baselined to the CURRENT live `sma168>sma480` 6h@80/4h@65) — **LIVE +62.88% vs best vol-scaled +43.88% = −18.99pp**, 0 of 21 variants clear +5pp-over-live with sub-period consistency. The leak fix did NOT revive it; the current HRST-optimized baseline *widens* the gap (−19pp vs the old −2.99pp-below-bar). Long horizons (7h −1.3%, 8h −5.5%) lose in bear windows, so the low-vol→8h assumption backfires. (Bear-window caveat, but a bull window is very unlikely to flip a −19pp deficit.)
...
python crypto_trading_system_faye.py P BTC 6h                    # Mode P — PySR feature discovery (single horizon ~40 min sequential, ~15 min parallel-P; see Runtime Reference)
python crypto_trading_system_faye.py H BTC 5,6,7,8h              # Mode H — horizon sweep
python crypto_trading_system_faye.py DV BTC 6h                   # Mode DV — grid + validate
python crypto_trading_system_faye.py D BTC 6h                    # Mode D — grid optimization only
python crypto_trading_system_faye.py V BTC 6h                    # Mode V — re-validate existing D results
python crypto_trading_system_faye.py R BTC 5,6,7,8h              # Mode R — regime backtest (all detectors)
python crypto_trading_system_faye.py R BTC --replay 2880         # Mode R — 4-month replay
python crypto_trading_system_faye.py HRS ETH 5,6,7,8h --replay 1440  # Full pipeline (H→R→S), writes regime config
python crypto_trading_system_faye.py --help                       # Show all modes, options, examples
...
```
crypto_trading_system_faye.py  (THE engine — Modes P/D/V/H/S/R/HRS/HRST/T/G/C; ed.py retired 2026-07-01)
  └── hardware_config.py
crypto_revolut_ed_v2.py  (Ed V2 auto-trader — reads regime_config_ed.json)
  └── crypto_live_trader_ed.py  (regime-aware signal generation)
        └── crypto_trading_system_faye.py  (compute_signal_core — serving since 2026-06-05)

# Optimizer
crypto_optimizer_bot.py  (Telegram bot for remote optimization)
  └── crypto_trading_system_faye.py  (spawned as subprocess — repointed 2026-07-01)
```
...
- **Production selection = best of {D-grid, refined} by `return×WR`** (NOT refined-only — corrected 2026-07-01 via HRST-log audit). Mode V backtests D candidates, refines the top 3 (75 trials each since H75, was 50 before 2026-05-18), re-backtests, then picks the highest `return×WR` across BOTH pools. Refined usually wins (D candidates typically underperform), **but a Grid candidate can legitimately win**: in the 2026-06-29 leakage-free ETH HRST, 4h/5h → Refined while **6h/7h/8h → Grid** (the live bull 6h = Grid w150). ⚠️ **Metric divergence on long horizons:** those Grid winners had *lower refine-APF* than the refined config they beat (6h: Grid D#2 beat Refined APF=2.969), because the **refine optimizes APF while final selection uses `return×WR`** — they disagree on 6/7/8h. So the recurring "w150 Grid, never Refined" pattern (flagged 2026-06-13) is a legit selection artifact, NOT a refine failure: the refine ran 3/3 configs (120-205 min) on all 5 horizons (HRST-log verified 2026-07-01). Open design Q: should the refine objective match the selection metric?
...
| `crypto_live_shadow.py` | **Live (monitor)** | Real-time shadow check — runs `compute_signal_core()` in parallel with `generate_live_signal()` each cycle, logs both to `config/shadow_signal_diff.csv`. Primary live-correctness monitor (tripwire <99%). **Must mirror the live inference path — see Critical Rule 23.** Closed-bar step-back added 2026-06-05 (mirror of fix #2; line 220) after it silently fell out of sync Jun 3-4 (100%→48%). Shadow errors are swallowed — never affects trading. **2026-07-01: feature-building migrated ed → faye** — `build_all_features` + `_compute_pysr_features` now route through `_live_trader` (= faye's, with `FAYE_MODELS_DIR=models`), matching the live path exactly; closes a Rule-23 mirror gap (the shadow built features with `crypto_trading_system_ed.py` while live uses faye). Smoke-verified **signal-neutral** (`tools`/scratch `smoke_ed_migration.py`: 196 cols identical, live 4h optimal_features bit-identical) — removes the **last live dependency** on ed.py. |
| `crypto_trading_system_meta.py` | Research | Standalone meta-labeling research tool. Imports pipeline bits from `crypto_trading_system_faye.py` (repointed ed → faye 2026-07-01); invoked via faye's `--meta-filter P` flag. R3 RESOLVED: AB matrix variants D+E showed meta hurts on strong baselines (E vs A −2.12pp). Kept available for SOL/BTC/XRP retests. |
| `start_ed_v2.bat` | **Live** | Launches Ed V2 trader with auto-restart + log tee. Auto-detects Desktop/Laptop venv. |
| `crypto_optimizer_bot.py` | **Live** | Telegram bot for remote optimization. Inline keyboard menus. Sequential job queue, subprocess execution. Separate bot token (`config/telegram_optimizer_config.json`). Below-normal priority. Spawns `crypto_trading_system_faye.py` (repointed ed → faye 2026-07-01; sets `_FAYE_WARNINGS_BAKED=1` so faye's stdout stays piped; jobs write to `models_faye/` staging → manual promote). |
...
The current live config lives in [TODO.md](TODO.md) `📌 LIVE STATE` section. This block intentionally does NOT duplicate it because live config changes more often than this stable-reference file — keeping the source of truth in one place (TODO.md) prevents drift. **Always trust TODO.md over the quick summary below if they disagree.** **⚡ 2026-06-30 00:32: live promoted to `sma168>sma480` · bull 6h@80% · bear 4h@65% · rally-cooldown gates ON · shields OFF (leakage-free 2026-06-29 HRST winner, +65.14%/86%WR/94tr 60d). The `price>sma72` 4h/4h quick-summary below is SUPERSEDED — see [TODO.md](TODO.md) `📌 LIVE STATE` for the current block + rollback.**
