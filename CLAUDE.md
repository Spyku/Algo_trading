...
- **5h, 6h, 7h, 8h** — core band, default sweep in Ed ([line 5428](crypto_trading_system_ed.py#L5428)). **Current live (G_narrow models on H75 engine, promoted 2026-05-21 21:56 CEST): ETH detector=`sma24>sma100`, bull=5h@65% RF+LGBM w=281 γ=0.9981, bear=8h@65% RF+LGBM w=293 γ=0.9990**, shields OFF (both regimes), rally cooldown OFF (both regimes — manually toggled 2026-05-23 22:21; threshold values preserved in JSON in case re-enabled: bull rr8h≥2.0 OR rr14h≥6.0 cd=6h; bear rr10h≥5.5 OR rr12h≥2.0 cd=8h), min_sell_pnl=0%, max_hold=10h. **Macro cache mtime fix patched 2026-05-27 11:22** (TODO 0527 — root cause of live-vs-backtest gap). Prior live (2026-05-18 → 2026-05-21): H75-fresh, bull=5h@75% XGB+LGBM w=100 γ=0.9993 / bear=8h@65% RF+LGBM w=162 γ=0.9954 on same detector, gates ENABLED. Earlier (2026-04-30 → 2026-05-18): bull=6h@85% / bear=5h@65% on `named:price>sma72`.
...
| `crypto_trading_system_ed_g_narrow_d_parallel.py` | **Research fork** (added 2026-05-22; 4 fix categories landed 2026-05-24 before TODO 0524 launch) | Parallelization benchmark over `crypto_trading_system_ed_g_narrow_d.py`. Imports g_narrow_d, applies identity-preserving monkey-patches on top of its K=5 wrap. **Original 3 parallel patches (May 22)**: (1) `_g_factories_seeded` reads `G_PARALLEL_LGBM_DEVICE` env var instead of hardcoding `device='gpu'`; (2) `_g_deku_eval_median_k` replaced with `ThreadPoolExecutor(5)` running K=5 seeds concurrently — same seeds, same median selection; (3) `_refine_top_configs_hybrid` replaced with 3-worker version (`max_workers=3`, CPU-only when K>1 due to GPU multi-context contention) — all 3 D-top configs refine in parallel. **May 24 fix batch (4 categories — applied after the May 22 Stage 2 9-12h run was invalidated by the first bug)**: **FIX #0 grid+output propagation**: fork calls `ENGINE.main()` which reads constants from ENGINE's namespace, not G's. Patched `ENGINE.GRID_COMBOS/WINDOWS/FEATURES/GAMMAS ← G.*` (was running engine's wider `[10,13,17,25]/[0.999,0.997,0.995]` instead of G's narrow `[10,15,20]/[0.999,0.996]`), `ENGINE.N_FEATURES_RANGE` 40/80 caps → 100 per G's "drop hard cap" design, and `ENGINE.MODELS_DIR/CONFIG_DIR/PRODUCTION_CSV/REGIME_CONFIG_PATH/RESUME_DIR ← G.*` (so `G_NARROW_MODELS_DIR`/`G_NARROW_CONFIG_DIR` env vars actually route writes — silently ignored pre-fix). Plus: `os.makedirs(_d, exist_ok=True)` loop because engine's `to_csv` assumes the redirected dir exists, and module-level `warnings.filterwarnings("ignore", message="X does not have valid feature names...")` because PowerShell 5.1's `*>&1` redirect was wrapping LGBM warnings as `NativeCommandError` and breaking the Tee. Workers spawned via `ProcessPoolExecutor` re-import the fork on Windows so all patches propagate. Production CSVs/configs/engine source untouched. Target: refine 17.7h → ~4h on full HRST (3-5× speedup). **Validation status**: TODO 0522 Stage 2 (9-12h, May 22) INVALID due to FIX #0 grid bug. TODO 0524 (top-5 HRST 5,6,8,11,12h) launched on Desktop 2026-05-24 20:27 with all 4 fix categories landed. If 0524 confirms reproducibility vs the May 20-21 direct g_narrow_d.py baseline, port the same 3 parallel patches into engine's H_STRICT_FAMILY K=5 block (lines 8831-8909) to speed up all post-H75 HRSTs by ~13h per run. |
...
Quick summary of current live (2026-05-21 → present, G_narrow models on H75 engine + macro_cache mtime fix 2026-05-27): detector `sma24>sma100`, bull=5h@65% RF+LGBM w=281 γ=0.9981 12f, bear=8h@65% RF+LGBM w=293 γ=0.9990 16f, shields OFF (both regimes), rally cooldown OFF (both regimes, manually toggled 2026-05-23 22:21; thresholds preserved in JSON), min_sell_pnl=0%, max_hold=10h, max_position_usd=$14,300. ETH block in `regime_config_ed.json` is clean (no legacy top-level keys — stripped 2026-05-27 12:10). Per-asset enabled state: ETH live, BTC/SOL/LINK/XRP all `enabled: false` (HRST done, results not promoted), BNB code-wired since 2026-05-02 (PySR/HRST not yet run).
