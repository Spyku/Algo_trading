...
0b. **Remove `/optimize` and `/optstatus` from trader bot** — User doesn't want to launch optimization from the trader (optimizer lives in its own bot). Currently removed from `/help` but handlers remain wired. Delete `_handle_optimize_command` + `_handle_optstatus_command` + dispatcher branches + `_optimize_proc` / `_optimize_lock` globals in `crypto_revolut_ed_v2.py` (~80 lines around line 1705 + branches around line 2515). Verify nothing else imports them.
