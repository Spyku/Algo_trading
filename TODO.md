...
## 📊 At-a-glance — active TODO dashboard (2026-05-31 — FAYE H done, RST pending)
...
| 🔥 **P0 (NEXT)** | **Launch RST for FAYE ETH 6h,7h,8h** — Mode R + Mode S + Mode T on the production CSV from today's H run | **Desktop** (~60-100 min) | 📅 READY — `python crypto_trading_system_faye.py RST ETH 6h,7h,8h --replay 1440`. Uses existing winners in `models_faye/crypto_faye_production.csv` (6h +55.3% Grid, 7h +37.54% Refined, 8h +46.37% Refined). Sleep-guard active. ETA 60-100 min. No refine, so bug #15/16/17/18 fixes don't affect this run. |
| 🔥 **P0 (BLOCKED)** | **Promote FAYE winners → live `models/` + restart trader** | **Desktop** (10 min) | ⏸ BLOCKED on (a) RST validation results, (b) hard A/B legacy run for bug #16 (P3-last below), (c) confirming Step 3 trade count for 6h winner is statistically robust (not a 5-trade cherry-pick of conf=90%). Until all 3 green, treat current FAYE winners as suspect. Recipe when ready: `Copy-Item models_faye\crypto_faye_production.csv models\crypto_ed_production.csv -Force; Copy-Item config_faye\regime_config_ed.json config\regime_config_ed.json -Force`. |
| 🔥 **P0 (TODAY's H run summary)** | **FAYE H ETH 6h,7h,8h --replay 1440** finished 2026-05-31 ~14:30 | — | ✅ DONE — Winners: 6h RF+LGBM w=150 γ=0.996 f=15 **+55.30%** WR=79.3% (Grid); 7h RF+LGBM w=151 γ=0.9992 f=15 **+37.54%** WR=82.4% (Refined); 8h RF+LGBM w=155 γ=0.9997 f=11 **+46.37%** WR=83.0% (Refined). All in `models_faye/crypto_faye_production.csv`. Ran with 5 bugs discovered/fixed mid-run (see Recently CLOSED). |
