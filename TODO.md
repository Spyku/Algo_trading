...
|---|---|---|---|
| 🔥🔥 **P0 — 0629 — FULL REVIEW (read first tomorrow)** | **BACKTEST ≠ LIVE = training-window EDGE (root-caused tonight) + 5 fixes committed + 1 pending HRST. ⚠️ AUTO-TRADE IS OFF; trader needs a restart.** ━━━ **THE BIG FINDING** ━━━ Chased "why did live 4h/4h lose −3.77%/wk (−$493) when the backtest says +3.85%". Ruled OUT, each by a test: execution/fills (order log = clean maker fills at the real bid; market-drift+venue decomp = **only 1.7pp**), timing/lag (1m spot lag-sweep: 60–90s ≈ **0pp**), data revision (inference AND training features **0.000000** diff — point-in-time rule HOLDS), device/nondeterminism (engine refits live's FROZEN training matrix at **97.5%**), and two of my own **timezone errors** (ledger bare ts = CEST, bars = UTC). **ROOT CAUSE:** the backtest (`generate_signals`, faye) and the live trader (`generate_live_signal`, [crypto_live_trader_ed.py:611](crypto_live_trader_ed.py#L611)) are **two code paths that build different training windows** on identical data — differ by ~1-2 edge rows; that 1 highest-γ-weight row flips **25% of signals** (live↔backtest agreement **75%**). `FAYE_FAITHFUL_WINDOW=1` ([faye.py:3106](crypto_trading_system_faye.py#L3106)) replicates live's edge → **75%→99%**. ━━━ **THE LEAK (F2, FIXED)** ━━━ live trained **1 row PAST the leakage-free boundary** (its newest training row's label used the FORMING bar) on **162/162** hours; backtest over-embargoed 1 row. Fixed both to leakage-free (`_leak_rows=(n-1)-i` drop, last training row = `i-horizon`): trader [:905](crypto_live_trader_ed.py#L905) + shadow `crypto_live_shadow.py` (mirrored, Rule F2) + `FAYE_FAITHFUL_WINDOW` edge-3→edge-4. NOT an embargo (dropped row's label came from the incomplete forming bar). ━━━ **5 COMMITS TONIGHT** ━━━ `30c270e` atomic writes (the 7 merge/download `to_csv` the 06-24 fix missed → kills the "No columns to parse"/"time data" race) · `3bd053e` cross_asset.csv history-clobber (a FAYE-HRST/trader write race truncated 1637→9 rows → xa_corr NaN → 8h PySR NaN → 0 signals; restored + hardened `_yf_merge_with_existing` + new **[4] DATA INTEGRITY** daily-sanity check) · `08ddb27` **incremental ETH derivatives download** (P1 blocking pre-order download 33s→4s; was a full 2022→now re-pull every 2h; validated byte-identical; this is the live order-placement-latency fix, separate from the backtest gap) · `2460937` FAYE_FAITHFUL_WINDOW (75→99%) · `da57e72` F2 leak fix. ━━━ **DO TOMORROW (in order)** ━━━ **(1)** restart the FLAT trader → loads atomic-writes + incremental-download + leak-fix; **confirm shadow `live==core ≥99%`** (the post-deploy validation) + **`python tools/sanity_check.py --quick`** shows `[4] DATA INTEGRITY PASS`. **(2)** launch leakage-free re-selection: `python crypto_trading_system_faye.py HRST ETH 4,5,6,7,8h --replay 1440` (leakage-free is now the DEFAULT — no env var) (~5-9h; PySR 4-8h exists). **Both 4h/4h AND 8h/5h were selected on the broken backtest → both suspect; re-select faithfully.** **(3)** re-run the 4h/4h-vs-8h/5h head-to-head with the flag (old verdict ran on the divergent path — untrustworthy). ━━━ **OPEN Qof for the review** ━━━ (a) make `FAYE_FAITHFUL_WINDOW` the **DEFAULT** for generate_signals? edge-4 is leakage-free AND matches live, so safe — but decide vs the embargo-A/B semantics; do it AFTER the HRST lands, not mid-flight. (b) parallelize the P2 derivatives (4 other-asset full re-pulls ≈132s, post-order — bandwidth only). (c) the head-to-head numbers from earlier tonight (4h/4h +2.40% vs 8h/5h −2.29%) are on the NON-faithful path → ignore. ━━━ **PROCESS LESSON (for me)** ━━━ I chased execution→data→fills→venue→embargo (all wrong, several from sloppy tz/benchmark) before the one test that settled it: **compare the two SIGNAL streams / trade lists FIRST.** `bt_execution_lag.py` (committed) + the scratchpad diagnostics encode the methodology. | **Laptop (trader) + Desktop (HRST)** | 🔥 **REVIEW + DO (1)(2)(3). Auto-trade OFF; trader restart pending.** |
| ✅ DONE 0625 — LIVE | **Telegram `/setup` toggle persists + single popup — FIXED & LIVE (trader rebooted 2026-06-25 23:15 UTC, session `ed_v2_20260625_011517`; this restart also loaded the 06-24 snapshot-write fix).** Fix: new `_setup_commit(cfg, trading_cfg)` ([crypto_revolut_ed_v2.py:3917](crypto_revolut_ed_v2.py#L3917)) saves the snapshot to disk + syncs live `trading_cfg` after every toggle/setter (enabled/tp/maker/conf/max, 6 mutation points), and the redundant confirmation message was dropped so each action sends ONE menu (was confirmation+menu = 2 popups). Smoke `tools/smoke_test_setup_toggle.py` 11/11 (persists, syncs, sticks WITHOUT /cfg_save, one menu / zero stray confirmation). **⏳ Reboot the trader (when flat) to load it.** ROOT CAUSE (high conf): the asset `enabled` toggle ([crypto_revolut_ed_v2.py:3968](crypto_revolut_ed_v2.py#L3968)) mutates only the in-memory `_setup_state['cfg']` SNAPSHOT and sends a misleading "✅ done" — it persists to `regime_config_ed.json` ONLY on `_setup_finish`→`save_trading_config` ([:3904](crypto_revolut_ed_v2.py#L3904)), so a toggle "doesn't stick" unless setup is finished. INCONSISTENT: the auto-trade toggle ([:3987](crypto_revolut_ed_v2.py#L3987)) DOES `save_position` immediately. Same latent bug on tp ([:3999](crypto_revolut_ed_v2.py#L3999)) + maker ([:4010](crypto_revolut_ed_v2.py#L4010)). "2 differently-designed popups that contradict" (medium conf — needs a live reboot to reproduce): (a) handlers send a plain confirmation AND a separate button-menu = 2 messages; (b) setup menu renders from the SNAPSHOT while the main keyboard (`_main_buttons()` :3465, via /status & cycle) renders from LIVE config → mid-edit they disagree. **FIX:** after each toggle `save_trading_config(cfg)` + sync `trading_cfg[asset]=cfg[asset]` (persist + live-sync, matching auto-trade); collapse confirmation+menu to one message; render menus from one re-read of live config. **Smoke-test before commit; takes effect on next restart → implement+commit anytime (code is safe while invested), reboot when FLAT.** WORKAROUND LIVE: BTC set `enabled:false` directly in config 0625 (backup `config/regime_config_ed_pre_btcoff_20260625.json`) — bypasses the broken toggle. (Earlier mis-diagnosis: thought it was 2 trader instances; user confirmed ONE engine, 2 differently-*designed* menus.) | Laptop (code) | ✅ LIVE (rebooted 06-25 23:15 UTC, smoke 11/11) |
| ✅ DONE 0624 | **Snapshot-write race fixed ("No columns to parse from file" — broke live cycle 06-24 18:01).** `download_macro_data.py` IV/orderbook/whale saves did bare `to_csv` (truncate→write) + **unguarded** `read_csv` of the existing file → a reader/Drive-sync catching it mid-write at 0 bytes raised `EmptyDataError`. **A** read-guard (`getsize>0`+try/except → self-heal to fresh), **B** atomic write (`_atomic_to_csv` = temp+`os.replace` → never seen empty). Smoke `tools/smoke_test_snapshot_atomic.py` 9/9. (Open follow-up: the same anomaly day also had the `🔄 Models updated` rerun = benign fingerprint change, and a transient `WinError 10054`/`getaddrinfo` net blip = self-recovered.) | Laptop | committed+pushed |
| 📌 | **FAYE LIVE — CURRENT (verified 2026-06-24 against `config/regime_config_ed.json`, [crypto_live_trader_ed.py:168](crypto_live_trader_ed.py#L168)):** detector **`price>sma72`** · **bull 4h@65%** / **bear 4h@70%** (both RF+LGBM w250 γ0.996, feat-set D, 10 feats) · **BOTH rally-cooldown gates OFF** (disabled 06-23 — no gate beats no-gate on 4h: **0/2304** sweep configs, reconfirmed by a fresh rerun 06-24) · **shields OFF** · min_sell_pnl 0% · max_hold 10h · max_position $14,300 · maker ON · **ETH only** (BTC/SOL/LINK `enabled:false`). Inference via `compute_signal_core` (faye). **Rollback to the 06-22 gates-ON promote state:** `cp config/regime_config_ed_pre_gateoff_20260623.json config/regime_config_ed.json` (hot-reloads ≤5 min). **⚠️ WATCH:** 4h/4h is historically-marginal (Mode-C history ~2mo, recent-window-favoring); the rally-free OOS comparison is still open. ━━━ **HISTORICAL TRAIL (newest → oldest)** ━━━ ✅ **Gate/shield RESTORED + LIVE 2026-06-18 23:16** (laptop edit → Drive → **trader restarted on desktop**): **bull shield ON · bull gate ON (double: rr30≥4.0 OR rr36≥6.5 cd24) · bear gate ON (single: rr8≥2.5 cd14, t_long=999) · bear shield OFF** — Idea-1 single bear gate kept (full detail in the 📌 LIVE STATE block below). **⚡ UPDATED 2026-06-20 (laptop, ETH flat): bull shield ON→OFF (fresh Mode T) + BULL GATE DISABLED (no bull gate).** Live now (until 06-22) = **bear single `rr8≥2.5 cd14` + NO bull gate + both shields OFF** (Idea-1 best; T found no STRICT single-gate winner so the bull gate wasn't worth keeping). Backups `config/regime_config_ed_pre_singlegate_T_20260620.json` + `config/regime_config_ed_pre_bullgateOFF_20260620.json`. **⚡⚡ PROMOTED 2026-06-22 (ETH flat, user-directed, Mode-C PROMOTE) → CURRENT LIVE: detector `price>sma72`, bull 4h@65 / bear 4h@70 (both RF+LGBM w250 γ0.996, feat-set D 10 feats, NO sparse), gates ON [bull rr10≥2.0 OR rr18≥7.0 cd16 / bear rr8≥5.0 OR …≥3.0 cd48], shields OFF, min_sell 0, max_hold 10.** Source = sparse-feature HRSTC (`logs/hrst/faye_HRSTC_ETH_4_5_6_7_8h_1440_20260621_001628.log`), Mode-C gate **PROMOTE: challenger beat incumbent 6/6 windows +12.2pp** (med+worst +58.3 vs +46.1; span Apr 23→Jun 22). Gates verified: flat ✓ · PySR-4h `historical` ✓ · detector ENABLED ✓ · clean config diff (only ETH regime changed, all live settings preserved) ✓. Backups `config/regime_config_ed_pre_promote_20260622_pricesma72.json` + `models/crypto_ed_production_pre_promote_20260622_pricesma72.csv`. **⚠️ WATCH:** 4h/4h is a historically-marginal horizon; Mode-C history only ~2 months; the 15' arc just showed recent-window wins can reverse OOS — monitor live. Gates came out DOUBLE (real t_long, not the single-9999 sentinel) though faye's sweep is single — investigate later (likely Mode-T no-STRICT-winner kept a seeded double). **Rollback:** `cp config/regime_config_ed_pre_promote_20260622_pricesma72.json config/regime_config_ed.json; cp models/crypto_ed_production_pre_promote_20260622_pricesma72.csv models/crypto_ed_production.csv` (hot-reloads ≤5 min, do FLAT). **⚡ 2026-06-23 (ETH flat) — ETH rally-cooldown gates DISABLED + single-formed (RESOLVES the "investigate later" DOUBLE-gate above).** Root cause: FAYE only ever computes SINGLE gates ([faye.py:7975](crypto_trading_system_faye.py#L7975)), but the 06-21 4h/4h gate sweep found **NO STRICT WINNER ×4 → `config not modified`** ([faye.py:8048](crypto_trading_system_faye.py#L8048) — the no-winner branch is a no-op, never writes `enabled:false`), so stale **pre-06-16 `enabled:true` DOUBLES** survived in the `_noprod` staging config and the verbatim promote shipped them live (NOT computed by the 4h run). Gate-vs-no-gate, measured on the run's own sweep CSVs (`output/rally_cd_ETH_20260622_1405*.csv`): **0 / 2304 gate configs beat no-gate on ANY segment** (best bull −1.3pp REF, every bear −25pp) → no gate is correct on 4h/4h (reverses the 5h/8h Idea-1 +7.7pp — a cooldown hurts a fast/frequent horizon). Set `enabled:false` + single-form (`h_long=h_short`, `t_long=9999`) in all THREE live-chain configs: `config/regime_config_ed.json`, `config_faye/regime_config_faye.json`, `config_faye/regime_config_faye_noprod.json` (the real `--promote` source [faye.py:8434](crypto_trading_system_faye.py#L8434) + reseed origin [faye.py:674](crypto_trading_system_faye.py#L674)). Cleared stale `rally_cooldown_until=2026-06-24T12:00Z` in `position_ed_v2_ETH.json`. Backups `*_pre_gateoff_20260623.json`. **Rollback:** `cp config/regime_config_ed_pre_gateoff_20260623.json config/regime_config_ed.json`. **✅ ENGINE GUARD (a) BUILT 2026-06-23** ([faye.py:8048](crypto_trading_system_faye.py#L8048)): the sweep's no-winner branch now WRITES an explicit `enabled:false` single-form gate (`h_long=h_short`, `t_long=9999`) instead of `return None`/no-op — config always reflects the sweep's actual conclusion; a stale double can never survive a run again. Regression test `tools/test_gate_disable_on_nowinner.py` PASSES (seeds a stale enabled DOUBLE + flat no-rally stream → asserts disabled single-form). Byte-compiles clean; convergence unaffected (reads config fingerprint, not return). **🔵 STILL OPEN:** (b) `--promote` gate-validation (assert live gate ≡ sweep output), and the broader **selection-ranking review** — the binary STRICT-3of3 cliff rejects gates that merely tie on a rally-less half; a clean ranking should pick the best of {each gate, no-gate} by one robust score rather than abstain. See the ✅ DONE 0620 row ↓. Pre-restore backup `config/regime_config_ed_pre_restore_correct_20260618_231601.json`. Rollback baselines: `archive/g_narrow_live_20260531_142202/` + `config/regime_config_ed_pre_*.json`. [Prior row text — `tsmom_672h`/bull-6h/bear-5h@80, June-2 — was STALE: never updated after the 06-16 sma48 promotion. Corrected here.] **⚠️ CONFIG-DRIFT INCIDENT (found 2026-06-12):** the live `regime_config_ed.json` had silently DRIFTED on June-8 23:18 to **bull 8h / bear 8h@70 / both rally-cooldown gates `enabled:false` / bear shield ON / min_sell_pnl 0.5** — from an "inconclusive" June-8 8h promotion (`logs/ed_v1_20260602_215119.log` "Promoting ETH 8h → production return=8.98%") that left NO `_pre_` backup and dropped the validated gates. So the trader ran ~4 days with NO cooldown gates + wrong horizons. **NOT caused by the 0611/0612 feature testing** (all `--no-persist`; config mtime was June-8, pre-testing). **RESTORED 2026-06-12 08:52** to the June-2 RST winner (bull 6h@65 / bear 5h@80 / shields OFF both / gates ON [bull rr8h≥2.0 OR rr14h≥6.0 cd6h; bear rr10h≥5.5 OR rr12h≥2.0 cd8h] / min_sell_pnl 0) — gate params recovered from `config/regime_config_ed_pre_RST_bear5h_20260602.json` + winner log `ed_v1_20260602_210324.log` (line 478/611). Backup of the drifted config: `config/regime_config_ed_pre_restore_6h5h_20260612_085249.json`. Applied while trader was INVESTED per explicit user override of Rule 19. | **Desktop** (always) | 🟢 running |
...
| 🟡 **QUEUED (0620)** | **Mode T single-gate — collapse the live bull DOUBLE gate to single. CODE DONE + pushed; launch a fresh Mode T once the desktop frees.** ✅ Ported faye's single-gate `_sweep_rally_cooldown` into `crypto_trading_system_ed.py` (the engine the user actually runs Mode T on — "faye doesn't do anything right now"): the sweep is now ONE condition `rr[h]≥t` per candidate (was the 2-window `rr[h_s]≥t_s OR rr[h_l]≥t_l` double); loop `pairs`→single `(h,h,t,DEAD_TL=9999,cd)`, plateau dims 5→3 (hs,ts,cd), winner print drops the OR leg. **5-key config schema preserved** (`h_long=h_short`, `t_long_pct=9999` dead sentinel) so the trader's `_update_rally_cooldown`, T↔G cmp and `/gate` consume it unchanged. ✅ Trader Telegram display (`crypto_revolut_ed_v2.py` new `_fmt_gate`, dead-leg thr 900 covers 999/9999) now HIDES the bogus `…OR rrXh≥9999%` leg → single-gate configs render as ONE gate (verified live: **bear → `rr8h≥2.5%`**, was `…OR rr30h≥999%`; **needs trader restart** to activate display). Both committed+pushed **`e5b30a2`**. **STILL PENDING (user's chosen path):** the live `config/regime_config_ed.json` **bull** gate is a GENUINE double `rr30≥4.0 OR rr36≥6.5 cd24` — neither the engine change (affects FUTURE runs only) nor the display fix collapses a real double. User chose the PROPER path = **re-run Mode T to recompute the bull (+bear) gate single from data**, NOT a hand-pick. **LAUNCH WHEN:** a Fujiwara HRST (15/30) on the desktop FINISHES (frees the machine) **AND ETH is FLAT** (Rule 19 — `run_mode_t` writes BOTH single gates straight to live `regime_config_ed.json` = the promotion; ETH was `cash` at 2026-06-20). Cmd: `python crypto_trading_system_ed.py T ETH --replay 1440`. **✅ DONE 2026-06-20 (laptop, ETH flat):** Mode T ran (single-gate sweep = my port, 1,152 single cfgs) but **NO STRICT WINNER** (`beats_3of3=15` but `STRICT(3of3+plateau≥0.7)=0` both regimes, both T↔G passes — `logs/hrst/ed_T_ETH_1440_20260620_225455.log`) → gates left unchanged (the documented Idea-1 window-fragility: a gate beats no-gate baseline but none robust). T's ONLY change: bull shield ON→OFF (kept, user choice). **Since no single gate cleared STRICT AND Idea-1 found the bull gate net-harmful, user chose to DISABLE the bull gate entirely** → live now = **bear single `rr8≥2.5 cd14` + NO bull gate** (Idea-1 best). Applied to live `config/regime_config_ed.json` (`bull.rally_cooldown.enabled=false`), ETH flat. Backups: `config/regime_config_ed_pre_singlegate_T_20260620.json` (pre-T) + `config/regime_config_ed_pre_bullgateOFF_20260620.json` (post-T pre-disable). | **Laptop** | ✅ **DONE 0620** — live: bear single gate + no bull gate; bull shield OFF. |
