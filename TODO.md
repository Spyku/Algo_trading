...
**Open follow-ups:**
- Relaunch G_narrow_d on Laptop for the 3-way verdict (TODO 1705 — diagnose laptop crash first).
- Watch for first H75-driven BUY signal (5h@75% bull or 8h@65% bear with new gates).
- After ~1-2 weeks of live H75 performance, audit realized return vs sim and decide whether to keep, rollback, or rerun comparison.

---

### 🟢 TODO 1805D — G_narrow_d full HRST on Desktop, isolated (2026-05-18 late evening)

**Search anchor**: `TODO 1805D`

**Status (2026-05-19 ~00:30 CEST)**: 🔄 **JUST LAUNCHED on Desktop, isolated to `models_g_desktop/` + `config_g_desktop/`**. Running concurrently with TODO 1805 (H75 fresh data, also Desktop, isolated to `models_h75/`) and TODO 1805C (G on Laptop, default `models/`). 3-way concurrent isolation verified via `[G_NARROW_D_ISO]` startup banner. ETA ~30-42h (GPU queue contention with concurrent 1805 H75 on same machine).

**Why this exists**: TODO 1805C runs G on Laptop. This TODO runs G ALSO on Desktop, isolated, as a backup/confirmation:
1. Cross-machine reproducibility: confirm G's Mode T total is consistent across Laptop and Desktop (rules out single-machine noise)
2. Defensive check on the H75 promotion (TODO 1706 → live): ensure G is genuinely worse than H75, not just on Laptop but also on Desktop
3. Hedge if Laptop G dies again (Laptop has crashed mid-HRST twice now per TODO 1805C history)

H75 is ALREADY PROMOTED to live (per the 22:02 PROMOTED entry at the top). This TODO is confirmatory — does not block production decisions. Even if Desktop G > H75 by some margin, the promotion stands unless the gap is large enough to justify a rollback (>+5pp Mode T vs H75 + user decision per [[feedback-production-swap-when-flat]]).

#### Critical isolation (added to G_narrow_d.py 2026-05-18)

Same env-var-driven pattern as H_strict_family. G_narrow_d.py now reads `G_NARROW_MODELS_DIR` and `G_NARROW_CONFIG_DIR` env vars and redirects ALL writes. Default behavior preserved when env vars unset.

Desktop G must NOT collide with:
- **Laptop G** (TODO 1805C) — writes to default `models/`/`config/`
- **Desktop H75** (TODO 1805 in-flight) — writes to `models_h75/`/`config_h75/`
- **Live trader** — uses production CSV (insulated by `--no-persist`)

Desktop G writes to `models_g_desktop/` + `config_g_desktop/`. Full mutual isolation.

#### Step 1 — Setup isolated dirs

```powershell
cd "G:\Autres ordinateurs\My laptop\engine"
New-Item -ItemType Directory -Path "models_g_desktop" -Force | Out-Null
New-Item -ItemType Directory -Path "config_g_desktop" -Force | Out-Null
Copy-Item "models\crypto_ed_production.csv" "models_g_desktop\crypto_ed_production.csv"
Copy-Item "config\regime_config_ed.json" "config_g_desktop\regime_config_ed.json"
Copy-Item "models\crypto_ed_grid_ETH_*_G_NARROW_D.csv" "models_g_desktop\"
Copy-Item "models\crypto_ed_best_models.csv" "models_g_desktop\crypto_ed_best_models.csv"
Get-ChildItem models_g_desktop | Format-Table Name, Length, LastWriteTime
```

Expected files in `models_g_desktop/`:
- `crypto_ed_production.csv` (~11 KB — seed for --no-persist)
- `crypto_ed_best_models.csv` (~69 KB — candidate input)
- `crypto_ed_grid_ETH_5h_G_NARROW_D.csv` (~2.2 KB)
- `crypto_ed_grid_ETH_6h_G_NARROW_D.csv` (~2.2 KB)
- `crypto_ed_grid_ETH_7h_G_NARROW_D.csv` (~2.2 KB)

**Note**: `crypto_ed_grid_ETH_8h_G_NARROW_D.csv` does NOT exist (both prior G HRST attempts stopped before 8h Mode D). Desktop G will run 8h Mode D fresh (~12 min) as part of the HRST chain.

#### Step 2 — Launch G HRST on Desktop with isolation
...
#### Critical guards (live-tested)

- **`"ETH,"` with TRAILING COMMA** — without it, `ETH` matches `endswith('h')` parser → all 9 assets default
- **`--no-data-update` REQUIRED** — snapshot env var only redirects READS; without this flag, engine downloads + writes to LIVE `data/macro_data/`
- **`--no-persist` REQUIRED** — writes go to `*_noprod.*` derived from the isolated dirs
- **`--skip` REQUIRED** — Mode D runs only for 8h (5h/6h/7h grids exist post-Step-1)
- **`--grid-tag G_NARROW_D` REQUIRED** — tells engine to read/write G-tagged grid CSVs; without it, engine uses untagged grids (production B's)
- **Both env vars MUST be set** — otherwise G writes to default `models/`/`config/`, colliding with Laptop G run

#### Startup banners to confirm

1. `[G_NARROW_D_SNAPSHOT] pd.read_csv redirected: ...` ← snapshot ON
2. **`[G_NARROW_D_ISO] output dirs redirected: models=models_g_desktop config=config_g_desktop`** ← **isolation active — abort if missing**
3. `--no-persist ACTIVE` block listing `models_g_desktop/crypto_ed_production_noprod.csv` and `config_g_desktop/regime_config_ed_noprod.json`
4. `ED: Mode HRST | ETH | 5h,6h,7h,8h`
5. `Mode D results already exist for ETH 5h — skipping D (--skip)` × 3 (5h/6h/7h)
6. `EXHAUSTIVE GRID: ETH 8h — 36 evals` ← G's narrow 36-eval grid for the missing 8h
7. Per horizon Mode V: `75 trials per config | gamma ±0.002 | features ±5 | window ±20h`
8. `[G_NARROW_D_K5] _deku_eval_with_pruning patched (K=5 seeds=[42, 43, 44, 45, 46])` ← appears later when workers spawn (post Step 1 dispatch)

**Any banner missing or wrong = abort and fix.**

#### ETA (Desktop, concurrent with H75 in-flight + live trader)

| Concurrency scenario | Per-horizon Mode V | 4 horizons | R + S + T+G | Total |
|---|---|---|---|---|
| G alone on Desktop | ~5h | ~20h | ~1.5h | ~22h |
| **G + H75 (TODO 1805) concurrent** | ~7-10h (GPU queue contention) | ~28-40h | ~1.5h | **~30-42h** |

Done midday-to-late 2026-05-20 worst case. Trader unaffected (priority arrangement).

#### Step 3 — After HRST completes, snapshot final results

```powershell
$ts = Get-Date -Format "yyyyMMdd_HHmm"
$dest = "models\_archive_g_desktop_COMPLETE_$ts"
New-Item -ItemType Directory -Path $dest -Force | Out-Null
Copy-Item "models_g_desktop\*" $dest -Recurse
Copy-Item "config_g_desktop\*" $dest -Recurse
Copy-Item "logs\ed_v1_$(Get-Date -Format yyyyMMdd)_*.log" $dest -ErrorAction SilentlyContinue
```

#### Success criteria → cross-machine + cross-variant verdict

| Comparison | Interpretation | Action |
|---|---|---|
| Desktop G Mode T ≈ Laptop G Mode T (±3pp) | reproducible — G's "real" Mode T identified | Use either; compare to H75 |
| Desktop G ≠ Laptop G (>5pp) | machine-dependent noise | Investigate K=5 seed handling, Optuna sampler determinism |
| Best-of-G < H75's Mode T (which was the promotion winner) | confirms H75 promotion was correct | Defensive check passed; no action |
| Best-of-G > H75's Mode T by ≤+5pp | borderline | Document, don't act |
| Best-of-G > H75's Mode T by >+5pp | possible promotion regret | Discuss with user — rollback only when trader flat per [[feedback-production-swap-when-flat]] |

#### Cross-contamination matrix (Laptop G + Desktop G + Desktop H75 all running)

| Shared file | Laptop G writes | Desktop G writes | Desktop H75 writes | Collisions |
|---|---|---|---|---|
| Grid CSVs (5h/6h/7h G) | `models/...G_NARROW_D.csv` | `models_g_desktop/...G_NARROW_D.csv` | n/a (different tag) | ✅ none |
| Grid CSV 8h G | `models/` (whoever finishes first) | `models_g_desktop/` | n/a | ✅ none |
| Grid CSVs H | n/a | n/a | `models_h75/...H_STRICT_FAMILY.csv` | ✅ none |
| production_noprod.csv | `models/` | `models_g_desktop/` | `models_h75/` | ✅ none |
| best_models.csv | `models/` | `models_g_desktop/` | `models_h75/` | ✅ none |
| regime_config_ed_noprod.json | `config/` | `config_g_desktop/` | `config_h75/` | ✅ none |
| Live trader prod files | never (--no-persist) | never (--no-persist) | never (--no-persist) | ✅ trader safe |

All three runs mutually isolated. Live trader insulated from all three.

---

### 🟢 TODO 1805C — Relaunch G_narrow_d full HRST on Laptop (2026-05-18, today)

**Search anchor**: `TODO 1805C`

**Status (2026-05-19 ~00:30 CEST)**: 🔄 **RUNNING on Laptop**. User confirmed running 2026-05-18 evening. From Desktop's Drive view, log writes lag arbitrarily — verify state directly on Laptop with `Get-Process python` and `tail -f` on the active log. **Per [[feedback-drive-sync-lag]]: don't assert dead-from-Desktop without direct Laptop check.**

**Why this exists**: G_narrow_d was supposed to finish on Laptop per TODO 1705 (2026-05-17) but the Laptop crashed mid-HRST. The H75 promotion entry's "Open follow-ups" explicitly lists "Relaunch G_narrow_d on Laptop for the 3-way verdict (TODO 1705 — diagnose laptop crash first)." Now that H75 is live and the in-flight TODO 1805 H75-on-fresh-data is occupying Desktop, today is the right time to relaunch G on Laptop in parallel.

**Goal**: complete the 3-way verdict table from TODO 1706 — `B (production, +89.41% Mode T) vs G_narrow_d (?) vs H75 (?)`. Currently only H75 is filled in (the promoted config). G is the missing leg.

#### Step 0 — Diagnose the 2026-05-17 Laptop crash FIRST

**Do not relaunch until root cause is known.** Check `logs/ed_v1_*` on Laptop for the crash signature. Common causes seen historically:
- Modern Standby — Laptop went to sleep mid-run (logs go silent, no exception). Fix: run from a powered terminal with `powercfg /setactive SCHEME_MIN` or use `start_ed_v2.bat`-style auto-restart wrapper.
- OOM — Laptop has less RAM than Desktop (32GB vs 32GB but more apps open). Check Task Manager peak memory during prior G runs.
- LGBM GPU OOM — RTX 3070 Ti has 8GB VRAM vs Desktop 4080's 16GB. Symptom: silent worker death after a few hours of Mode V.
- Drive sync conflict — both machines writing to the same Drive-synced files. Less likely since `--no-persist` isolates to `*_noprod.csv` and grid CSVs use `--grid-tag G_NARROW_D`.

If diagnosis is inconclusive after 15-30 min of log review, switch the launch to Desktop instead (sequential after TODO 1805 H75 finishes) — slower wall-clock but reliable.

#### Step 1 — Snapshot existing G artifacts BEFORE relaunch

```powershell
cd "G:\Autres ordinateurs\My laptop\engine"
$ts = Get-Date -Format "yyyyMMdd_HHmm"
$dest = "models\_archive_g_narrow_d_relaunch_$ts"
New-Item -ItemType Directory -Path $dest -Force | Out-Null
Copy-Item models\crypto_ed_grid_ETH_*_G_NARROW_D.csv $dest -ErrorAction SilentlyContinue
Copy-Item logs\ed_v1_20260517_022856.log $dest -ErrorAction SilentlyContinue
Copy-Item logs\ed_v1_20260516_144648.log $dest -ErrorAction SilentlyContinue
```

#### Step 2 — DATA CHOICE (decide before launch)

| Option | Command suffix | Pros | Cons |
|---|---|---|---|
| **A: May 15 snapshot** (apples-to-apples with original 3-way plan) | `--no-data-update` + `$env:V2_DATA_SNAPSHOT="data\_reliability_hrst_snapshot_desktop_20260515_154801"` | Matches H75 promotion HRST exactly. Original TODO 1705 / 1706 3-way comparison table fills cleanly. Live trader insulated from any data writes. | G runs on data 3 days stale by now. If detector or feature ranking is sensitive to recent regime drift, G's Mode T won't reflect what G would do TODAY. |
| **B: Fresh data** (apples-to-apples with TODO 1805 in-flight H75 fresh-data run) | (no `--no-data-update`, no snapshot env var) | Parallels TODO 1805 — clean head-to-head G vs H on freshest data. Easiest interpretation if 1805 wins on fresh data. | Engine downloads + writes to live `data/macro_data/` (~5-10 min added). 3-way table mixes data eras: B was promoted on different data than G + H are now run on. |

**Recommendation**: **Option A (snapshot)** for the original 3-way verdict, BECAUSE H75 was promoted off that same snapshot. The fresh-data H75 (TODO 1805) is a separate question (does H75 hold up on fresh data?) and shouldn't be the comparison anchor for G. After both A-result-G and TODO-1805-H75 finish, you'll have **two** comparison axes:
- Snapshot: B (+89.41% from May 15) vs G (from Option A) vs H (the H75 promotion HRST)
- Fresh data: (B not run on fresh data) vs G (would need separate run) vs H (TODO 1805)

#### Step 3 — Launch command (Option A — recommended)
...
**Critical guards** (per TODO 1705 — all live-tested):
- **`"ETH,"` with TRAILING COMMA, double-quoted** — without comma, `ETH` matches `endswith('h')` parser in the OLDER fork (g_narrow_d predates the merged-engine asset-first fix). Confirm `ASSET: ETH` at startup, not `ASSET: BTC`.
- **`--no-data-update` REQUIRED** for snapshot — without it, fresh downloads overwrite snapshot data on disk and break the comparison.
- **`--no-persist` REQUIRED** — production CSV stays untouched.
- **`--skip`** — skips Mode D for 5h/6h/7h horizons (grids exist from prior runs). Without `--skip`, Mode D re-runs (~40 min wasted).

#### Step 3 alt — Launch command (Option B — fresh data)

```powershell
$env:RELIABILITY_K = "5"
python crypto_trading_system_ed_g_narrow_d.py HRST "ETH," 5h,6h,7h,8h --replay 1440 --no-persist --grid-tag G_NARROW_D
```

(No snapshot env var. No `--no-data-update`. No `--skip` because fresh data invalidates the existing grid CSVs — Mode D will re-run for all 4 horizons.)

#### Startup banners to confirm

- Option A: `[G_NARROW_D_SNAPSHOT] pd.read_csv redirected: ...` — snapshot redirect ON
- Both: `[G_NARROW_D_K5] _deku_eval_with_pruning patched (K=5 seeds=[42, 43, 44, 45, 46])` — K=5 active
- Option A: `[--no-data-update] Skipping macro + OHLCV downloads. Using whatever is currently on disk.`
- Both: `ED: Mode HRST | ETH | 5h,6h,7h,8h | 150 trials` (NOT all 9 assets)
- Per horizon Option A: `[--skip] Found existing grid CSV — skipping Mode D` (5h/6h/7h) + `EXHAUSTIVE GRID: ETH 8h — 36 evals` (only 8h re-runs)
- Per horizon Option B: `EXHAUSTIVE GRID: ETH Xh — 36 evals` for all 4 horizons (fresh data, no skip)

If banners are wrong, abort and re-export env vars.

#### ETA (Laptop)

- Option A: Mode D 8h ~12 min + Mode V 4h @ K=5 + 75 trials wider-Optuna ~6.5h × 4 = ~26h + Mode R/S/T+G ~1.5h = **~22-27h total**. Finishes evening 2026-05-19 to morning 2026-05-20.
- Option B: + extra ~30-40 min for Mode D × 4 horizons (no skip) + ~10 min data download. **~23-28h total**.

#### Step 4 — Snapshot final results after HRST completes
...
#### Success criteria → 3-way verdict table (fill once G finishes)

| Variant | Mode T total (May 15 snapshot) | vs B (+89.41%) | Notes |
|---|---|---|---|
| B (production, pre-H75) | +89.41% | — | baseline |
| G_narrow_d | TBD | TBD | This run |
| H75 (promoted) | TBD (cross-ref from H75 promotion HRST log) | TBD | Live since 2026-05-18 22:02 |

| Outcome | Action |
|---|---|
| G > H75 by >5pp on Mode T (snapshot) | G's wider-Optuna + force-combo architecture beats H75's strict-(combo,w) dedup. Consider rolling back H75 and promoting G when trader flat. **Discuss with user first** — H75 is already live and producing real OOS data; rollback has cost. |
| H75 > G by >5pp on Mode T (snapshot) | Confirms tonight's H75 promotion was correct. SHELVE G_narrow_d. Document why wider Optuna lost to strict dedup. |
| G ≈ H75 (±5pp) on Mode T (snapshot) | Both architectures equally valid. Keep H75 live (simpler code, smaller engine delta from B). Note G as a future-revival candidate if H75 starts underperforming live. |
| G < B's +89.41% on Mode T (snapshot) | Wider-Optuna doesn't help. SHELVE G_narrow_d definitively. |

#### Comparison context (filled per-horizon — pre-relaunch)

| Horizon | B (production, pre-H75) | G_narrow_d (Desktop 2026-05-17 partial, killed mid-7h) | H75 (promoted, 2026-05-18) |
|---|---|---|---|
| 5h Mode V | +59.05% XGB+LGBM 150h | **+74.54%** RF+LGBM 281h | XGB+LGBM 100h γ=0.9993 7f (return TBD from log) |
| 6h Mode V | +68.47% XGB+LGBM 150h | +46.40% RF+LGBM 137h | XGB+LGBM 106h γ=0.9986 18f |
| 7h Mode V | +11.21% XGB+LGBM 72h (lottery) | +29.94% XGB+LGBM 100h (killed mid-refine) | XGB+LGBM 100h γ=0.9990 20f |
| 8h Mode V | +56.54% RF+LGBM 150h | not run (killed before 8h) | **RF+LGBM 162h γ=0.9954 6f** ← live bear |
| **Mode T total** | **+89.41%** | pending TODO 1805C | pending audit of promotion HRST log |

---

### 🔍 TODO 1805B — Migrated from CLAUDE.md: 5 pending research items (2026-05-18)

**Search anchor**: `TODO 1805B`

**Why this exists**: CLAUDE.md (post-2026-05-18 split) is supposed to be stable reference material only; volatile TODOs belong here in TODO.md. Audit of CLAUDE.md surfaced 5 pending research items still embedded in the Engine Reference Card that should be tracked here so they don't get lost. Items 6 (BTC SOPR) and 7 (Ed15 15-min build) are already tracked/resolved in TODO.md and not duplicated below.

These items are NOT all created today — preserving their discovery dates inline. The 1805B number marks today's migration into the canonical scoreboard.

#### Item 1 — Vol-scaled horizons: validate on 4-month window

- **Source**: CLAUDE.md "What's promising (tested, pending decision)" line 94
- **Discovery date**: 2026-04-19
- **Status**: Tested on ETH 2mo replay via `tools/test_vol_scaled_horizon.py`. Best: `vol_2band low→8h high→6h @90%` = +33.82% / 46 trades / 65% WR. Beats current tsmom regime (+28.80%) by **+5.02pp** and beats every single-horizon baseline. Logic: high vol (>70th pctile) → shorter 6h horizon (faster signal); low vol → longer 8h (more confirmation).
- **Next step**: Validate on 4-month window. Consider replacing tsmom detector or combining tsmom+vol.
- **Effort**: low — script exists, just needs `--replay 2880` rerun + compare REF totals.
- **Caveat**: now that H75 is live with `sma24>sma100` (different detector than tsmom_672h), the comparison anchor has shifted. Vol-scaled is now competing against H75's detector, not tsmom. Re-baseline before validating.

#### Item 2 — ETH derivatives features: re-run Mode D/HRST with NaN fix

- **Source**: CLAUDE.md "What's promising" line 95
- **Discovery date**: 2026-04-19
- **Status**: Added Binance derivatives ETH download (was BTC-only). LGBM importance: `deriv_funding_chg1d` ranked **#4 on 6h (4.2%)** / **#2 on 8h (3.7%)**, `deriv_oi_chg3d` **#5 on 6h (3.3%)**, `deriv_oi_chg1d` **#4 on 8h (3.6%)**. Higher than established features (`adx_14h`, `price_to_sma100h`). Mode D initially produced 0 valid candidates due to OI NaN (30-day history only); fixed by excluding sparse features from `dropna()` (LGBM handles NaN natively).
- **Next step**: Re-run Mode D or full HRST with the NaN fix to get actual production candidates.
- **Effort**: low — `python crypto_trading_system_ed.py HRST ETH 5,6,7,8h --1440 --nopersist` and check whether deriv_funding / deriv_oi appear in winner feature lists.
- **Status under H75**: the in-flight TODO 1805 HRST on fresh data (running now) will surface whether these derivatives win seats under the strict-(combo,w) + REFINE_TRIALS=75 architecture. **No separate action needed if 1805 includes them.** Verify in the post-run audit.

#### Item 3 — Per-regime feature set

- **Source**: CLAUDE.md "What's untested (queued)" line 99 + "Regime-conditional asymmetries" line 112
- **Discovery date**: open since at least 2026-04
- **Status**: Untested as a deliberate design choice. Asymmetry already happens organically (6h model: 7 features mostly PySR+technical; 8h model: 32 features kitchen sink) but never tested with bear-uses-macro / bull-uses-technical as an explicit constraint.
- **Next step**: Add a constrained Mode D variant where the bear-horizon feature pool is restricted to macro+sentiment+cross-asset, and the bull-horizon pool to technical+PySR. Compare Mode T REF to unconstrained baseline.
- **Effort**: low-medium — code change to Mode D's feature ranking pool. A few hours of work + 1 HRST run.
- **Decision criterion**: constrained variant must beat unconstrained baseline by ≥+5pp on Mode T REF to justify the added complexity.

#### Item 4 — Multi-horizon ensemble emergency-exit

- **Source**: CLAUDE.md "What's untested (queued)" line 100
- **Discovery date**: 2026-04-27 forensic (4th angle)
- **Status**: Untested. Idea: force exit when 5h AND 8h both flip SELL within 1h. Per-horizon signal cache `data/eth_per_horizon_signals_90d.pkl` already exists. Distinct from rejected 5-min price-action triggers and from T1b entry-side ensemble (tested + shelved 2026-04-27).
- **Next step**: Write `tools/test_multi_horizon_emergency_exit.py` that replays ETH 90d using cached per-horizon signals; flag every hour where 5h AND 8h are both SELL; compute return delta vs baseline single-horizon SELL.
- **Effort**: 1-2h.
- **Hypothesis**: should reduce max drawdown without sacrificing return — emergency exit only fires when two independent horizons agree, so signal is high-conviction.

#### Item 5 — On-chain derivatives: weak or correlation-crowded-out?

- **Source**: CLAUDE.md Engine Reference Card line 49 (the legacy `(TODO 1a)` reference)
- **Discovery date**: 2026-04-19 (first post-wiring HRS audit)
- **Status**: 38 on-chain derivatives (hashrate, active_addresses, tx_count, fees_native, exchange_inflow/outflow, plus chg/zscore/ratio variants) below selection threshold across all ETH horizons. Only `oc_mvrv_chg1d` (Grade 4, 66.7%) and `oc_exchange_netflow_chg5d` (Grade 3, 8h only) survived.
- **Open question**: are the 38 dead features genuinely weak signals, OR are they correlation-crowded-out by the kitchen-sink technical+macro pool? Distinction matters: if correlation-crowded, a feature set with fewer technical features might revive them.
- **Next step**: Run a "minimal-pool" Mode D variant — restrict feature pool to ONLY on-chain + 5-10 anchor technical features (hour_cos, price_to_sma100h, logret_120h). See if any of the 38 derivatives crack the top-10 LGBM importance in that smaller pool.
- **Effort**: medium — one custom Mode D run with feature pool override + analysis script.
- **Priority**: low — `oc_mvrv_chg1d` alone is already pulling its weight; unlikely the other 38 hide a bigger signal.

#### Triage summary

| Item | Effort | Priority | Trigger to action |
|---|---|---|---|
| 1. Vol-scaled horizons 4mo validation | low | medium | After TODO 1805 H75-on-fresh-data finishes (re-baseline) |
| 2. ETH derivatives Mode D rerun | low | high | **Likely auto-resolved by TODO 1805** — check winner feature lists post-run |
| 3. Per-regime feature set | low-medium | low | Quiet period (no other HRST in flight) |
| 4. Multi-horizon emergency exit | 1-2h | medium-high | Any time — small script, fast iteration |
| 5. On-chain correlation question | medium | low | Last on the list — `oc_mvrv_chg1d` is already valuable |

#### Cleanup intent for CLAUDE.md

These items remain in CLAUDE.md's Engine Reference Card as historical research notes (the card is supposed to be stable reference), BUT the inline `(TODO 1a)` and `(TODO 2)` parenthetical cross-references on lines 49-50 are stale (predate the 2026-05-18 split). Optional follow-up: replace those inline refs with `(see TODO 1805B Item 5)` and `(resolved — see TODO.md SOPR audit)` for accuracy. Not done in this migration to keep CLAUDE.md edits minimal — user can request cleanup if desired.

---

### 🟢 TODO 1805 — H75 HRST on fresh data (Desktop, 2026-05-18 late evening)

**Search anchor**: `TODO 1805`

**Status (2026-05-19 ~00:30 CEST)**: 🔄 **IN FLIGHT on Desktop**. Launched late evening 2026-05-18. Running concurrently with TODO 1805D (G_narrow_d on Desktop, isolated to `models_g_desktop/`) and TODO 1805C (G_narrow_d on Laptop, default `models/`). All three runs mutually isolated via env-var-driven output dirs — see TODO 1805D's contamination matrix for the full cross-machine ✅ none table.

**Why this exists**: H75 was promoted to live earlier tonight (~22:02 CEST) based on an HRST that ran against the `_reliability_hrst_snapshot_desktop_20260515_154801/` snapshot (data frozen May 15). The user wants a fresh-data re-run of the same architecture (H75 engine = strict (combo,w) dedup + REFINE_TRIALS=75 + K=default multi-seed) to validate that the May 15 snapshot didn't bias the per-horizon Mode V winners — and to get a more current OOS read before the first H75-driven live trades fire.

**Launch command (Desktop, run now)**:

```powershell
python .\crypto_trading_system_ed.py ETH 5,6,7,8h HRST --1440 --nopersist
```

CLI normalization handles `--1440 → --replay 1440` and `--nopersist → --no-persist` (engine lines 7015-7026). No trailing comma needed on `ETH` since the parser was merged with the asset-first ordering fix.

**Key differences vs the 2026-05-18 promotion HRST**:
- **Fresh data**: NO `--no-data-update` → engine downloads macro + onchain + derivatives + OHLCV at startup (~5-10 min), writes to live `data/macro_data/*.csv`. This is intentional — user wants the freshest snapshot.
- **No env-var overrides**: `V2_DATA_SNAPSHOT` not set → live data path used. `RELIABILITY_K` not set → default K (whatever the merged engine ships with).
- **`--no-persist` ON**: writes go to `crypto_ed_production_noprod.csv` + `regime_config_ed_noprod.json`. **Live trader untouched.** Manual promotion required if H75-on-fresh wins.

**Startup banners to confirm**:
- Data download phase: `MACRO & SENTIMENT DATA DOWNLOAD` lines, then `ONCHAIN DATA DOWNLOAD` etc. ~5-10 min
- `ED: Mode HRST | ETH | 5h,6h,7h,8h | 150 trials`
- `[H_STRICT_FAMILY] _diversity_key changed to (combo, w) — 1 V2 slot per (model_family, window) cluster` ← H75 engine active
- Mode V per horizon: `75 trials per config | gamma ±0.002 | features ±5 | window ±20h` ← 75-trial budget

**ETA**: ~20h on Desktop (5-10 min data download + 4× Mode D + 4× Mode V@75 + R + S + T+G). Done midday-to-late-afternoon 2026-05-19.

**Success criteria → comparison with promoted H75 (May 15 snapshot)**:

| Metric | Promoted H75 (May 15 snapshot) | H75 on fresh data | Decision |
|---|---|---|---|
| Mode T total | TBD (logged in tonight's HRST log) | pending | — |
| Per-horizon winners | 5h XGB+LGBM 100h γ=0.9993 7f / 8h RF+LGBM 162h γ=0.9954 6f | pending | If winners migrate substantially → snapshot was stale; consider re-promotion |
| Detector + thresholds | `sma24>sma100` bull=5h@75% / bear=8h@65% | pending | If detector changes → architectural instability, dig in |

| Outcome | Action |
|---|---|
| Fresh data ≈ snapshot (winners + Mode T within ±5pp) | Promoted H75 is robust to data freshness. No action. |
| Fresh data > snapshot by >5pp on Mode T | Snapshot was stale. Promote the fresh-data config (when trader flat). |
| Fresh data < snapshot by >5pp on Mode T | Snapshot may have lucky-window selection. Watch live H75 carefully — consider rollback to pre-H75 if live underperforms. |
| Per-horizon winners change combo/window family entirely | Architectural noise — flag and discuss with user before promoting either. |

**Open question for after the run**: should H75 HRSTs be scheduled on a fresh-data cadence (e.g. weekly/biweekly) instead of run-on-demand? The May 15 snapshot is 3 days old at promotion time — not catastrophic, but the +7pp sim-to-live gap on the prior LIVE config suggests model selection is sensitive to recent regime drift.

---

### 🟢 TODO 1706 — H_strict_family full HRST @ 75 trials on Desktop (2026-05-17 evening)

**Search anchor**: `TODO 1706`

**Why this exists**: TODO 1705 launches G_narrow_d full HRST on Laptop concurrently. Both runs share the same Drive-synced engine directory, which means they'd compete for writes to `models/crypto_ed_production_noprod.csv`, `config/regime_config_ed_noprod.json`, and `models/crypto_ed_best_models.csv` — corrupting both runs' Mode T results and risking production-CSV pollution. This TODO sets up H_strict_family @ 75 trials (per today's REFINE_TRIALS bump) with **fully isolated output directories** so G and H can run concurrently without infecting each other or production.

**Isolation mechanism (added 2026-05-17 to engine fork)**: `crypto_trading_system_ed_h_strict_family.py` now reads `H_STRICT_MODELS_DIR` and `H_STRICT_CONFIG_DIR` env vars and redirects ALL writes (RESUME_DIR, PRODUCTION_CSV, REGIME_CONFIG_PATH, MODELS_DIR, CONFIG_DIR) to the override dirs. Default behavior preserved when env vars are unset. Confirmed by startup banner: `[H_STRICT_FAMILY_ISO] output dirs redirected: models=models_h75 config=config_h75`.

#### Step 0 — Kill the in-flight H V 8h@50 first

The earlier V 8h-only run (started 18:37) is using the old REFINE_TRIALS=50 and writes to the default `models/` path. That collides with G's writes on Laptop. Kill it before launching H75:

```powershell
Get-Process python | Where-Object { $_.StartTime -gt (Get-Date '2026-05-17 18:30') } | Stop-Process
```

Sunk cost is ~3h of compute — acceptable since H75 is the proper apples-to-apples test vs G75 anyway.

#### Step 1 — Pre-flight: create isolated dirs and seed them

```powershell
cd "G:\Autres ordinateurs\My laptop\engine"

# Create isolated output dirs
New-Item -ItemType Directory -Path "models_h75" -Force | Out-Null
New-Item -ItemType Directory -Path "config_h75" -Force | Out-Null

# Seed production_noprod.csv source (—no-persist copies production.csv at startup;
# without this, H75 would have nothing to seed from in models_h75/)
Copy-Item "models\crypto_ed_production.csv" "models_h75\crypto_ed_production.csv"
Copy-Item "config\regime_config_ed.json" "config_h75\regime_config_ed.json"

# Copy H_strict Mode D grid CSVs into the isolated dir (so --skip recognizes them)
Copy-Item "models\crypto_ed_grid_ETH_*_H_STRICT_FAMILY.csv" "models_h75\"

# Copy current best_models.csv (some code paths read it for candidate lookup)
Copy-Item "models\crypto_ed_best_models.csv" "models_h75\crypto_ed_best_models.csv"

# Verify
Get-ChildItem "models_h75" | Format-Table Name, Length, LastWriteTime
```

Expected files in `models_h75/`:
- `crypto_ed_production.csv` (seed for --no-persist)
- `crypto_ed_best_models.csv` (read by Mode V for candidate context)
- `crypto_ed_grid_ETH_5h_H_STRICT_FAMILY.csv` ... 8h (4 grid CSVs for --skip)

#### Step 2 — Launch H_strict_family full HRST @ 75 trials, isolated outputs
...
- **`"ETH,"` with TRAILING COMMA, double-quoted** — `ETH` alone matches `endswith('h')` parser → all 9 assets get loaded
- **`--no-data-update` REQUIRED** — without it engine downloads + writes to LIVE `data/macro_data/` (snapshot only redirects READS)
- **`--no-persist` REQUIRED** — routes writes to `*_noprod` versions in the isolated dirs
- **`--skip` REQUIRED** — skips Mode D for 5h/6h/7h/8h (all 4 grid CSVs exist post-Step-1); without `--skip`, Mode D re-runs (~40 min wasted)
- **Both `H_STRICT_MODELS_DIR` and `H_STRICT_CONFIG_DIR` env vars MUST be set** — otherwise H writes to default `models/` and `config/`, colliding with G's writes on Laptop. **No banner about "ISO" at startup = abort and re-export env vars**.
- **REFINE_TRIALS=75 is baked in** — verify via startup line `75 trials per config | gamma ±0.002 | features ±5 | window ±20h`. If you see `50 trials`, the file edit didn't propagate — pull latest from git.

#### Startup banners to confirm (FOUR are isolation-critical)

- `[H_STRICT_FAMILY_SNAPSHOT] pd.read_csv redirected: ...` ← snapshot redirect ON
- `[H_STRICT_FAMILY_K5] _deku_eval_with_pruning patched (K=5 seeds=[42, 43, 44, 45, 46])` ← K=5 active
- **`[H_STRICT_FAMILY_ISO] output dirs redirected: models=models_h75 config=config_h75`** ← **isolation active — abort if missing**
- `[H_STRICT_FAMILY] _diversity_key changed to (combo, w) — 1 V2 slot per (model_family, window) cluster` ← dedup active
- `[--no-data-update] Skipping macro + OHLCV downloads. Using whatever is currently on disk.` ← downloads off
- `ED: Mode HRST | ETH | 5h,6h,7h,8h | 150 trials` ← all 4 horizons, HRST mode
- Per horizon: `Mode D results already exist for ETH 5h — skipping D (--skip)` etc.
- Mode V per horizon: `75 trials per config | gamma ±0.002 | features ±5 | window ±20h` ← 75-trial budget active

#### ETA (Desktop)

- Mode D: skipped for all 4 horizons (--skip + grids exist) → 0 min
- Mode V × 4 horizons @ K=5 × 75 trials (Desktop ~4.5h/horizon based on H@50 actual × 1.5×): ~**18h**
- Mode R + S + T+G: ~**1.5h**
- **Total: ~19-20h. Done midday-to-early-afternoon 2026-05-18.**

#### Step 3 — After HRST completes, snapshot final results

```powershell
$ts = Get-Date -Format "yyyyMMdd_HHmm"
$dest = "models\_archive_h_strict_75_COMPLETE_$ts"
New-Item -ItemType Directory -Path $dest -Force | Out-Null
Copy-Item "models_h75\*" $dest -Recurse
Copy-Item "config_h75\*" $dest -Recurse
Copy-Item "logs\ed_v1_$(Get-Date -Format yyyyMMdd)_*.log" $dest -ErrorAction SilentlyContinue
```

Then `models_h75/` and `config_h75/` can be deleted to keep the workspace clean.

#### Success criteria → next step (3-way verdict)

Once both TODO 1705 (G HRST) and TODO 1706 (H75 HRST) complete, fill this table:

| Variant | Mode T total | vs B (+89.41%) | Decision |
|---|---|---|---|
| B (production) | +89.41% | — | baseline |
| G_narrow_d | TBD | TBD | TODO 1705 outcome |
| H_strict @ 75 | TBD | TBD | TODO 1706 outcome |

| Winner | Action |
|---|---|
| G > H > B (both beat prod) | Architecturally, G's wider Optuna ranges + force-combo are the dominant levers. Promote G when trader flat. |
| H > G > B (both beat prod) | H's strict `(combo, window)` dedup is the dominant lever. Promote H when trader flat. Simpler code, smaller engine delta. |
| B > G and B > H | Both architectural changes fail at the Mode T level despite per-horizon Mode V wins. SHELVE both. Production B's lottery-prone 7h is still the global optimum on the full regime-switching chain. |
| One of G/H ≈ B (±5pp) and the other below | The above-B variant is the candidate; the other shelves. Promote when trader flat. |
| G > H but H still > B | G's win likely from wider Optuna catching long-window winners (e.g., 281h for 5h horizon). H's smaller architectural delta is the "safer" production change. **Discuss with user** — code-simplicity vs alpha trade-off. |
