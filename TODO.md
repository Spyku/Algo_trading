...
| ✅ FIXED 2026-05-31 | **Bug #17 — refine worker pool halved when n_cfgs < 3** (root cause found + fixed in 1 commit) | — | ✅ FIXED via formula change. **Root cause** (NOT what I first suspected): default workers formula was `trial_split * max(1, n_cfgs - 1)`. For SPLIT=3 and n_cfgs=2 (8h Mode D only produced 2 ranked candidates instead of 6), this returned 3 instead of 6. I had hypothesized a Windows kernel handle leak across pool boundaries — that was wrong. Evidence: `crypto_ed_best_models.csv` row count showed 6h=10 ranked candidates, 7h=6, 8h=2; top3_for_refine slice produced n_cfgs=3 / 3 / 2 respectively; my formula gave 6/6/3 workers. **Fix**: change to `min(trial_split * n_cfgs, 6)` — caps at 6 (28-core sweet spot) but no longer subtracts 1. New behavior: n_cfgs=3 → 6 workers (unchanged), n_cfgs=2 → 6 workers (was 3), n_cfgs=1 → caller skips to serial path. **Separate question** (worth a fast follow-up): why did 8h Mode D produce only 2 ranked candidates while 6h produced 10? Either market characteristic (low-quality 8h grid hits) or a Mode D filter quirk. Not blocking. |
