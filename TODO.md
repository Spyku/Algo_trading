...
> ✅ **Follow-up DONE 2026-07-01:** `validate_backtest_vs_live.py` + sanity [5] now filter **recent-only** — boundary = prod-CSV mtime (= last promotion, timezone-correct UTC epoch); pre-boundary hours excluded (logged under a prior model); thin post-promote sample → **WARMING-UP = PASS** (no thin-sample false FAIL); `--all-hours` overrides for cross-config forensics. **Validated:** the 07-01 daily false-FAIL (4h 119/200 = **59.5%**) → **39/39 = 100.0% PASS**, 161 pre-boundary hrs excluded. This matters because **[5] drives the daily verdict** ([sanity_check.py:284](tools/sanity_check.py#L284) `bad` set) — the straddle was flipping the whole sanity to ATTENTION. NB the leak fix itself was already VERIFIED (post-fix backtest==live 15/15 = 100%).
