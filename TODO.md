...
| 🔥 **P0** | **NEAR_LIVE_MODE HRST fork stack — running on Desktop** | **Desktop** (~6-8h, parallel) | 🟢 RUNNING (launched 2026-05-27 21:49) — 3 new sandbox files shipped commits `0dce8af` + `373f463` + `8efb37c`. Tests realistic backtest with `na_policy=mean_last_10` (NEW policy, fills NaN with mean-of-last-10), `step=1` (every hour), `signal=ternary` BUY/SELL/HOLD, `embargo=horizon` (kept per user). Goal: produce a Mode T REF that actually predicts live performance. Files: `crypto_signal_core_nearlive.py`, `crypto_trading_system_ed_step6_nearlive.py`, `crypto_trading_system_ed_g_narrow_d_parallel_nearlive.py`. Snapshot env unset → reads live data; `NEAR_LIVE_MODE=1`; output dirs `models_g_desktop_nearlive/`. **No promotions allowed regardless of result** — this is diagnostic. |
