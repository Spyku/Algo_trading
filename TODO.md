...
## 📊 At-a-glance — active TODO dashboard (2026-05-30 — post-FAYE consolidation)

| Pri | Item | Where | Status |
|---|---|---|---|
| 📌 | **G_narrow LIVE** (CSV+config since 2026-05-21 21:56) on H75 engine + macro_cache mtime fix (2026-05-27 11:22) — `sma24>sma100` / bull 5h@65% / bear 8h@65%, shields OFF, **gates ON** (reverted to Mode T optimal 2026-05-27 13:18) | **Desktop** (always) | running |
| 🔥 **P0 (TODAY)** | **v3 HRST on Desktop** (5h done, 6h running, 7h+8h pending) — running `crypto_trading_system_ed_g_narrow_d_parallel_nearlive_v3.py HRST ETH 5h,6h,7h,8h --replay 1440 --no-persist --no-data-update` started 2026-05-29 19:10. 5h winner: ETH RF+LGBM w=200 γ=0.999 10f Grid +49.56%. | **Desktop** (~17h remaining) | 🟢 RUNNING — 6h in Mode D grid 8/60 (last check). ETA ~18:00 May 30 ±1.5h (each horizon ~5h45m; refine dominates 4h/5h45m). **CHECK PERIODICALLY**: when Mode T REF prints for all 4 horizons, the run is done. **No promotions during run.** |
| 🔥 **P0 (TODAY/EVE)** | **Promote v3 winners + restart trader** when HRST done AND trader is flat (Critical Rule 19) | **Desktop** (10 min) | 📅 NEXT — recipe: `Copy-Item models_g_desktop_nearlive\crypto_ed_production.csv models\crypto_ed_production.csv -Force; Copy-Item config_g_desktop_nearlive\regime_config_ed.json config\regime_config_ed.json -Force` then restart trader. Trader hot-reloads within 5 min. Target restart time: ~18:00-20:00 May 30 (after final 8h Mode T REF prints). |
| 🔥 **P0 (THIS WEEK)** | **First FAYE HRST** to validate the consolidated codepath produces results equivalent to v3 on the same data | **Desktop or Laptop** (~17-30h) | 📅 NEXT after v3 promo — `python crypto_trading_system_faye.py HRST ETH 5h,6h,7h,8h --replay 1440 --no-data-update`. Writes to `models_faye/` + `config_faye/` (isolated from prod). Compare Mode T REF + per-horizon winners against v3's output (same May data). If equivalent ±2pp → FAYE replaces v3 as the canonical engine path. **No FAYE-to-prod promotion until this validates.** |
...
| 🚀 P3 (last) | **Validate FAYE bug #16 perf patch** (trial chunking + early stop + 6-worker pool, commits `9f286f8` + `3b2426b`, defaults ON 2026-05-31) | **Desktop or Laptop** (next H run as natural test) | 📅 PENDING VALIDATION — patch went straight to defaults without DV/HRST equivalence test against legacy 3-worker / no-chunk / no-early-stop. Three independent sources of potential divergence: (a) chunked TPE with different seeds explores HP space differently than single 75-trial study — best-across-chunks might miss the global optimum; (b) early stop at patience=15 might fire too soon and prune real improvements; (c) 6 workers × 5 K=5 threads = 30 threads on 28 cores might add per-iter latency that didn't exist in 15-thread runs. **Test plan**: when next clean H run starts (after current 6h,7h,8h --replay 1440 finishes), run `$env:FAYE_REFINE_TRIAL_SPLIT=1; $env:FAYE_REFINE_EARLYSTOP_PATIENCE=0` legacy run on the SAME data, diff Mode V Step 3 winners against new defaults. Acceptance: refined winners within ±2pp return on Step 3, same combo/window/feature pick. If divergent, debug (likely early-stop too aggressive — try patience=30) or set defaults back to legacy. **Until validated, treat refined picks as suspect**. Optimization further (queue items): re-dispatch unused workers on a TPE-shared study via RDB storage so chunks share TPE history (recovers sample efficiency); make WORKERS auto-scale by `cpu_count()` not hardcoded sweet-spot for 28-core. |
| 🔥 P2 | **Bug #17 — Pool spawn capacity degrades across horizons** (discovered 2026-05-31 ~10:50 during H ETH 6h,7h,8h --replay 1440 run) | **Laptop** (~2-4h investigation) | 📅 NEXT — During the H run, file evidence shows: 6h refine spawned **6 workers** at 02:25:37 (correct). 7h refine spawned **6 workers** at 05:46:26 (correct). 8h refine spawned **only 3 workers** at 09:06:40 (~50% capacity loss). Banner consistently printed `pool=6` for all three. Effect: 8h refine takes ~2× as long as designed (~4.5h actual vs ~2h with full 6 workers). What happens between 7h end (08:03) and 8h start (09:06): 7h Step 3 backtest pool (`_run_parallel_backtests`) + 8h Mode D 8-worker grid pool + 8h Step 1 6-worker backtest pool. **Hypothesis**: ProcessPoolExecutor cleanup is incomplete on Windows; named-pipe handles or kernel objects leak across pool boundaries. After enough pool creation/destruction cycles, new pools can only spawn a subset. **Investigation steps**: (1) Add `gc.collect()` + log `multiprocessing.active_children()` between phases — see if zombies accumulate. (2) Reproduce on a fresh process by running 3 consecutive `ProcessPoolExecutor(max_workers=6)` cycles, each submitting 9 tasks, with `_hard_shutdown_pool` between — verify pool capacity degrades. (3) Test workaround: `time.sleep(5)` before creating new pool, gives OS time to reclaim handles. (4) Test workaround: pre-create ONE refine pool at startup, reuse for all 3 horizons. (5) Check loky resource_tracker (we see PID 35156 in the run) — possibly holding stale handles. **Why P2 not P3**: this is a 2× speedup leak in a path we just shipped to defaults. Real cost: ~2.5h extra per H run today, will compound on future runs. Bisect: did the legacy 3-worker dispatcher have this issue too? If yes, it was hidden because legacy only used 3 workers anyway. Bug only became visible after bug #16's 6-worker default. |
