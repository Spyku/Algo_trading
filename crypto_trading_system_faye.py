...
Outputs (all in FAYE-isolated directories — does NOT touch Ed's production files):
  ...
  models_faye/eth_models_<H>h.pkl           (trained models, ed-pickle-compatible)
  models_faye/mode_d_full_<asset>_<H>h_<ts>.csv (per-eval CSV w/ K=5 seed breakdown)
  config_faye/regime_config_faye.json       (per-asset: detector, bull/bear horizon+conf+shield)
...
RESUME_DIR = f'{FAYE_MODELS_DIR}/.resume_hourly'
...
def run_mode_s_serial(assets_list, horizons, args=None):
    ...
    Flow:
        ...
        6. Write winning detector + horizons + confidences to regime_config_faye.json
    """
...
def run_mode_t_serial(assets_list, args=None):
    ...
    Saves winner to regime_config_faye.json:
...
def _build_regime_indicators_and_detectors(asset):
    ...
    # named detector found in regime_config_faye.json (back-compat for legacy
...
def _apply_mode_r_to_config(r_results):
    """Write Mode R's winning horizons to regime_config_faye.json so Mode S picks them up."""
    if not r_results:
        return
    tcfg_path = REGIME_CONFIG_PATH
    try:
...
def _sweep_rally_cooldown(asset, signals, asset_cfg, replay_h, rank='recent',
    ...
    Writes winner to regime_config_faye.json if write_config=True:
...
""")
    ...
    if run_all_metrics and mode == 'D':
        ...
        for metric in sorted(VALID_METRICS):
            ...
            tcfg_path = f'{CONFIG_DIR}/regime_config_faye{metric_suffix}.json'
            csv_path = _get_models_csv_path()
            try:
                with open(tcfg_path) as f:
                    tcfg = json.load(f)
...
def run_mode_t(assets_list, args=None):
    ...
    # regime_config_faye.json. Replicate that prediction here.
    try:
        with open(REGIME_CONFIG_PATH) as f:
