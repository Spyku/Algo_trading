"""
Crypto 15-Min ML Trading System — V15 Cacarot
============================================================
Based on V5.5. Uses 15-minute candles from Binance.
Horizons 1-8 = 15', 30', 45', 60', 75', 90', 105', 120'.
...
CLI Usage:
  python crypto_trading_system_v15.py D ETH 1,2,3,4,5,6,7,8h 1y
  python crypto_trading_system_v15.py D BTC 4,8h 1y
  python crypto_trading_system_v15.py F BTC 4,8h
  python crypto_trading_system_v15.py F BTC
  python crypto_trading_system_v15.py G BTC

Outputs:
  charts/{ASSET}_15m_backtest.png
  models/crypto_15m_best_models.csv
  models/crypto_15m_chart_data.json
  config/trading_config_15m.json  (updated by Mode F)
"""
...
AVAILABLE_HORIZONS = [1, 2, 3, 4, 5, 6, 7, 8]  # 1-8 candles = 15'-120'
...
def generate_signals(asset_name, model_names, window_size, replay_hours=REPLAY_HOURS,
    ...
    print(f"  Generated {len(signals)} 15-min signals for {asset_name}")
    return signals
...
def generate_backtest_chart(asset_name, signals, model_info=None):
    ...
    Saves to {asset}_15m_backtest.png and opens it.
...
def run_mode_b(assets_list, horizon_filter=None, skip_data_update=False):
    ...
    if horizon_filter is None:
        ...
        for asset_name in assets_to_run:
            try:
                with open(f'{CONFIG_DIR}/trading_config_15m.json') as _f:
                    _tcfg = json.load(_f)
            ...
            for _, row in asset_rows.iterrows():
                ...
                sigs = generate_signals(asset_name, model_names, window, _hours_to_rows(720),
                                        feature_override=feature_override, horizon=h, gamma=html_gamma)
...
def run_mode_d(assets_list, diag_years=1, horizon=PREDICTION_HORIZON, permtest=False, resume=False):
    ...
    for asset_name in assets_list:
        ...
        if total_rows > MAX_DIAG_ROWS:
            ...
            print(f"  Trimmed: {total_rows:,} -> {len(df_full):,} rows (last ~45 days)")
...
def run_strategy_comparison(assets_list, horizons=None):
    ...
    tcfg_path = f'{CONFIG_DIR}/trading_config_15m.json'
    try:
    ...
    for asset in assets_list:
        ...
        print(f"\n  >>> Updated trading_config_15m.json: {asset} -> strategy={best_strat}, min_confidence={best_conf_final}%")
        ...
        for h in available_horizons:
            ...
            try:
                sigs = generate_signals(asset, r['models'].split('+'),
                                        ...
                                        feature_override=feats, horizon=int(h), gamma=row_gamma)
                sigs = simulate_portfolio(sigs)
...
def _run_quick_asset(asset):
    ...
    # Determine strategy
    # Read strategy from trading_config_15m.json, fall back to both_agree
    try:
        with open(f'{CONFIG_DIR}/trading_config_15m.json') as _f:
            _tcfg = json.load(_f)
    ...
    for h in [4, 8]:
        ...
        qa_gamma = float(r.get('gamma', 1.0)) if pd.notna(r.get('gamma', 1.0)) else 1.0
        ...
        sigs = generate_signals(asset, model_names, window, _hours_to_rows(720),
                                feature_override=feature_override, horizon=h, gamma=qa_gamma)
        sigs = simulate_portfolio(sigs)
        if h == 4:
            signals_4h = sigs
        else:
...
def main():
    ...
    # ================================================================
    # CLI SHORTCUT: python crypto_trading_system_v15.py D BTC 4,8s 1y
    # Supports: MODE [ASSETS] [STEPS] [YEARS]
    # Steps are candles: s1=15', s2=30', s4=1h, s8=2h
    # ================================================================
    ...
    if cli_args and cli_args[0].upper() in ('B', 'D', 'DF', 'E', 'F', 'G', '5', '6', '7'):
        ...
        print("=" * 60)
        print("  CRYPTO 15-MIN ML TRADING SYSTEM -- V15 Cacarot")
        print("  Assets: BTC, ETH, XRP, DOGE")
        print(f"  Steps: {', '.join(_horizon_label(h) for h in AVAILABLE_HORIZONS)} (1 step = 15 min)")
