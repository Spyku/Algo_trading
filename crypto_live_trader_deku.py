"""
Crypto Live Trader (Deku) — Signal Generation for Deku Models
=============================================================
Same as crypto_live_trader.py but imports from crypto_trading_system_deku
and reads models/crypto_deku_best_models.csv.

Usage:
  python crypto_live_trader_deku.py                  # Run once (BTC default)
  python crypto_live_trader_deku.py --loop           # Run every hour
  python crypto_live_trader_deku.py --loop --asset BTC
  python crypto_live_trader_deku.py --setup          # Setup Telegram
"""
...
# LOAD MODEL CONFIG
# ============================================================
MODELS_CSV = 'models/crypto_deku_best_models.csv'
...
def run_once(asset_name, _stale_warning=False):
    print(f"\n{'='*60}")
    print(f"  LIVE SIGNAL [DEKU]: {asset_name}")
...
def run_loop(asset_name):
    print(f"\n{'='*60}")
    print(f"  LIVE TRADER [DEKU]: {asset_name}")
    ...
    send_telegram(f"<b>[DEKU] Live Trader Started</b>\n\nAsset: {asset_name}\nMin conf: {MIN_CONFIDENCE}%\nTime: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    while True:
        try:
            ...
            print("\n  Stopped.")
            send_telegram(f"<b>[DEKU] Stopped</b>\nCycles: {cycle}")
            break
        except Exception as e:
            print(f"\n  ERROR: {e}")
            send_telegram(f"<b>[DEKU] Error</b>\n<code>{e}</code>")
            time.sleep(120)
...
def setup_telegram():
    ...
    send_telegram("<b>[DEKU] Telegram connected!</b>")
