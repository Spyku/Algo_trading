"""
Crypto Hourly Trading System V5.1
============================================================
ML trading system for BTC, ETH, XRP with 1h, 2h, 4h and 8h horizons.
125 features -> walk-forward ML -> BUY/SELL/HOLD signals.

Modes:
  B. Quick run (saved models)    D. Full pipeline (feature analysis -> diagnostic)
  E. Iterative refinement        F. Strategy comparison   5/6/7. Quick BTC/ETH/XRP

CLI Usage (skip all menus):
  python crypto_trading_system.py B BTC 4,8h          # Mode B, BTC, 4h+8h
  python crypto_trading_system.py B BTC 1,2,4,8h      # Mode B, BTC, all horizons
  python crypto_trading_system.py D ETH 8h 1y         # Mode D, ETH, 8h, 1 year
  python crypto_trading_system.py D BTC 1,2h 1y       # Mode D, BTC, 1h+2h (short windows)
  python crypto_trading_system.py D BTC,ETH 4,8h 2y   # Mode D, multiple assets

Strategies (Mode F selects best):
  both_agree  : BUY when 4h AND 8h agree
  either_agree: BUY when 4h OR 8h fires
  8h_and_1h   : BUY when 8h AND 1h agree
  8h_and_2h   : BUY when 8h AND 2h agree
  any_agree   : BUY when ANY horizon fires
...
AVAILABLE_HORIZONS = [1, 2, 4, 8]  # 1h, 2h, 4h, 8h models
...
REPLAY_HOURS = 200
DIAG_STEP = 72
DIAG_WINDOWS       = [48, 72, 100, 150, 200]   # windows for 4h/8h horizons
DIAG_WINDOWS_SHORT = [24, 48, 72, 100, 150]   # windows for 1h/2h horizons (shorter = better fit)
...
# CHART DATA EXPORT
# ============================================================
def _compute_strategy_signal(sigs, strategy, min_conf):
    """
    Central strategy logic. sigs = dict {horizon: {'signal', 'confidence'} or None}.
    Handles all strategies including 1h/2h variants.
    Returns (signal, confidence).

    Strategies:
      both_agree  : BUY when 4h AND 8h agree, SELL when either says SELL
      either_agree: BUY when 4h OR 8h fires,  SELL when either says SELL
      4h_only     : use 4h signal only
      8h_only     : use 8h signal only
      1h_only     : use 1h signal only
      2h_only     : use 2h signal only
      8h_and_1h   : BUY when 8h AND 1h agree, SELL when either says SELL
      8h_and_2h   : BUY when 8h AND 2h agree, SELL when either says SELL
      any_agree   : BUY when ANY horizon fires, SELL when any says SELL
    """
    def _s(h): return sigs.get(h, {}).get('signal', 'HOLD') if sigs.get(h) else 'HOLD'
    def _c(h): return sigs.get(h, {}).get('confidence', 50) if sigs.get(h) else 50

    # --- single horizon strategies ---
    if strategy in ('4h_only', '8h_only', '1h_only', '2h_only'):
        h = int(strategy.split('h')[0])
        s, c = _s(h), _c(h)
        if s == 'SELL': return 'SELL', c
        if s == 'BUY' and c >= min_conf: return 'BUY', c
        return 'HOLD', c

    # --- multi-horizon strategies ---
    # Get relevant horizons per strategy
    if strategy == 'both_agree':
        ...
        horizons = [h for h in [1, 2, 4, 8] if sigs.get(h)]
    else:
        horizons = [4, 8]

    # SELL: any relevant horizon says SELL
    if any(_s(h) == 'SELL' for h in horizons):
        sell_confs = [_c(h) for h in horizons if _s(h) == 'SELL']
        return 'SELL', max(sell_confs)

    # BUY logic
    if strategy in ('both_agree', '8h_and_1h', '8h_and_2h'):
        # AND: all horizons must BUY with conf >= min_conf
        if all(_s(h) == 'BUY' and _c(h) >= min_conf for h in horizons):
            return 'BUY', min(_c(h) for h in horizons)
    elif strategy in ('either_agree', 'any_agree'):
        # OR: any horizon BUYs with conf >= min_conf
        for h in horizons:
            if _s(h) == 'BUY' and _c(h) >= min_conf:
                return 'BUY', _c(h)

    return 'HOLD', max((_c(h) for h in horizons), default=50)


def generate_strategy_html(asset_name, signals_by_horizon, strategy='both_agree'):
    """
    Generate interactive HTML charts with one panel per horizon + combined + portfolio.
    signals_by_horizon: dict {1: [...], 2: [...], 4: [...], 8: [...]}
    Backward compat: also accepts (asset, signals_4h, signals_8h, strategy) positional call.
    """
    os.makedirs(CHARTS_DIR, exist_ok=True)

    # Normalise legacy call: generate_strategy_html(asset, sigs4h, sigs8h, strategy)
    if not isinstance(signals_by_horizon, dict):
        signals_by_horizon = {4: signals_by_horizon, 8: strategy}
        strategy = 'both_agree'

    sig_maps = {h: {s["datetime"]: s for s in sigs} for h, sigs in signals_by_horizon.items() if sigs}
    all_times = sorted(set(dt for m in sig_maps.values() for dt in m))
    active_horizons = sorted(sig_maps.keys())
    ...
    merged = []
    for dt in all_times:
        price = next((sig_maps[h][dt]["close"] for h in active_horizons if dt in sig_maps[h]), None)
        if price is None:
            continue
        sigs_at_dt = {h: sig_maps[h].get(dt) for h in active_horizons}
        combined, _ = _compute_strategy_signal(sigs_at_dt, strategy, MIN_CONFIDENCE)
        row = {"datetime": dt, "close": price, "combined": combined}
        for h in active_horizons:
            s = sigs_at_dt.get(h)
            row[f"sig{h}"] = s["signal"] if s else "HOLD"
            row[f"conf{h}"] = s["confidence"] if s else 50
        merged.append(row)

    if not merged:
        print("  No merged signals to chart.")
        return

    # Overall stats (full window)
    o_cash, o_held, o_in, o_entry, o_trades, o_wins = 1000.0, 0.0, False, 0, 0, 0
    o_start_px = merged[0]["close"]
    for m in merged:
        price = m["close"]
        if m["combined"] == "BUY" and not o_in:
            ...
            o_held = 0; o_in = False
    if o_in and merged:
        o_cash = o_held * merged[-1]["close"] * (1 - TRADING_FEE)
    o_strat_ret = (o_cash / 1000.0 - 1) * 100
    o_bh_ret = (merged[-1]["close"] / o_start_px - 1) * 100
    ...
    for label, hours in [("1month", 720), ("1week", 168)]:
        ...
        def _markers(data, sig_key, conf_key):
            bx, by, bt, sx, sy, st = [], [], [], [], [], []
            for d in data:
                if d.get(sig_key) == "BUY":
                    bx.append(d["datetime"]); by.append(d["close"]); bt.append(f"BUY {d.get(conf_key,0):.0f}%")
                elif d.get(sig_key) == "SELL":
                    sx.append(d["datetime"]); sy.append(d["close"]); st.append(f"SELL {d.get(conf_key,0):.0f}%")
            return bx, by, bt, sx, sy, st

        # Per-horizon chart JS blocks
        horizon_charts_js = ""
        horizon_div_ids   = []
        for h in active_horizons:
            did = f"c{h}h"
            horizon_div_ids.append(did)
            bx, by, bt, sx, sy, st = _markers(data, f"sig{h}", f"conf{h}")
            horizon_charts_js += f"""
Plotly.newPlot('{did}',[pl,
  {{x:{json.dumps(bx)},y:{json.dumps(by)},mode:'markers',name:'{h}h BUY',text:{json.dumps(bt)},marker:{{color:blue,symbol:'triangle-up',size:8}}}},
  {{x:{json.dumps(sx)},y:{json.dumps(sy)},mode:'markers',name:'{h}h SELL',text:{json.dumps(st)},marker:{{color:red,symbol:'triangle-down',size:8}}}}
],ml('{h}h Model',200),{{responsive:true}});
"""
        horizon_divs_html = "\n".join(f'<div id="c{h}h" class="chart"></div>' for h in active_horizons)

        cbx, cby, cbt, csx, csy, cst = _markers(data, "combined", f"conf{active_horizons[0]}")
        cbt = ["STRATEGY BUY"] * len(cbx)
        cst = ["STRATEGY SELL"] * len(csx)

        all_div_ids = horizon_div_ids + ["cComb", "cPort"]
        sync_ids_js = json.dumps(all_div_ids)
        horizons_label = ", ".join(f"{h}h" for h in active_horizons)

        strat_cls  = "blu" if strat_ret > 0 else "red"
        bh_cls     = "blu" if bh_ret > 0 else "red"
        alpha_cls  = "blu" if alpha > 0 else "red"
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .stats {{ display: flex; gap: 24px; padding: 12px 24px; border-bottom: 1px solid #1a1f2e; flex-wrap: wrap; }}
  .stat {{ text-align: center; min-width: 70px; }}
  .stat .val {{ font-size: 20px; font-weight: 700; }}
  .stat .lbl {{ font-size: 10px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; }}
  .blu {{ color: #3b82f6; }} .red {{ color: #ef4444; }} .chart {{ width: 100%; }}
</style>
</head>
<body>
<div class="header">
  <div>
    ...
    <div class="sub">Strategy: {strategy} | Min conf: {MIN_CONFIDENCE}% | 0.09% fees | Horizons: {horizons_label}</div>
  </div>
  <div style="text-align:right"><div class="sub">${{prices[-1]:{price_fmt}}} | {{dates[-1]}}</div></div>
</div>
<div class="stats">
  <div class="stat"><div class="val {strat_cls}">{strat_ret:+.1f}%</div><div class="lbl">Strategy</div></div>
  <div class="stat"><div class="val {bh_cls}">{bh_ret:+.1f}%</div><div class="lbl">Buy & Hold</div></div>
  <div class="stat"><div class="val {alpha_cls}">{alpha:+.1f}%</div><div class="lbl">Alpha</div></div>
...
syncIds.forEach(function(id){{
  document.getElementById(id).on('plotly_relayout',function(ed){{
    if(ed['xaxis.range[0]']&&ed['xaxis.range[1]']){{
      ...
      syncIds.forEach(function(o){{if(o!==id)Plotly.relayout(o,{{'xaxis.range':r}})}});
    }}
    if(ed['xaxis.autorange']){{syncIds.forEach(function(o){{if(o!==id)Plotly.relayout(o,{{'xaxis.autorange':true}})}});}}
  }});
}});
</script>
</body>
...
def generate_signal_table_html(asset_name, signals_by_horizon, strategy='both_agree'):
    """Generate interactive HTML table: price, price+1h, delta, signals, strategy, correct."""
    os.makedirs(CHARTS_DIR, exist_ok=True)

    # Normalise legacy call
    if not isinstance(signals_by_horizon, dict):
        signals_by_horizon = {4: signals_by_horizon, 8: strategy}
        strategy = 'both_agree'
    ...
    for i, dt in enumerate(all_times):
        ...
        if i + 1 < len(all_times):
            ...
            next_s = next((sig_maps[h].get(next_dt) for h in active_horizons if sig_maps[h].get(next_dt)), None)
        ...
        row = {'dt': dt, 'price': price, 'price_next': price_next, 'delta': delta, 'combined': combined}
        for h in active_horizons:
            s = sigs_at_dt.get(h)
            row[f'sig{h}'] = s['signal'] if s else 'N/A'
            row[f'conf{h}'] = s['confidence'] if s else 0
    ...
    # Build horizon header columns
    horizon_headers = ''.join(f'<th>{h}h Sig</th><th>{h}h Conf</th>' for h in active_horizons)
    # Build table rows
    table_rows = ""
    for r in rows:
        def _sc(sig):
        ...
        horizon_cells = ''.join(
            f'<td class="{_sc(r[f"sig{h}"])}">{r[f"sig{h}"]}</td><td>{r[f"conf{h}"]:.0f}%</td>'
            for h in active_horizons
        )
        table_rows += f"""<tr>
  <td>{r['dt']}</td>
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .stats {{ padding: 10px 24px; display: flex; gap: 30px; border-bottom: 1px solid #1a1f2e; flex-wrap: wrap; }}
  .stat .val {{ font-size: 18px; font-weight: 700; }}
  .stat .lbl {{ font-size: 10px; color: #6b7280; text-transform: uppercase; }}
  .blu {{ color: #3b82f6; }} .red {{ color: #ef4444; }}
  ...
  .buy {{ color: #3b82f6; }} .sell {{ color: #ef4444; }} .hold {{ color: #eab308; }}
...
<div class="header">
  <div>
    ...
    <div class="sub">Strategy: {strategy} | Min confidence: {MIN_CONFIDENCE}% | Horizons: {horizons_label}</div>
  </div>
  <div style="text-align:right"><div class="sub">{rows[-1]['dt']}</div></div>
</div>
<div class="stats">
  <div class="stat"><div class="val {'blu' if accuracy >= 55 else 'red'}">{accuracy:.0f}%</div><div class="lbl">Signal Acc</div></div>
  <div class="stat"><div class="val">{len(actionable)}</div><div class="lbl">Signals</div></div>
  <div class="stat"><div class="val blu">{correct_buys}/{len(buy_calls)}</div><div class="lbl">Buy Correct</div></div>
  <div class="stat"><div class="val red">{correct_sells}/{len(sell_calls)}</div><div class="lbl">Sell Correct</div></div>
</div>
<div class="filter">
...
<thead><tr>
  <th>Time</th><th>Price</th><th>Price+1h</th><th>Δ%</th>
...
function filterRows(f,btn){{
  ...
  document.querySelectorAll('#tbl tbody tr').forEach(r=>{{
    var s=r.cells[4].textContent.trim();
    r.style.display=(f==='all'||s===f)?'':'none';
  }});
}}
</script>
</body>
        ...
        for asset_name in assets_to_run:
            ...
            for _, row in asset_rows.iterrows():
                ...
                sigs = generate_signals(asset_name, model_names, window, 720,
                                        feature_override=feature_override, horizon=h)
                signals_by_horizon[h] = simulate_portfolio(sigs)

            if signals_by_horizon:
                generate_strategy_html(asset_name, signals_by_horizon, strategy=strategy)
                generate_signal_table_html(asset_name, signals_by_horizon, strategy=strategy)
...
def run_mode_d(assets_list, diag_years=1, horizon=PREDICTION_HORIZON, permtest=False):
    ...
    # Use shorter windows for 1h/2h horizons — override globally for this run
    global DIAG_WINDOWS
    _orig_diag_windows = DIAG_WINDOWS
    if horizon <= 2:
        DIAG_WINDOWS = DIAG_WINDOWS_SHORT
        print(f"  [Short-horizon mode: using DIAG_WINDOWS={DIAG_WINDOWS_SHORT}]")
    ...
    # Restore original DIAG_WINDOWS
    DIAG_WINDOWS = _orig_diag_windows
...
def run_strategy_comparison(assets_list, horizons=None):
    ...
    for asset in assets_list:
        ...
        # Load all available horizon configs (1h, 2h, 4h, 8h)
        signals_by_h = {}
        accs_by_h = {}
        for h in [1, 2, 4, 8]:
            ...
            if len(cfg_h) == 0:
                continue
            row_h = cfg_h.iloc[0]
            opt = row_h.get('optimal_features', '')
            feats = [f.strip() for f in str(opt).split(',') if f.strip()] if pd.notna(opt) and str(opt).strip() not in ('', 'nan') else None
            sigs = generate_signals(asset, row_h['models'].split('+'),
                                    int(row_h['best_window']), REPLAY_HOURS,
                                    feature_override=feats, horizon=h)
            signals_by_h[h] = simulate_portfolio(sigs)
            accs_by_h[h] = float(row_h.get('accuracy', 65)) / 100

        if not signals_by_h:
            print(f"  No saved models for {asset}. Run Mode D first.")
            continue

        has_4h = 4 in signals_by_h
        has_8h = 8 in signals_by_h
        has_1h = 1 in signals_by_h
        has_2h = 2 in signals_by_h

        sig_maps = {h: {s['datetime']: s for s in sigs} for h, sigs in signals_by_h.items()}
        all_times = sorted(set(dt for m in sig_maps.values() for dt in m))
        ...
        if has_4h and has_8h:
            ...
            strategies += ['8h_only']
        if has_8h and has_1h:
            strategies.append('8h_and_1h')
        if has_8h and has_2h:
            strategies.append('8h_and_2h')
        if has_1h:
            strategies.append('1h_only')
        if has_2h:
            strategies.append('2h_only')
        if len(signals_by_h) >= 3:
            strategies.append('any_agree')

        base_acc = sum(accs_by_h.values()) / len(accs_by_h)
        ...
        for strat in strategies:
            ...
            for dt in all_times:
                sigs_at_dt = {}
                price = None
                for h, hmap in sig_maps.items():
                    s = hmap.get(dt)
                    if s:
                        if price is None: price = s['close']
                        sigs_at_dt[h] = {'signal': s['signal'], 'confidence': s['confidence']}
                if price is None:
                    continue
                signal, _ = _compute_strategy_signal(sigs_at_dt, strat, MIN_CONFIDENCE)
            ...
            if in_pos and all_times:
                last_s = next((sig_maps[h].get(all_times[-1]) for h in sorted(sig_maps) if sig_maps[h].get(all_times[-1])), None)
                if last_s: cash = held * last_s['close'] * (1 - TRADING_FEE)
        ...
        for thresh in CONF_THRESHOLDS:
            ...
            for dt in all_times:
                ...
                if price is None:
                    continue
                signal, _ = _compute_strategy_signal(sigs_at_dt, best_strat, thresh)
                if signal == 'BUY' and not in_pos:
                    held = cash * (1 - TRADING_FEE) / price
            ...
            if score_c > best_conf_score:
                best_conf_score = score_c; best_conf = thresh; marker = " <-- BEST"
...
def _run_quick_asset(asset):
    ...
    for h in [1, 2, 4, 8]:
        ...
        signals_by_horizon[h] = sigs

    if signals_by_horizon:
        generate_strategy_html(asset, signals_by_horizon, strategy=strategy)
        generate_signal_table_html(asset, signals_by_horizon, strategy=strategy)
    ...
    if results:
        ...
        if results:
            # Build sigs_at_now dict for _compute_strategy_signal
            sigs_now = {h: {'signal': r['signal'], 'confidence': r['confidence']}
                        for h, r in results.items()}
            combined, conf_combined = _compute_strategy_signal(sigs_now, strategy, MIN_CONFIDENCE)

            # Build human-readable reason
            active = sorted(results.keys())
            sig_parts = ' | '.join(f"{h}h={results[h]['signal']}({results[h]['confidence']:.0f}%)" for h in active)
            reason = f"{sig_parts}  →  [{strategy}]"

            price = results[max(results.keys())].get('price', 0)
            arrow = '▲' if combined == 'BUY' else '▼' if combined == 'SELL' else '─'
            print(f"\n  >>> {arrow} COMBINED [{strategy}]: {combined} ({conf_combined:.0f}%)")
            print(f"  >>> {reason}")
...
def main():
    ...
    if cli_args and cli_args[0].upper() in ('B', 'D', 'E', 'F', '5', '6', '7'):
        ...
        print("=" * 60)
        print("  CRYPTO HOURLY ML TRADING SYSTEM -- V5.1")
