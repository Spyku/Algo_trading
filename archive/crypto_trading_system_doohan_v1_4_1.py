"""
Doohan V1.4.1 — Multi-sampler Optuna (TPE + 2×Random × 150 trials) + continuous gamma/features
============================================================
...
Doohan — Optuna-based joint optimization + XGBoost:
  ...
  - Continuous gamma: suggest_float(0.995, 0.999) — lets TPE exploit gradients
  - Continuous features: suggest_int(10, 40) — finer granularity than discrete list
  - Windows: [72, 100, 150, 200] — 48h and below deliberately excluded
  - Combos: LGBM-required only (confirmed)
  - Diversity-aware top-6 save with gamma/feature bands
  - 3 samplers: TPE (exploit) + 2× Random (explore)
...
CLI Usage:
  python crypto_trading_system_doohan_v1_4_1.py D BTC 8h
  python crypto_trading_system_doohan_v1_4_1.py DG BTC 8h
  python crypto_trading_system_doohan_v1_4_1.py G BTC 8h
  python crypto_trading_system_doohan_v1_4_1.py DF BTC,ETH 4,8h
  python crypto_trading_system_doohan_v1_4_1.py B BTC

Outputs:
  charts/{ASSET}_backtest.png
  models/crypto_doohan_v1_4_1_best_models.csv       (top 6 candidates from Mode D)
  models/crypto_doohan_v1_4_1_production.csv         (best live performer from Mode G)
...
PRODUCTION_CSV = 'models/crypto_doohan_v1_4_1_production.csv'
...
def _get_models_csv_path():
    ...
    if OPTUNA_METRIC != 'apf':
        return f'{MODELS_DIR}/crypto_doohan_v1_4_1_best_models_{OPTUNA_METRIC}.csv'
    return f'{MODELS_DIR}/crypto_doohan_v1_4_1_best_models.csv'


def _backup_models_csv():
    """Create a timestamped backup of production CSV before writing (failsafe against contamination)."""
    src = f'{MODELS_DIR}/crypto_doohan_v1_4_1_best_models.csv'
    if os.path.exists(src) and not MODELS_CSV_OVERRIDE:
        import shutil
        bak = f'{MODELS_DIR}/crypto_doohan_v1_4_1_best_models_backup.csv'
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False):
    ...
    print("\n" + "=" * 70)
    print(f"  DOOHAN V1.4.1 MODE D: MULTI-SAMPLER OPTUNA -- {horizon}h HORIZON")
    ...
    for asset_name in assets_list:
        ...
        # Step 3: Multi-sampler Optuna studies (V1.4.1)
        n_samplers = len(DOOHAN_SAMPLERS)
        ...
        print(f"  Search space: {len(combo_options)} combos × {len(DIAG_WINDOWS)} windows × gamma[{DOOHAN_GAMMA_MIN}-{DOOHAN_GAMMA_MAX}] × features[{DOOHAN_FEAT_MIN}-{feat_max_actual}]")
        ...
        for t in all_completed_trials:
            score_key = (round(t.value, 3), t.params['combo'], t.params['window'],
                         t.params.get('gamma', 0), t.params.get('n_features', 0))
...
def main():
    ...
    if cli_args and cli_args[0].upper() in ('B', 'D', 'DF', 'DG', 'F', 'G', 'H', 'HF', '5', '6', '7'):
        ...
        print("=" * 60)
        print("  CRYPTO HOURLY ML TRADING SYSTEM -- DOOHAN V1.4.1")
        print("  Optuna TPE + 2xRandom | Continuous gamma+features | LGBM-required combos")
