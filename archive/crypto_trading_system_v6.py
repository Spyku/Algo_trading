"""
Crypto Hourly Trading System — V5.4 Production
============================================================
ML trading system for BTC, ETH, XRP, DOGE with 4h and 8h horizons.
125 features -> walk-forward ML -> BUY/SELL/HOLD signals.

V5.4 (production):
  - Orphan worker cleanup at Mode D/E start (prevents CPU drain from killed runs)
  ...
  - Lighter LGBM for feature analysis (n_est=100, depth=4)
  - LOKY_MAX_CPU_COUNT capped at N_JOBS_PARALLEL
...
CLI Usage (skip all menus):
  python crypto_trading_system.py B BTC 4,8h
  python crypto_trading_system.py D ETH 8h 1y
  python crypto_trading_system.py D BTC,ETH 4,8h 1y
...
# ============================================================
# V6 ENHANCEMENTS — each toggleable for A/B testing
# ============================================================
ENHANCEMENTS = {
    'wavelet_denoising':      False,  # Denoise close price via DWT before computing features
    'fractional_diff':        False,  # Add fractionally differentiated price features (d≈0.35)
    'hmm_regime':             False,  # Add Hidden Markov Model regime state as feature
    'xgboost_model':          False,  # Add XGBoost to model pool (31 combos instead of 15)
    'sample_weighting':       False,  # Time-decay + label-uniqueness sample weights
    'entropy_filter':         False,  # Filter low-conviction signals via Shannon entropy
    'tri_state_labels':       False,  # 3-class labels: BUY / SELL / NO-ACTION
    'stacking_ensemble':      False,  # Stacking meta-learner instead of majority voting
    'dynamic_feature_select': False,  # Mutual-information feature selection per walk-forward step
    'meta_labeling':          False,  # Second model predicts if primary signal is correct
    'adversarial_validation': False,  # Drop features causing train/test distribution shift
    'kelly_sizing':           False,  # Fractional Kelly position sizing in backtest
}
...
def build_hourly_features(df_hourly, horizon=PREDICTION_HORIZON, verbose=True):
    ...
    # --- V6: Wavelet denoising ---
    if ENHANCEMENTS.get('wavelet_denoising'):
        try:
            import pywt
            close_arr = df['close'].values.astype(float)
            coeffs = pywt.wavedec(close_arr, 'db4', level=3)
            # Zero out the two highest frequency detail coefficients
            coeffs[-1] = np.zeros_like(coeffs[-1])
            coeffs[-2] = np.zeros_like(coeffs[-2])
            denoised = pywt.waverec(coeffs, 'db4')[:len(close_arr)]
            df['close'] = denoised
            if verbose:
                print("    [V6] Wavelet denoising applied to close price")
        except ImportError:
            print("    [V6] WARNING: pywt not installed — pip install PyWavelets")

    # --- V6: Fractional differentiation features ---
    if ENHANCEMENTS.get('fractional_diff'):
        try:
            def _fracdiff(series, d=0.35, window=100):
                """Fractional differentiation via expanding-window binomial weights."""
                weights = [1.0]
                ...
                for t in range(window, len(series)):
                    result[t] = np.dot(weights, series[t - window + 1:t + 1])
                return result

            df['fracdiff_close'] = _fracdiff(df['close'].values, d=0.35)
            df['fracdiff_volume'] = _fracdiff(df['volume'].values, d=0.35)
            df['fracdiff_close'] = df['fracdiff_close'].ffill().bfill()
            df['fracdiff_volume'] = df['fracdiff_volume'].ffill().bfill()
            if verbose:
                print("    [V6] Fractional differentiation features added (d=0.35)")
        except Exception as e:
            print(f"    [V6] WARNING: Fractional diff failed: {e}")
    ...
    # --- V6: Tri-state labels ---
    if ENHANCEMENTS.get('tri_state_labels'):
        rolling_std = future_return.rolling(200, min_periods=50).std().shift(horizon)
        threshold = 0.3 * rolling_std.fillna(0.01)
        df['label'] = 1  # NO-ACTION (class 1)
        df.loc[future_return > rolling_median + threshold, 'label'] = 2  # BUY
        df.loc[future_return < rolling_median - threshold, 'label'] = 0  # SELL
        df['label'] = df['label'].astype(int)
        if verbose:
            counts = df['label'].value_counts().sort_index()
            print(f"    [V6] Tri-state labels: SELL={counts.get(0,0)} HOLD={counts.get(1,0)} BUY={counts.get(2,0)}")
    else:
    ...
    # --- V6: Add fractional diff features to feature list ---
    if ENHANCEMENTS.get('fractional_diff') and 'fracdiff_close' in df.columns:
        feature_cols.extend(['fracdiff_close', 'fracdiff_volume'])
...
CHARTS_DIR = 'charts'
MODELS_DIR = 'models'
# Allow test harnesses to redirect model output to a separate CSV (prevents production contamination)
...
def build_all_features(df_hourly, asset_name='BTC', horizon=PREDICTION_HORIZON, verbose=True):
    ...
    # --- V6: HMM regime detection (GMM proxy — no C build needed) ---
    if ENHANCEMENTS.get('hmm_regime'):
        try:
            from sklearn.mixture import GaussianMixture
            ret = df['close'].pct_change().fillna(0).values
            vol = pd.Series(ret).rolling(24).std().fillna(0.01).values
            X_hmm = np.column_stack([ret[24:], vol[24:]])
            gmm = GaussianMixture(n_components=3, covariance_type='full', n_init=3, random_state=42)
            gmm.fit(X_hmm)
            states = np.full(len(df), 0)
            states[24:] = gmm.predict(X_hmm)
            df['hmm_regime'] = states
            # One-hot encode regime states
            for s in range(3):
                df[f'hmm_regime_{s}'] = (df['hmm_regime'] == s).astype(int)
            all_cols.extend(['hmm_regime_0', 'hmm_regime_1', 'hmm_regime_2'])
            added += 3
            if verbose:
                print(f"    [V6] GMM regime detection: 3 states added as features")
        except Exception as e:
            print(f"    [V6] WARNING: Regime detection failed: {e}")
...
# --- V6: Add XGBoost to model pool ---
if ENHANCEMENTS.get('xgboost_model'):
    try:
        import xgboost as xgb
        ALL_MODELS['XGB'] = lambda: xgb.XGBClassifier(
            n_estimators=300, max_depth=4, learning_rate=0.05,
            use_label_encoder=False, eval_metric='logloss',
            random_state=42, verbosity=0, n_jobs=1,
        )
        print("  [V6] XGBoost added to model pool")
    except ImportError:
        print("  [V6] WARNING: xgboost not installed — pip install xgboost")
...
def _eval_one_config(features_np, labels_np, closes_np, combo, window, n, step, model_factories, pred_horizon=4):
    ...
    # Read enhancement flags from env vars (safe for joblib workers)
    _enh = {}
    for _k in ['sample_weighting', 'entropy_filter', 'tri_state_labels', 'stacking_ensemble',
                'dynamic_feature_select', 'meta_labeling', 'adversarial_validation', 'kelly_sizing']:
        _ev = os.environ.get(f'ENH_{_k.upper()}')
        _enh[_k] = _ev is not None and _ev.lower() in ('1', 'true', 'yes')
    ...
    # V6: Stacking — accumulate OOF predictions for meta-learner
    if _enh['stacking_ensemble']:
        from sklearn.linear_model import LogisticRegression as _LR
        oof_X = []  # list of [proba_model1, proba_model2, ...]
        oof_y = []  # true labels
        meta_ready = False

    # V6: Meta-labeling — accumulate primary predictions and outcomes
    if _enh['meta_labeling']:
        from sklearn.ensemble import RandomForestClassifier as _MetaRF
        meta_X_hist = []
        meta_y_hist = []
    ...
    for i in range(min_start, n, step):
        ...
        # V6: Dynamic feature selection (mutual information)
        feature_mask = None
        if _enh['dynamic_feature_select']:
            try:
                from sklearn.feature_selection import mutual_info_classif
                mi = mutual_info_classif(X_train, y_train, random_state=42, discrete_features=False)
                median_mi = np.median(mi)
                feature_mask = mi > median_mi
                if feature_mask.sum() < 5:
                    feature_mask = None  # too few features, skip
                else:
        ...
        # V6: Adversarial validation — drop features causing distribution shift
        if _enh['adversarial_validation'] and X_train.shape[0] > 20:
            try:
                from sklearn.ensemble import RandomForestClassifier as _AdvRF
                X_comb = np.vstack([X_train, X_test])
                y_comb = np.array([0]*len(X_train) + [1]*len(X_test))
                adv = _AdvRF(n_estimators=30, max_depth=2, random_state=42, n_jobs=1)
                adv.fit(X_comb, y_comb)
                adv_score = adv.score(X_comb, y_comb)
                if adv_score > 0.6:
                    imp = adv.feature_importances_
                    keep = imp < np.percentile(imp, 85)
                    if keep.sum() >= 5:
        ...
        # V6: Sample weighting (time decay + label uniqueness)
        sample_weight = None
        if _enh['sample_weighting']:
            n_train = len(y_train)
            time_weights = np.linspace(0.5, 1.0, n_train)
            # Approximate uniqueness: labels use `horizon` future hours, consecutive overlap
            overlap = max(1, pred_horizon)
            uniqueness = np.ones(n_train)
            for j in range(n_train):
                concurrent = min(overlap, n_train - j)
                uniqueness[j] = 1.0 / max(concurrent, 1)
            sample_weight = time_weights * uniqueness
            sample_weight = sample_weight / sample_weight.mean()  # normalize to mean=1

        # Train models and collect votes + probas
        votes = []
        probas = []
        for model_name in combo:
            try:
                ...
                proba = model.predict_proba(X_test_s)[0]
                votes.append(pred)
                probas.append(proba[1] if len(proba) > 1 else proba[0])
            except Exception:
                continue
        if not votes:
            continue

        # V6: Stacking ensemble — use meta-learner instead of majority vote
        if _enh['stacking_ensemble'] and meta_ready and len(probas) == len(combo):
            try:
                meta_input = np.array(probas).reshape(1, -1)
                ensemble_pred = meta_model.predict(meta_input)[0]
            except Exception:
                ensemble_pred = 1 if sum(votes) > len(votes) / 2 else 0
        elif _enh['tri_state_labels']:
            # For tri-state: BUY=2, HOLD=1, SELL=0
            vote_counts = {}
            for v in votes:
        ...
        # V6: Stacking — accumulate OOF data and train meta-learner when enough data
        if _enh['stacking_ensemble'] and len(probas) == len(combo):
            oof_X.append(probas)
            oof_y.append(y_true)
            if len(oof_X) >= 10 and not meta_ready:
                try:
                    meta_model = _LR(max_iter=500, random_state=42)
                    meta_model.fit(np.array(oof_X), np.array(oof_y))
                    meta_ready = True
                except Exception:
                    pass
            elif meta_ready and len(oof_X) % 5 == 0:
                try:
                    meta_model.fit(np.array(oof_X), np.array(oof_y))
                except Exception:
                    pass

        # V6: Entropy filter — skip low-conviction signals
        if _enh['entropy_filter'] and len(probas) >= 2:
            p_arr = np.array(probas)
            # Binary entropy per model, then average
            eps = 1e-10
            entropies = -(p_arr * np.log(p_arr + eps) + (1 - p_arr) * np.log(1 - p_arr + eps))
            avg_entropy = entropies.mean()
            if avg_entropy > 0.65:
                # High disagreement — treat as HOLD (don't count as correct/incorrect, don't trade)
                continue

        # V6: Meta-labeling — secondary model filters primary signal
        if _enh['meta_labeling']:
            avg_proba = np.mean(probas)
            proba_std = np.std(probas) if len(probas) > 1 else 0
            meta_features = [avg_proba, proba_std, len(combo)]
            meta_X_hist.append(meta_features)
            primary_correct = 1 if ensemble_pred == y_true else 0
            meta_y_hist.append(primary_correct)
            if len(meta_X_hist) >= 15 and not meta_model_ready:
                try:
                    _meta_rf = _MetaRF(n_estimators=50, max_depth=3, random_state=42, n_jobs=1)
                    _meta_rf.fit(np.array(meta_X_hist), np.array(meta_y_hist))
                    meta_model_ready = True
                except Exception:
                    pass
            if meta_model_ready:
                try:
                    meta_pred = _meta_rf.predict_proba(np.array(meta_features).reshape(1, -1))[0]
                    if meta_pred[1] < 0.4:  # meta-model says primary is likely wrong
                        continue  # skip this signal
                    if len(meta_X_hist) % 5 == 0:
                        _meta_rf.fit(np.array(meta_X_hist), np.array(meta_y_hist))
                except Exception:
                    pass

        # Accuracy tracking
        if _enh['tri_state_labels']:
            # For tri-state: correct if predicted class matches
            if ensemble_pred == y_true:
                correct += 1
            total += 1
            # Portfolio: BUY=2, SELL=0, HOLD=1
            price = closes_np[i]
            if ensemble_pred == 2 and not in_position:
                in_position = True
            ...
            # V6: Kelly sizing — scale position by Kelly fraction
            if _enh['kelly_sizing'] and trades >= 5:
                win_rate_so_far = wins / trades
                avg_w = (total_gain / wins) if wins > 0 else 0.01
                avg_l = abs(total_loss / (trades - wins)) if (trades - wins) > 0 else 0.01
                kelly_f = win_rate_so_far - (1 - win_rate_so_far) / (avg_w / max(avg_l, 0.001))
                position_frac = max(0.1, min(kelly_f * 0.5, 1.0))  # half-Kelly, floor 10%
            else:
                position_frac = 1.0
            ...
            if ensemble_pred == 1 and not in_position:
                in_position = True
                entry_price = price * (1 + TRADING_FEE)
                entry_frac = position_frac  # remember position size
            elif ensemble_pred == 0 and in_position:
                sell_price = price * (1 - TRADING_FEE)
                trade_return = (sell_price - entry_price) / entry_price
                if _enh['kelly_sizing']:
                    portfolio *= (1 + trade_return * entry_frac)
                else:
                    portfolio *= (1 + trade_return)
                trades += 1
                if trade_return > 0:
        ...
        if in_position:
            current_val = portfolio * (closes_np[i] / entry_price)
        else:
...
def run_mode_b(assets_list, horizon_filter=None, skip_data_update=False):
    ...
    _csv = MODELS_CSV_OVERRIDE if MODELS_CSV_OVERRIDE else f'{MODELS_DIR}/crypto_hourly_best_models.csv'
    if not os.path.exists(_csv):
        print(f"\nERROR: {_csv} not found!")
        print("Please run Mode D first to find best models.")
        return

    df_best = pd.read_csv(_csv)
...
def run_mode_d(assets_list, diag_years=1, horizon=PREDICTION_HORIZON, permtest=False, resume=False, training_period=None):
    """
    ...
    for asset_name in assets_list:
        ...
        if best_config:
            ...
            best_config['training_period'] = training_period or f'{diag_years}y'
    ...
    # MODELS_CSV_OVERRIDE redirects output to a test CSV, preventing production contamination
    csv_path = MODELS_CSV_OVERRIDE if MODELS_CSV_OVERRIDE else f'{MODELS_DIR}/crypto_hourly_best_models.csv'
    ...
    if mode == 'B':
        ...
        for h in horizons:
            if len(horizons) > 1:
                ...
                print(f"{'#'*60}")
            if mode in ('D', 'DF'):
                run_mode_d(assets_list, diag_years=diag_years, horizon=h, permtest=flag_permtest, resume=flag_resume, training_period=f'{diag_years}y')
            elif mode == 'E':
