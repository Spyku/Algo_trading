...
Patches crypto_trading_system.py in-place.
...
TARGET = Path('crypto_trading_system.py')
BACKUP = Path('crypto_trading_system.py.bak')

# ─────────────────────────────────────────────────────────────────────────────
# Each patch: (description, old_text, new_text)
# ─────────────────────────────────────────────────────────────────────────────
PATCHES = []

# ── PATCH 1: Replace _quick_accuracy with _quick_score + backward-compat wrapper ──
PATCHES.append((
    "_quick_accuracy → _quick_score (adds alpha to walk-forward)",
...
# ── PATCH 2: _perm_one_feature — add baseline_alpha param, return alpha_drop ──
PATCHES.append((
    "_perm_one_feature — add alpha_drop",
...
# ── PATCH 3: _test_permutation_importance — use _quick_score, display alpha ──
PATCHES.append((
    "_test_permutation_importance — score on acc + alpha",
...
# ── PATCH 4: _ablation_one_feature — add baseline_alpha, return alpha_change ──
PATCHES.append((
    "_ablation_one_feature — add alpha_change",
...
# ── PATCH 5: _test_ablation — use _quick_score, display alpha ──
PATCHES.append((
    "_test_ablation — score on acc + alpha",
...
# ── PATCH 6: _reduced_one_set — return alpha + combined_score ──
PATCHES.append((
    "_reduced_one_set — return alpha + combined_score",
...
# ── PATCH 7: _test_reduced_sets — display alpha, rank by combined_score ──
PATCHES.append((
    "_test_reduced_sets — rank by combined_score",
...
# ── PATCH 8: _score_features — add alpha signals in permutation + ablation ──
PATCHES.append((
    "_score_features — add alpha signals",
...
# ── PATCH 9: run_feature_analysis final comparison — use combined_score ──
PATCHES.append((
    "run_feature_analysis final comparison — combined_score",
...
# ── PATCH 10: run_mode_d — download macro data before update_all_data ──
PATCHES.append((
    "run_mode_d — download macro data first",
...
def main():
    if not TARGET.exists():
        print(f"ERROR: {TARGET} not found. Run this from the algo_trading folder.")
        sys.exit(1)
    ...
    print(f"  File    : {original_len:,} → {len(content):,} chars")
    if failed:
        print(f"\n  SKIPPED patches (already applied or code changed):")
        for f in failed:
            print(f"    - {f}")
    else:
