...
DEKU_VERSION = '1.3'              # Deku V1.3: CPCV + two-phase Optuna
...
def _get_models_csv_path():
    """Return the models CSV path — V1.3 uses separate file to avoid contaminating production."""
    if MODELS_CSV_OVERRIDE:
        return MODELS_CSV_OVERRIDE
    if OPTUNA_METRIC != 'apf':
        return f'{MODELS_DIR}/crypto_deku_v13_best_models_{OPTUNA_METRIC}.csv'
    return f'{MODELS_DIR}/crypto_deku_v13_best_models.csv'
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False):
    """
    DEKU V1.3 Mode D: Two-phase Optuna + CPCV validation.

    Pipeline:
    1. Download fresh data
    2. Build all features, cap at 6 months
    3. LGBM importance ranking (~1 min)
    4. Two-phase Optuna study:
       - Phase 1: 78 random trials (3 per combo) — guaranteed exploration
       - Phase 2: 72 TPE trials — exploitation of promising regions
    ...
    total_trials = n_phase1 + n_phase2

    print("\n" + "=" * 70)
    print(f"  DEKU V1.3 MODE D: TWO-PHASE OPTUNA + CPCV -- {horizon}h HORIZON")
    print(f"  Models: {', '.join(ALL_MODELS.keys())} ({len(combo_options)} combos)")
    print(f"  Phase 1: {n_phase1} random trials ({PHASE1_TRIALS_PER_COMBO}/combo) | Phase 2: {n_phase2} TPE trials")
    ...
    for asset_name in assets_list:
        ...
        # Step 3: Two-phase Optuna study
        print(f"\n{'='*70}")
        print(f"  TWO-PHASE OPTUNA: {asset_name} {horizon}h")
        ...
        print(f"  Phase 1: {n_phase1} random ({PHASE1_TRIALS_PER_COMBO}/combo) | Phase 2: {n_phase2} TPE | Total: {total_trials}")
        print(f"{'='*70}")

        # Suppress Optuna's default logging
        optuna.logging.set_verbosity(optuna.logging.WARNING)

        # Phase 1: Random sampler — guaranteed exploration of all 26 combos
        study = optuna.create_study(
            ...
            study_name=f'deku_v13_{asset_name}_{horizon}h',
        )
        ...
        # ── Phase 1: Random exploration (3 trials per combo) ──
        print(f"\n  Phase 1: {n_phase1} random trials ({PHASE1_TRIALS_PER_COMBO} per combo)...")
        study.optimize(objective, n_trials=n_phase1, show_progress_bar=False)
        phase1_best = study.best_value if study.best_trial else 0.0
        print(f"  Phase 1 done: best {OPTUNA_METRIC}={phase1_best:.3f}")

        # ── Phase 2: TPE exploitation building on Phase 1 results ──
        # Switch sampler to TPE — it will use Phase 1 trials as seed observations
        study.sampler = optuna.samplers.TPESampler(seed=43, n_startup_trials=0)
        print(f"\n  Phase 2: {n_phase2} TPE trials (exploiting Phase 1 results)...")
        study.optimize(objective, n_trials=n_phase2, show_progress_bar=False)
        ...
        if min_test_rows >= 100:
            ...
            for ci, trial in enumerate(candidates):
                ...
                for pi, (train_idx, test_idx) in enumerate(cpcv_paths):
                    ...
                    p_funding = funding_np_all[test_idx]
                    n_p = len(test_idx)
                    ...
                    p_result = _deku_eval_with_pruning(
                        ...
                        funding_threshold=c_funding_thresh, funding_np=p_funding,
                        horizon=horizon
                    )
        ...
        if full_result:
            ...
            print(f"\n  {'='*70}")
            print(f"  WINNER (by CPCV): {asset_name} {horizon}h  {pbo_icon}")
