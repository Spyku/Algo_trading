"""
Crypto Hourly Trading System — V5.6 (Literature Features)
============================================================
Based on V5.5. Adds 14 literature-backed technical features derived
from OHLCV data that capture new dimensions not in V5.5:

V5.6 new features (HIGH priority):
  - Garman-Klass volatility (7.4x more efficient than close-to-close)
  - ADX (trend strength regardless of direction)
  - Rolling Hurst exponent (regime detection: trending vs mean-reverting)
  - MFI (volume-weighted RSI — leading indicator)

V5.6 new features (MEDIUM priority):
  - Parkinson volatility (range-based, 5.2x more efficient)
  - Rogers-Satchell volatility (works with non-zero drift / trending markets)
  - Realized skewness (crash risk / return asymmetry)
  - Realized kurtosis (tail risk)
  - Amihud illiquidity ratio (price impact per unit volume)
  - OBV rate of change (volume-trend divergence)
  - MFI (already in HIGH)
  - Close position in range (intra-bar sentiment)
  - Body ratio / candle efficiency (conviction behind each candle)
  - CCI (Commodity Channel Index — overbought/oversold with different sensitivity)

Modes:
  B. Quick run (saved models)    D. Full pipeline (1-8h horizons)
  E. Iterative refinement        F. Strategy Optimizer (2-phase confidence sweep)
  G. Horizon pair test (all pairs x 2 strategies over 168h)
  5/6/7. Quick BTC/ETH/XRP

CLI Usage:
  python crypto_trading_system_v5.6.py D ETH 1,2,3,4,5,6,7,8h 1y  # all 8 horizons
  python crypto_trading_system_v5.6.py D BTC 4,8h 1y               # standard
  python crypto_trading_system_v5.6.py F BTC 4,8h                  # strategy optimizer
  python crypto_trading_system_v5.6.py F BTC                       # defaults to 4,8h
  python crypto_trading_system_v5.6.py G BTC                       # pair test (168h)

Outputs:
  charts/{ASSET}_backtest.png
  models/crypto_hourly_best_models.csv
  models/crypto_hourly_chart_data.json
  config/trading_config.json  (updated by Mode F)
"""
...
AVAILABLE_HORIZONS = [1, 2, 3, 4, 5, 6, 7, 8]  # V5.5: all horizons
...
def build_hourly_features(df_hourly, horizon=PREDICTION_HORIZON, verbose=True):
    ...
    # ---- V5.6: RANGE-BASED VOLATILITY ESTIMATORS ----
    ...
    # Parkinson (1980): uses high-low range, 5.2x more efficient
    park_single = log_hl**2 / (4 * np.log(2))
    df['parkinson_vol_14h'] = park_single.rolling(14).mean().apply(np.sqrt)

    # Rogers-Satchell (1991): works correctly with non-zero drift (trending markets)
    log_hc = np.log(df['high'] / df['close'])
    log_ho = np.log(df['high'] / df['open'])
    log_lc = np.log(df['low'] / df['close'])
    log_lo = np.log(df['low'] / df['open'])
    rs_single = log_hc * log_ho + log_lc * log_lo
    df['rogers_satchell_vol_14h'] = rs_single.rolling(14).mean().apply(np.sqrt)

    # ---- V5.6: ADX — TREND STRENGTH (Wilder 1978) ----
    _tr = pd.DataFrame({
        'hl': df['high'] - df['low'],
        'hc': abs(df['high'] - df['close'].shift(1)),
        'lc': abs(df['low'] - df['close'].shift(1))
    ...
    # Zero out +DM when -DM is larger, and vice versa
    ...
    # ---- V5.6: HURST EXPONENT (regime detection) ----
    # Rolling R/S analysis over 168h window
    _hurst_input = df['logret_1h'] if 'logret_1h' in df.columns else df['close'].pct_change()
    _hurst_vals = _hurst_input.values
    _hurst_result = np.full(len(_hurst_vals), np.nan)
    _hurst_window = 168
    for _hi in range(_hurst_window, len(_hurst_vals)):
        seg = _hurst_vals[_hi - _hurst_window:_hi]
        if np.isnan(seg).any():
            continue
        ...
        s = np.std(seg, ddof=1)
        if s > 1e-10 and r > 0:
            _hurst_result[_hi] = np.log(r / s) / np.log(_hurst_window)
    df['hurst_168h'] = _hurst_result
    df['hurst_168h'] = df['hurst_168h'].ffill()

    # ---- V5.6: MFI — MONEY FLOW INDEX (volume-weighted RSI) ----
    ...
    mf_ratio = pos_flow / (neg_flow + 1e-10)
    df['mfi_14h'] = 100 - (100 / (1 + mf_ratio))

    # ---- V5.6: REALIZED SKEWNESS & KURTOSIS ----
    _ret_1h = df['close'].pct_change()
    for w in [48, 168]:
        r_mean = _ret_1h.rolling(w).mean()
        r_std = _ret_1h.rolling(w).std()
        standardized = (_ret_1h - r_mean) / (r_std + 1e-10)
        df[f'realized_skew_{w}h'] = standardized.pow(3).rolling(w).mean()
        df[f'realized_kurt_{w}h'] = standardized.pow(4).rolling(w).mean() - 3  # excess kurtosis

    # ---- V5.6: AMIHUD ILLIQUIDITY ----
    dollar_volume = df['volume'] * df['close']
    df['amihud_14h'] = (abs(_ret_1h) / (dollar_volume + 1e-10)).rolling(14).mean()
    # Log-transform to reduce skewness (Amihud values span orders of magnitude)
    df['amihud_14h'] = np.log1p(df['amihud_14h'] * 1e6)

    # ---- V5.6: OBV RATE OF CHANGE ----
    obv_sign = np.sign(df['close'].diff())
    obv = (df['volume'] * obv_sign).cumsum()
    obv_sma = obv.rolling(20).mean()
    df['obv_roc_20h'] = (obv - obv_sma) / (obv_sma.abs() + 1e-10)

    # ---- V5.6: CANDLE MICROSTRUCTURE ----
    candle_range = df['high'] - df['low']
    candle_range_safe = candle_range.replace(0, np.nan)
    df['close_position'] = (df['close'] - df['low']) / candle_range_safe
    df['close_position'] = df['close_position'].fillna(0.5)
    df['body_ratio'] = abs(df['close'] - df['open']) / candle_range_safe
    df['body_ratio'] = df['body_ratio'].fillna(0.0)
    # Rolling averages capture persistent patterns
    df['close_pos_12h'] = df['close_position'].rolling(12).mean()
    df['body_ratio_12h'] = df['body_ratio'].rolling(12).mean()

    # ---- V5.6: CCI — COMMODITY CHANNEL INDEX (Lambert 1980) ----
    tp_20 = typical_price.rolling(20).mean()
    tp_mad = typical_price.rolling(20).apply(lambda x: np.mean(np.abs(x - np.mean(x))), raw=True)
    df['cci_20h'] = (typical_price - tp_20) / (0.015 * tp_mad + 1e-10)
    ...
    feature_cols = [
        ...
        'parkinson_vol_14h', 'rogers_satchell_vol_14h',
        # V5.6: trend strength
        'adx_14h', 'plus_di_14h', 'minus_di_14h',
        # V5.6: regime detection
        'hurst_168h',
        # V5.6: volume-weighted momentum
        'mfi_14h',
        # V5.6: higher-order moments
        'realized_skew_48h', 'realized_skew_168h',
        'realized_kurt_48h', 'realized_kurt_168h',
        # V5.6: liquidity
        'amihud_14h',
        # V5.6: volume-price relationship
        'obv_roc_20h',
        # V5.6: candle microstructure
        'close_position', 'body_ratio',
        'close_pos_12h', 'body_ratio_12h',
        # V5.6: alternative overbought/oversold
        'cci_20h',
        # time features
...
def run_strategy_comparison(assets_list, horizons=None):
    ...
    Writes optimal strategy + min_confidence to trading_config.json.
    ...
    for asset in assets_list:
        ...
        if has_4h:
            ...
            print(f"  Generating 4h signals ({REPLAY_HOURS_F}h window)...")
        ...
        if has_8h:
            ...
            print(f"  Generating 8h signals ({REPLAY_HOURS_F}h window)...")
        ...
        print(f"\n  >>> Updated trading_config.json: {asset} -> strategy={best_strat}, min_confidence={best_conf_final}%")
...
def run_horizon_pair_comparison(assets_list):
    ...
    print(f"  Window: {REPLAY_HOURS_G}h (last week) | Fee: {TRADING_FEE*100:.2f}% each side")
    print("=" * 60)
    ...
    for asset in assets_list:
        ...
        print(f"\n  Generating {REPLAY_HOURS_G}h signals for each horizon...")
        sigs_by_horizon = {}
        ...
        for h in available_horizons:
            ...
            try:
                ...
                print(f"    {h}h: {len(sigs)} signals | model={r['models']} w={r['best_window']}h acc={r['accuracy']:.1f}%")
            except Exception as e:
        ...
        for i, (h1, h2, strat, ret, wr, tr) in enumerate(results):
            ...
            pair_str = f"{h1}h+{h2}h"
            print(f"  {pair_str:<10} {strat:<14} {ret:>+7.1f}% {wr:>7.0f}% {tr:>7d}{marker}")

        best = results[0]
        print(f"\n  >>> BEST PAIR for {asset}: {best[0]}h + {best[1]}h | {best[2]}")
...
def main():
    ...
    if cli_args and cli_args[0].upper() in ('B', 'D', 'E', 'F', 'G', '5', '6', '7'):
        ...
        # Parse horizon (default: 4,8h for Mode B and F, 4h for others)
        ...
        print("=" * 60)
        print("  CRYPTO HOURLY ML TRADING SYSTEM -- V5.5")
        ...
        print("  G. HORIZON PAIR TEST (all pairs x 2 strategies over 168h)")
        print("  ---")
        ...
        # Horizon selection (Mode F/G always uses both 4h+8h or all pairs)
        if mode == 'F':
            horizons = [4, 8]
            print(f"\nHorizon(s): 4h + 8h (Mode F always uses both)")
        elif mode == 'G':
