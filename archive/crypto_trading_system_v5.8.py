"""
Crypto Hourly Trading System — V5.8 Experimental (Temporal Decay)
============================================================
Based on V5.4 Production. Adds exponential decay sample weighting.
...
# V5.8: Exponential decay sample weighting
# gamma=1.0 means no decay (V5 baseline), gamma<1 emphasizes recent data
# Half-life in hours = ln(2) / ln(1/gamma)
DECAY_GAMMA = 1.0  # default: no decay (overridden by Mode D gamma sweep)

def get_decay_weights(n_samples, gamma=None):
    """Exponential decay weights: newest=1, oldest=gamma^(n-1).
    Returns None if gamma is 1.0 (no decay) for zero overhead."""
    if gamma is None:
        gamma = DECAY_GAMMA
    if gamma >= 1.0:
        return None
    ages = np.arange(n_samples - 1, -1, -1)  # oldest first = largest age
    return gamma ** ages
...
def _get_models_csv_path():
    ...
    return MODELS_CSV_OVERRIDE if MODELS_CSV_OVERRIDE else f'{MODELS_DIR}/crypto_hourly_best_models.csv'


def _backup_models_csv():
    """Create a timestamped backup of production CSV before writing (failsafe against contamination)."""
    src = f'{MODELS_DIR}/crypto_hourly_best_models.csv'
    if os.path.exists(src) and not MODELS_CSV_OVERRIDE:
        import shutil
        bak = f'{MODELS_DIR}/crypto_hourly_best_models_backup.csv'
...
def _quick_score(df_features, feature_cols, window=ANALYSIS_WINDOW, step=ANALYSIS_STEP, device=None, gamma=None):
    ...
    for i in range(min_start, n, step):
        ...
        sw = get_decay_weights(len(y_train), gamma=gamma)
...
def _test_lgbm_importance(df_features, feature_cols, gamma=None):
    ...
    sw = get_decay_weights(len(y), gamma=gamma)
...
def generate_signals(asset_name, model_names, window_size, replay_hours=REPLAY_HOURS,
                     feature_override=None, horizon=PREDICTION_HORIZON, gamma=None):
    warnings.filterwarnings('ignore')
...
# ============================================================
def _eval_one_config(features_np, labels_np, closes_np, combo, window, n, step, model_factories, pred_horizon=4, gamma=None):
    import os, warnings
    os.environ['PYTHONWARNINGS'] = 'ignore'
    warnings.filterwarnings('ignore')
    warnings.simplefilter('ignore')
...
def run_diagnostic_for_asset(asset_name, df_features, feature_cols, gamma=None):
    ...
    with _suppress_stderr():
        all_results = Parallel(n_jobs=N_JOBS_PARALLEL, verbose=0)(
            delayed(_eval_one_config)(
                ...
                pred_horizon=PREDICTION_HORIZON, gamma=gamma
            )
...
def run_mode_d(assets_list, diag_years=1, horizon=PREDICTION_HORIZON, permtest=False, resume=False, training_period=None, gamma=None):
    """
...
def _run_iteration_2(asset_name, df_features, prev_config, all_cols, horizon, gamma=None):
    """
    ...
    if len(np.unique(y)) >= 2:
        sw_imp = get_decay_weights(len(y), gamma=gamma)
        model_imp.fit(X, y, sample_weight=sw_imp)
    ...
    best_config = run_diagnostic_for_asset(asset_name, df_diag, refined_features, gamma=gamma)
...
def _run_iteration_3(asset_name, df_features, prev_config, all_cols, horizon):
    ...
    if len(np.unique(y)) < 2:
        ...
        sw_imp2 = get_decay_weights(len(y), gamma=gamma)
        model_imp.fit(X, y, sample_weight=sw_imp2)
...
def run_strategy_comparison(assets_list, horizons=None):
    ...
    tcfg_path = f'{CONFIG_DIR}/trading_config_v5.8_test.json'
    try:
...
def run_gamma_sweep(assets_list, horizons, diag_years=1, gammas=None):
    """V5.8 Mode G: Gamma Sweep — compare temporal decay settings.

    Uses EXISTING production model configs (features, window) from CSV.
    Only varies gamma (sample_weight decay) across the diagnostic walk-forward.
    Outputs a comparison table: gamma × (accuracy, return, win_rate, trades, score).
    """
    if gammas is None:
        gammas = [1.0, 0.999, 0.998, 0.997, 0.996, 0.995]
    ...
    for asset_name in assets_list:
        for horizon in horizons:
            # Load production config (features + window)
            config = _load_mode_d_config(asset_name, horizon)
            if config is None:
                print(f"  WARNING: No config for {asset_name} {horizon}h — skipping")
                continue

            feature_cols = config['optimal_features'].split(',') if config.get('optimal_features') else None
            if not feature_cols:
                print(f"  WARNING: No features for {asset_name} {horizon}h — skipping")
                continue

            prod_window = config['best_window']
            prod_combo = config['best_combo'].split('+')

            print(f"\n{'='*60}")
            print(f"  GAMMA SWEEP: {asset_name} {horizon}h | {config['best_combo']} | w={prod_window}")
            print(f"  Features: {len(feature_cols)} | Training: {diag_years}y")
            print(f"  Gammas: {gammas}")
            print(f"{'='*60}")

            # Load and prepare data
            global PREDICTION_HORIZON
            old_horizon = PREDICTION_HORIZON
            PREDICTION_HORIZON = horizon

            df_raw = load_data(asset_name)
            if df_raw is None:
                continue
            df, all_cols = build_all_features(df_raw, asset_name=asset_name, horizon=horizon)

            # Trim to training period
            diag_hours = int(diag_years * 365 * 24)
            if len(df) > diag_hours:
                df = df.tail(diag_hours).reset_index(drop=True)

            # Filter to features that exist
            feature_cols = [f for f in feature_cols if f in df.columns]
            df_diag = df.dropna(subset=feature_cols + ['label']).reset_index(drop=True)

            n = len(df_diag)
            features_np = df_diag[feature_cols].values.astype(np.float64)
            labels_np = df_diag['label'].values.astype(np.int32)
            closes_np = df_diag['close'].values.astype(np.float64)

            print(f"  Data: {n:,} rows after dropna")

            # Test each gamma with the SAME config (combo + window)
            for g in gammas:
                half_life = f"{np.log(2)/np.log(1/g):.0f}h" if g < 1.0 else "∞"
                print(f"\n  --- gamma={g} (half-life={half_life}) ---")

                t0 = time.time()
                result = _eval_one_config(
                    features_np, labels_np, closes_np,
                    tuple(prod_combo), prod_window, n, DIAG_STEP,
                    DIAG_MODELS, pred_horizon=horizon, gamma=g if g < 1.0 else None
                )
                elapsed = time.time() - t0
                ...
                combo_str, window, accuracy, total, cum_return, win_rate, trades, avg_gain, avg_loss, max_dd, combined_score = result

                # Compute ESS (effective sample size)
                if g < 1.0:
                    sw = get_decay_weights(prod_window, gamma=g)
                    ess = (sw.sum()**2) / (sw**2).sum()
                else:
                    ess = prod_window

                print(f"    Acc={accuracy*100:.1f}% | Ret={cum_return:+.1f}% | WR={win_rate:.0f}% | "
                      f"Trades={trades} | Score={combined_score:.4f} | ESS={ess:.0f} | {elapsed:.0f}s")

                results_all.append({
                    'asset': asset_name, 'horizon': horizon,
                    'gamma': g, 'half_life': half_life,
                    'accuracy': accuracy * 100, 'return_pct': cum_return,
                    'win_rate': win_rate, 'trades': trades,
                    'combined_score': combined_score, 'max_dd': max_dd,
                    'ess': ess, 'combo': combo_str, 'window': window
                })

            PREDICTION_HORIZON = old_horizon

    # Print comparison table
    if results_all:
        print(f"\n{'='*80}")
        print(f"  GAMMA SWEEP RESULTS COMPARISON")
        print(f"{'='*80}")

        df_results = pd.DataFrame(results_all)

        for asset_name in df_results['asset'].unique():
            for horizon in df_results['horizon'].unique():
                subset = df_results[(df_results['asset'] == asset_name) & (df_results['horizon'] == horizon)]
                if subset.empty:
                    continue

                print(f"\n  {asset_name} {horizon}h ({subset.iloc[0]['combo']}, w={subset.iloc[0]['window']})")
                print(f"  {'gamma':>7s} {'half-life':>10s} {'acc%':>6s} {'return%':>8s} {'WR%':>5s} {'trades':>6s} {'score':>8s} {'maxDD%':>7s} {'ESS':>5s}")
                print(f"  {'-'*7} {'-'*10} {'-'*6} {'-'*8} {'-'*5} {'-'*6} {'-'*8} {'-'*7} {'-'*5}")

                best_score = subset['combined_score'].max()
                for _, row in subset.iterrows():
                    marker = ' <<<' if row['combined_score'] == best_score else ''
                    print(f"  {row['gamma']:>7.3f} {row['half_life']:>10s} {row['accuracy']:>6.1f} "
                          f"{row['return_pct']:>+8.1f} {row['win_rate']:>5.0f} {row['trades']:>6.0f} "
                          f"{row['combined_score']:>8.4f} {row['max_dd']:>7.1f} {row['ess']:>5.0f}{marker}")

        # Save to CSV
        out_path = os.path.join(os.path.dirname(_get_models_csv_path()), 'gamma_sweep_results.csv')
...
def main():
    ...
    if cli_args and cli_args[0].upper() in ('B', 'D', 'DF', 'E', 'F', 'G', '5', '6', '7'):
        ...
        horizons = [4, 8] if mode in ('B', 'DF', 'G') else [4]
        for a in cli_args:
            if a.lower().endswith('h') and a[:-1].replace(',', '').isdigit():
                horizons = [int(h) for h in a[:-1].split(',')]

        # Parse training period (default: 1y). Supports: 1y, 2y, 9m, 6m
        diag_years = 1
        training_period_label = '1y'
        for a in cli_args:
            if a.lower().endswith('y') and a[:-1].isdigit():
                diag_years = int(a[:-1])
                training_period_label = f'{diag_years}y'
            elif a.lower().endswith('m') and a[:-1].isdigit():
                months = int(a[:-1])
                diag_years = months / 12
                training_period_label = f'{months}m'

        e_iterations = '2'

        print("=" * 60)
        print(f"  CLI: Mode {mode} | {','.join(assets_list)} | {','.join(str(h)+'h' for h in horizons)} | {training_period_label}")
        print("=" * 60)
        ...
        if mode in ('D', 'E'):
            ...
            print("  4. Last 9 months")
            print("  5. Last 6 months")
            range_choice = input("Enter choice (1-5) [1]: ").strip()
            if range_choice == '2':
                diag_years = 2
                training_period_label = '2y'
            elif range_choice == '3':
                diag_years = 4
                training_period_label = '4y'
            elif range_choice == '4':
                diag_years = 9 / 12
                training_period_label = '9m'
            elif range_choice == '5':
                diag_years = 6 / 12
                training_period_label = '6m'
            else:
                diag_years = 1
                training_period_label = '1y'
            print(f"Diagnostic range: last {training_period_label}")
    ...
    if mode == 'B':
        ...
        run_gamma_sweep(assets_list, horizons, diag_years=diag_years)
    else:
        for h in horizons:
            if len(horizons) > 1:
                print(f"\n{'#'*60}")
                ...
                print(f"{'#'*60}")
            if mode in ('D', 'DF'):
                run_mode_d(assets_list, diag_years=diag_years, horizon=h, permtest=flag_permtest, resume=flag_resume, training_period=training_period_label)
            elif mode == 'E':
