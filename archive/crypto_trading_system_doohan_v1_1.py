"""
Doohan V1.1 — Discrete Optuna search + LGBM-required combos + min window 48h
============================================================
...
CLI Usage:
  ...
  python crypto_trading_system_doohan.py B BTC

Outputs:
  charts/{ASSET}_backtest.png
  models/crypto_doohan_v1_1_best_models.csv
...
DIAG_STEP = 36
DIAG_WINDOWS = [48, 72, 100, 150, 200]  # V1.1: removed 24/36 — produce ~50% confidence in live, 0 trades
...
def _get_models_csv_path():
    ...
    if OPTUNA_METRIC != 'apf':
        return f'{MODELS_DIR}/crypto_doohan_v1_1_best_models_{OPTUNA_METRIC}.csv'
    return f'{MODELS_DIR}/crypto_doohan_v1_1_best_models.csv'


def _backup_models_csv():
    """Create a timestamped backup of production CSV before writing (failsafe against contamination)."""
    src = f'{MODELS_DIR}/crypto_doohan_v1_1_best_models.csv'
    if os.path.exists(src) and not MODELS_CSV_OVERRIDE:
        import shutil
        bak = f'{MODELS_DIR}/crypto_doohan_v1_1_best_models_backup.csv'
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False):
    ...
    print("\n" + "=" * 70)
    print(f"  DOOHAN MODE D: OPTUNA JOINT OPTIMIZATION -- {horizon}h HORIZON")
    ...
    for asset_name in assets_list:
        ...
        print(f"  Search space: {len(combo_options)} combos × {len(DIAG_WINDOWS)} windows × {len(DOOHAN_GAMMA_VALUES)} gammas × {len([f for f in DOOHAN_FEATURE_COUNTS if f <= max_n_features])} feat counts")
        ...
        study = optuna.create_study(
            ...
            study_name=f'doohan_{asset_name}_{horizon}h',
        )
        ...
        def objective(trial):
            ...
            n_feat = trial.suggest_categorical('n_features', [f for f in DOOHAN_FEATURE_COUNTS if f <= max_n_features])

            # Doohan: Prune duplicate param combos
        ...
        for t in completed_trials:
            score_key = (round(t.value, 3), t.params['window'], t.params.get('gamma', 0), t.params.get('n_features', 0))
...
def run_mode_h(assets_list, n_trials=MODE_H_DEFAULT_TRIALS, resume=False):
    ...
    for asset_name in assets_list:
        ...
        def objective(trial):
            ...
            n_feat = trial.suggest_categorical('n_features', [f for f in DOOHAN_FEATURE_COUNTS if f <= max_f])

            # Doohan: Prune duplicate param combos
            param_key = (h, combo_name, window, gamma, n_feat)
            if param_key in seen_params:
                raise optuna.TrialPruned(f"Duplicate: {param_key}")
            seen_params.add(param_key)
...
def run_strategy_comparison(assets_list, horizons=None):
    ...
    tcfg_path = f'{CONFIG_DIR}/trading_config_doohan_v1_1{metric_suffix}.json'
    try:
