"""
Crypto Hourly Trading System — DEKU 1.2.1.1
============================================================
Based on DEKU 1.2.1, adds:
  ...
  3. 3-fold rolling holdout: train on ~67%, validate on ~33%, averaged across 3 temporal folds
  4. Embargo: 4-candle gap between train/test in walk-forward (prevents label overlap leakage)
  5. Per-horizon feature ranges: 4h [4-40], 8h [4-50]

Everything else identical to DEKU. Isolated outputs — does NOT touch production.

CLI Usage:
  python crypto_trading_system_deku_1_2_1_1.py D BTC 4,8h
  python crypto_trading_system_deku_1_2_1_1.py D BTC 4,8h --trials 150
  python crypto_trading_system_deku_1_2_1_1.py DF BTC,ETH 4,8h
  python crypto_trading_system_deku_1_2_1_1.py B BTC

Outputs:
  charts/{ASSET}_backtest.png
  models/crypto_deku_1_2_1_1_best_models.csv
...
# Per-horizon feature ranges — 4h needs tighter to avoid overfitting, 8h keeps full range
N_FEATURES_RANGE = {
    4: (4, 40),   # 4h horizon: narrower range (overfitting observed with 80+ features)
    8: (4, 80),   # 8h horizon: full range (8h kept 75 features and performed well)
}
N_FEATURES_RANGE_DEFAULT = (4, 80)  # fallback for unknown horizons

# 3-fold rolling holdout — train on ~67%, validate on ~33%
N_HOLDOUT_FOLDS = 3
...
def _get_models_csv_path():
    ...
    if OPTUNA_METRIC != 'apf':
        return f'{MODELS_DIR}/crypto_deku_1_2_1_best_models_{OPTUNA_METRIC}.csv'
    return f'{MODELS_DIR}/crypto_deku_1_2_1_best_models.csv'


def _backup_models_csv():
    """Create a timestamped backup of production CSV before writing (failsafe against contamination)."""
    src = f'{MODELS_DIR}/crypto_deku_1_2_1_best_models.csv'
    if os.path.exists(src) and not MODELS_CSV_OVERRIDE:
        import shutil
        bak = f'{MODELS_DIR}/crypto_deku_1_2_1_best_models_backup.csv'
...
def run_mode_d_optuna(assets_list, horizon=PREDICTION_HORIZON, n_trials=DEKU_DEFAULT_TRIALS, resume=False):
    ...
    print("\n" + "=" * 70)
    print(f"  DEKU 1.2.1 MODE D: OPTUNA JOINT OPTIMIZATION -- {horizon}h HORIZON")
    ...
    for asset_name in assets_list:
        ...
        print(f"\n{'='*70}")
        print(f"  DEKU 1.2.1 OPTIMIZATION: {asset_name} ({horizon}h)")
        print(f"{'='*70}")
        ...
        # 3-fold rolling holdout: each fold trains on ~67%, validates on ~33%
        # Fold 1: train [0..60%), test [60..80%)
        # Fold 2: train [10..70%), test [70..90%)
        # Fold 3: train [20..80%), test [80..100%)
        # Optuna optimizes on training portion; holdout ranking averages across all 3 folds
        fold_train_frac = 0.60  # 60% train per fold
        fold_test_frac = 0.20   # 20% test per fold
        fold_stride = 0.10      # shift 10% between folds
        holdout_folds = []
        ...
        # Optuna trains on fold 1's training partition (earliest, most data before test)
        # This is the largest contiguous block that doesn't overlap any fold's test set
        ...
        study = optuna.create_study(
            ...
            study_name=f'deku_1_2_1_{asset_name}_{horizon}h',
        )
        ...
        # Each candidate is evaluated on all 3 folds; winner = best average holdout score
        # Deduplicate: keep only the best trial per unique (combo, window) pair
        # so holdout ranking compares genuinely different models, not gamma/feature variants
        seen_configs = set()
        unique_candidates = []
        ...
        N_CANDIDATES = min(15, len(unique_candidates))
        candidates = unique_candidates[:N_CANDIDATES]
        ...
        if min_fold_rows >= 200:
            for ci, trial in enumerate(candidates):
                ...
                for fi, (tr_s, tr_e, te_s, te_e) in enumerate(holdout_folds):
                    # Train on fold's training partition, evaluate on fold's test partition
                    fold_feat_train = features_np_all[tr_s:tr_e, :c_n_feat]
                    fold_labels_train = labels_np_all[tr_s:tr_e]
                    fold_closes_train = closes_np_all[tr_s:tr_e]
                ...
                # Last fold result (most recent data) for reporting
                last_fold_ret = fold_rets[-1]

                holdout_results.append((trial, avg_score, avg_ret, avg_acc, total_trades,
                                        avg_raw_pf, fold_scores, fold_rets, last_fold_ret))

            # Sort by average hold-out score (descending)
            ...
            print(f"  {'-'*125}")
            for i, (trial, avg_sc, avg_ret, avg_acc, tot_tr, avg_rpf, f_scores, f_rets, lf_ret) in enumerate(holdout_results[:10], 1):
        ...
        if full_result:
            ...
            # winner_ho = (trial, avg_score, avg_ret, avg_acc, total_trades, avg_raw_pf, fold_scores, fold_rets, last_fold_ret)
...
def run_mode_d(assets_list, horizon=PREDICTION_HORIZON, permtest=False, resume=False):
    ...
    if not MODELS_CSV_OVERRIDE:
        df_best.to_csv(f'{MODELS_DIR}/crypto_deku_1_2_1_best_models_mode_d.csv', index=False)
...
def run_strategy_comparison(assets_list, horizons=None):
    ...
    tcfg_path = f'{CONFIG_DIR}/trading_config_deku_1_2_1{metric_suffix}.json'
    try:
    ...
    for asset in assets_list:
        ...
        # Hold-out: evaluate strategies on last 33% of signals (matches fold 3's test window)
...
MODE_A_RESULTS_CSV = f'{MODELS_DIR}/testing_deku_1_2_1_a_results.csv'
...
def main():
    ...
    if cli_args and cli_args[0].upper() in ('B', 'D', 'DF', 'F', '5', '6', '7'):
        ...
        print("=" * 60)
        print(f"  DEKU 1.2.1: Mode {mode} | {','.join(assets_list)} | {','.join(str(h)+'h' for h in horizons)}{trials_str}")
        print("=" * 60)

    else:

        print("=" * 60)
        print("  CRYPTO HOURLY ML TRADING SYSTEM -- DEKU 1.2.1")
