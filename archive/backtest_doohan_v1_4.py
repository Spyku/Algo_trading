"""
Backtest: Doohan V1.4 top 6 candidates vs Deku Production
Loads all candidates from crypto_doohan_v1_4_best_models.csv (ranked by holdout).
Runs 1-week live replay at confidence thresholds 70%, 80%, 90%.
Saves the best live performer to crypto_doohan_v1_4_production.csv.

Usage:
    python backtest_doohan_v1_4.py              # backtest + save best
    python backtest_doohan_v1_4.py --no-save    # backtest only, don't save
"""
import sys, os, time
import pandas as pd
...
CANDIDATES_CSV = os.path.join('models', 'crypto_doohan_v1_4_best_models.csv')
PRODUCTION_CSV = os.path.join('models', 'crypto_doohan_v1_4_production.csv')
...
def main():
    ...
    if not os.path.exists(CANDIDATES_CSV):
        print(f"  ERROR: {CANDIDATES_CSV} not found. Run V1.4 Mode D first.")
        return
    ...
    print("=" * 80)
    print(f"  BACKTEST: DOOHAN V1.4 CANDIDATES vs DEKU — {','.join(assets)} {HORIZON}h")
    ...
    for asset in assets:
        ...
        for idx, row in asset_candidates.iterrows():
            ...
            samp = row.get('sampler', '?')
            label = f"V1.4 #{rank} {samp} (APF={ho_apf:.1f})"
        ...
        doohan_results = [(label, r) for label, r in results.items()
                          if r['cfg'].get('source') == 'doohan_v1.4'
    ...
    # ── Summary ──
    print(f"\n\n{'=' * 80}")
    print(f"  SUMMARY: DOOHAN V1.4 CANDIDATES vs DEKU — Last 7 days")
    print(f"{'=' * 80}")
