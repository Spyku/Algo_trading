"""
TESTING: V30.1 — Gamma Optimization for 30-Min System
============================================================
Tests 7 gamma values × 2 horizons (4h, 8h) to find optimal decay for V30.
...
Results saved to: models/testing_v30.1_results.csv
Charts saved to:  charts/v30.1_test/

Usage:
  python testing_v30.1.py                  # Run all 14 tests (7 gammas × 2 horizons)
  python testing_v30.1.py --resume         # Skip already-completed gamma/horizon combos
  python testing_v30.1.py --asset ETH      # Test ETH instead of BTC (default: BTC)

Based on crypto_trading_system_v30.py (V30 Cacarot).
"""
...
RESULTS_CSV = os.path.join(_SCRIPT_DIR, 'models', 'testing_v30.1_results.csv')
CHARTS_DIR_TEST = os.path.join(_SCRIPT_DIR, 'charts', 'v30.1_test')

# ============================================================
# Import V30 Cacarot — we'll call its functions directly
# ============================================================
import importlib
v30 = importlib.import_module('crypto_trading_system_v30')
...
def run_gamma_test(asset, gamma, horizon, resume=False):
    ...
    """
    print("\n" + "#" * 70)
    print(f"  GAMMA TEST: {asset} | gamma={gamma} | horizon={v30._horizon_label(horizon)}")
    print("#" * 70)

    t0 = time.time()

    # Override the gamma in the existing config (or create a fresh one)
    original_load = v30._load_mode_d_config
    ...
    # Patch the loader
    v30._load_mode_d_config = _patched_load

    # Override charts dir for isolation
    original_charts_dir = v30.CHARTS_DIR
    v30.CHARTS_DIR = CHARTS_DIR_TEST
    os.makedirs(CHARTS_DIR_TEST, exist_ok=True)

    try:
        v30.run_mode_d([asset], horizon=horizon)
    except Exception as e:
        print(f"  ERROR: {e}")
        import traceback
        traceback.print_exc()
        v30._load_mode_d_config = original_load
        v30.CHARTS_DIR = original_charts_dir
        return None
    finally:
        v30._load_mode_d_config = original_load
        v30.CHARTS_DIR = original_charts_dir
    ...
    # Read the result that was just saved to the V30 CSV
    csv_path = f'{v30.MODELS_DIR}/crypto_30m_best_models.csv'
...
def print_summary(asset):
    ...
    print("\n" + "=" * 90)
    print(f"  V30.1 GAMMA OPTIMIZATION RESULTS — {asset}")
    print("=" * 90)
    ...
    for h in HORIZONS:
        ...
        for _, row in h_df.iterrows():
            ...
            print(f"  {v30._horizon_label(int(row['horizon'])):<10} {gamma_str:<8} "
    ...
    for h in HORIZONS:
        ...
        if not h_df.empty:
            ...
            print(f"    {v30._horizon_label(h)}: gamma={best['gamma']:.3f} → "
...
def main():
    ...
    # Also support: python testing_v30.1.py ETH
    ...
    print("=" * 70)
    print(f"  V30.1 GAMMA OPTIMIZATION TEST")
    print(f"  Asset: {asset}")
    print(f"  Gammas: {GAMMAS_TO_TEST}")
    print(f"  Horizons: {[v30._horizon_label(h) for h in HORIZONS]}")
    ...
    for gamma in GAMMAS_TO_TEST:
        for horizon in HORIZONS:
            ...
            if resume and is_completed(asset, gamma, horizon):
                print(f"\n  [{test_num}/{total_tests}] gamma={gamma}, {v30._horizon_label(horizon)} — SKIPPED (already done)")
                skipped += 1
                continue

            print(f"\n  [{test_num}/{total_tests}] Testing gamma={gamma}, {v30._horizon_label(horizon)}...")
            ...
            if result:
                ...
                print(f"  FAILED: gamma={gamma}, {v30._horizon_label(horizon)}")
