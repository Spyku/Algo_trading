"""
Crypto Hourly Trading System — CASCA V1.1 (Fee-Aware Labels)
============================================================
ML trading system for BTC, ETH, XRP, DOGE with 4h and 8h horizons.
130 features -> walk-forward ML -> BUY/SELL/HOLD signals.

CASCA V1.1 — CASCA + fee-aware labels:
  - Labels: label=1 when future_return > 2×TRADING_FEE (0.22%)
    instead of return > 168h rolling median. Every correct BUY
    prediction is inherently profitable after round-trip fees.
  ...
  - Feature selection uses profit factor throughout
...
CLI Usage (skip all menus):
  ...
  python crypto_trading_system.py D ETH 8h
  python crypto_trading_system.py D BTC,ETH 4,8h

Outputs:
  charts/{ASSET}_backtest.png
  models/crypto_casca_v1.1_best_models.csv
...
def build_hourly_features(df_hourly, horizon=PREDICTION_HORIZON, verbose=True):
    ...
    # CASCA V1.1: Fee-aware labels — label=1 only when return covers round-trip fees
    # Every correct BUY prediction is inherently profitable after fees.
    # Replaces 168h rolling median (drift-robust but profit-blind).
    fee_threshold = 2 * TRADING_FEE  # 2 × 0.0011 = 0.0022 (0.22%) — covers BUY + SELL fees
    df['label'] = (future_return > fee_threshold).astype(int)
...
def _get_models_csv_path():
    ...
    return MODELS_CSV_OVERRIDE if MODELS_CSV_OVERRIDE else f'{MODELS_DIR}/crypto_casca_v1.1_best_models.csv'


def _backup_models_csv():
    """Create a timestamped backup of production CSV before writing (failsafe against contamination)."""
    src = f'{MODELS_DIR}/crypto_casca_v1.1_best_models.csv'
    if os.path.exists(src) and not MODELS_CSV_OVERRIDE:
        import shutil
        bak = f'{MODELS_DIR}/crypto_casca_v1.1_best_models_backup.csv'
...
def run_mode_b(assets_list, horizon_filter=None, skip_data_update=False):
    ...
    if not os.path.exists(_get_models_csv_path()):
        print("\nERROR: crypto_casca_v1.1_best_models.csv not found!")
...
def run_mode_d(assets_list, horizon=PREDICTION_HORIZON, permtest=False, resume=False):
    ...
    if not MODELS_CSV_OVERRIDE:
        df_best.to_csv(f'{MODELS_DIR}/crypto_casca_v1.1_best_models_mode_d.csv', index=False)
